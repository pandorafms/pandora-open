package PandoraOpen::GIS;
##########################################################################
# GIS Pandora Open functions.
...
PandoraOpen::GIS - Geographic Information System functions of Pandora Open
...
This module contains the B<GIS> (Geographic Information System) related  functions of B<Pandora Open>
...
sub distance_moved {
    ...
    my $asinaux =
      sqrt( $lat_aux +
          cos( $last_latitude * $to_radians ) * cos( $latitude * $to_radians )
          * $long_aux );
...
sub get_random_close_point {
    ...
    return ( $center_longitude, $center_latitude )
      if ( $pa_config->{'recon_location_scatter_radius'} == 0 );

    my $sign = int rand(2);
    my $longitude =
      ( $sign * (-1) + ( 1 - $sign ) ) *
      rand( $pa_config->{'recon_location_scatter_radius'} /
...
L<PandoraOpen::DB>, L<PandoraOpen::Tools>
