package PandoraOpen::PluginTools;
################################################################################
#
# Pandora Open Plugin functions library
#
...
sub csv_to_obj {
    ...
    # Clean \n\r
    my @hr_headers = map { trim(s/\"//gr) } split /,/, shift @lines;

    foreach my $line (@lines) {
        next if empty($line);

        my $i  = 0;
        my %hr = map { $hr_headers[ $i++ ] => trim(s/\"//gr) } split /,/, $line;
...
sub print_agent {
    ...
    if ( $group_password_specified == 0
        && !empty( $config->{'group_password'} ) )
    {
    ...
    elsif ( ref($modules_def) eq "HASH"
        && ( defined $modules_def->{'discovery'} ) )
    {
...
sub print_discovery_module {
    ...
    return
      if ( ref($global_data) ne "HASH"
        || !defined( $global_data->{'discovery'} ) );
...
sub print_module {
    ...
    $data->{tags} = (
        ...
        : (
            ...
            : ( $conf->{module_tag_list} ? $conf->{module_tag_list} : undef )
        )
    );
    ...
    $data->{interval} = (
        ...
        : (
            ...
            : ( $conf->{module_interval} ? $conf->{module_interval} : undef )
        )
    );
    ...
    $data->{module_group} = (
        ...
        : (
            ...
            : ( $conf->{module_group} ? $conf->{module_group} : undef )
        )
    );

    # Global instructions (if defined)
    $data->{unknown_instructions} = $conf->{unknown_instructions}
      unless ( defined( $data->{unknown_instructions} )
        || ( !defined( $conf->{unknown_instructions} ) ) );
    $data->{warning_instructions} = $conf->{warning_instructions}
      unless ( defined( $data->{warning_instructions} )
        || ( !defined( $conf->{warning_instructions} ) ) );
    $data->{critical_instructions} = $conf->{critical_instructions}
      unless ( defined( $data->{critical_instructions} )
        || ( !defined( $conf->{critical_instructions} ) ) );
    ...
    $data->{warning_inverse} = $data->{'winv'}
      if empty( $data->{warning_inverse} );
    $data->{critical_inverse} = $data->{'cinv'}
    ...
    if ( !( empty( $data->{module_group} ) ) ) {
        $xml_module .=
          "\t<module_group>" . $data->{module_group} . "</module_group>\n";
    }
    if ( !( empty( $data->{module_parent} ) ) ) {
        $xml_module .=
          "\t<module_parent>" . $data->{module_parent} . "</module_parent>\n";
    }
    ...
    if ( !( empty( $data->{critical_inverse} ) ) ) {
        $xml_module .=
            "\t<critical_inverse><![CDATA["
          . $data->{critical_inverse}
          . "]]></critical_inverse>\n";
    }
    if ( !( empty( $data->{warning_inverse} ) ) ) {
        $xml_module .=
            "\t<warning_inverse><![CDATA["
          . $data->{warning_inverse}
          . "]]></warning_inverse>\n";
    }
    if ( !( empty( $data->{min_warning_forced} ) ) ) {
        $xml_module .=
            "\t<min_warning_forced><![CDATA["
          . $data->{min_warning_forced}
          . "]]></min_warning_forced>\n";
    }
    if ( !( empty( $data->{max_warning_forced} ) ) ) {
        $xml_module .=
            "\t<max_warning_forced><![CDATA["
          . $data->{max_warning_forced}
          . "]]></max_warning_forced>\n";
    }
    if ( !( empty( $data->{min_critical_forced} ) ) ) {
        $xml_module .=
            "\t<min_critical_forced><![CDATA["
          . $data->{min_critical_forced}
          . "]]></min_critical_forced>\n";
    }
    if ( !( empty( $data->{max_critical_forced} ) ) ) {
        $xml_module .=
            "\t<max_critical_forced><![CDATA["
          . $data->{max_critical_forced}
          . "]]></max_critical_forced>\n";
    }
    if ( !( empty( $data->{str_warning_forced} ) ) ) {
        $xml_module .=
            "\t<str_warning_forced><![CDATA["
          . $data->{str_warning_forced}
          . "]]></str_warning_forced>\n";
    }
    if ( !( empty( $data->{str_critical_forced} ) ) ) {
        $xml_module .=
            "\t<str_critical_forced><![CDATA["
          . $data->{str_critical_forced}
          . "]]></str_critical_forced>\n";
    }
    if ( !( empty( $data->{max} ) ) ) {
    ...
    if ( !( empty( $data->{disabled} ) ) ) {
        $xml_module .=
          "\t<disabled><![CDATA[" . $data->{disabled} . "]]></disabled>\n";
    }
    ...
    if ( !( empty( $data->{status} ) ) ) {
        $xml_module .=
          "\t<status><![CDATA[" . $data->{status} . "]]></status>\n";
    }
    if ( !( empty( $data->{timestamp} ) ) ) {
        $xml_module .=
          "\t<timestamp><![CDATA[" . $data->{timestamp} . "]]></timestamp>\n";
    }
    if ( !( empty( $data->{custom_id} ) ) ) {
        $xml_module .=
          "\t<custom_id><![CDATA[" . $data->{custom_id} . "]]></custom_id>\n";
    }
    if ( !( empty( $data->{critical_instructions} ) ) ) {
        $xml_module .=
            "\t<critical_instructions><![CDATA["
          . $data->{critical_instructions}
          . "]]></critical_instructions>\n";
    }
    if ( !( empty( $data->{warning_instructions} ) ) ) {
        $xml_module .=
            "\t<warning_instructions><![CDATA["
          . $data->{warning_instructions}
          . "]]></warning_instructions>\n";
    }
    if ( !( empty( $data->{unknown_instructions} ) ) ) {
        $xml_module .=
            "\t<unknown_instructions><![CDATA["
          . $data->{unknown_instructions}
    ...
    if ( !( empty( $data->{module_ff_interval} ) ) ) {
        $xml_module .=
            "\t<module_ff_interval><![CDATA["
          . $data->{module_ff_interval}
          . "]]></module_ff_interval>\n";
    }
    if ( !( empty( $data->{crontab} ) ) ) {
        $xml_module .=
          "\t<crontab><![CDATA[" . $data->{crontab} . "]]></crontab>\n";
    }
    if ( !( empty( $data->{min_ff_event_normal} ) ) ) {
        $xml_module .=
            "\t<min_ff_event_normal><![CDATA["
          . $data->{min_ff_event_normal}
          . "]]></min_ff_event_normal>\n";
    }
    if ( !( empty( $data->{min_ff_event_warning} ) ) ) {
        $xml_module .=
            "\t<min_ff_event_warning><![CDATA["
          . $data->{min_ff_event_warning}
          . "]]></min_ff_event_warning>\n";
    }
    if ( !( empty( $data->{min_ff_event_critical} ) ) ) {
        $xml_module .=
            "\t<min_ff_event_critical><![CDATA["
          . $data->{min_ff_event_critical}
          . "]]></min_ff_event_critical>\n";
    }
    if ( !( empty( $data->{ff_type} ) ) ) {
        $xml_module .=
          "\t<ff_type><![CDATA[" . $data->{ff_type} . "]]></ff_type>\n";
    }
    ...
    if ( !( empty( $data->{each_ff} ) ) ) {
        $xml_module .=
          "\t<each_ff><![CDATA[" . $data->{each_ff} . "]]></each_ff>\n";
    }
    if ( !( empty( $data->{parent_unlink} ) ) ) {
        $xml_module .=
            "\t<module_parent_unlink><![CDATA["
          . $data->{parent_unlink}
    ...
    if ( !( empty( $data->{alerts} ) ) ) {
        foreach my $alert ( @{ $data->{alerts} } ) {
            $xml_module .=
              "\t<alert_template><![CDATA[" . $alert . "]]></alert_template>\n";
        }
    }
...
sub transfer_xml {
    ...
    if ( $xml =~ /\n/ || !-f $xml ) {
        ...
        if ( !( empty($name) ) ) {
            $file_name =
                $name . "."
              . sprintf( "%d", getCurrentUTimeMilis() . ( rand() * 10000 ) )
              . ".data";
        }
        else {
            # Inherit file name
        ...
        $conf->{temp} = $conf->{tmp}
          if ( empty( $conf->{temp} ) && defined( $conf->{tmp} ) );
        $conf->{temp} = $conf->{temporal}
          if ( empty( $conf->{temp} ) && defined( $conf->{temporal} ) );
        $conf->{temp} = $conf->{__system}->{tmp}
          if ( empty( $conf->{temp} ) && defined( $conf->{__system} ) )
          && ( ref( $conf->{__system} ) eq "HASH" );
        ...
        if ( empty($r) ) {
            print_stderror( $conf, "Cannot write to [" . $file_path . "]",
                $conf->{'debug'} );
            return;
        }
    ...
    $conf->{tentacle_client} = "tentacle_client"
      if empty( $conf->{tentacle_client} );
    ...
    if ( empty( $conf->{mode} ) ) {
        print_stderror( $conf,
"[ERROR] Nor \"mode\" nor \"transfer_mode\" defined in configuration."
        );
        return;
    }
...
`$conf->{tentacle_client} -v -a $conf->{tentacle_ip} -p $conf->{tentacle_port} $conf->{tentacle_opts} "$file_path"`;
            ...
            $msg =
`$conf->{tentacle_client} -v -a $conf->{tentacle_ip} -p $conf->{tentacle_port} $conf->{tentacle_opts} "$file_path" 2>&1`;
            $r = $?;
        }
        ...
        if ( $rc == 0 ) {
            print_stderror( $conf,
"[ERROR] There was a problem copying local file to $dest_dir: $!"
            );
            return;
        }
        else {
...
sub print_error {
    ...
    if (
        !(
               is_enabled( $conf->{informational_modules} )
            || is_enabled($always_show)
        )
      )
    {
        exit 1;
    ...
    print_module(
        ...
        {
            name => (
                empty( $conf->{'global_plugin_module'} )
                ? "Plugin execution result " . $0
                : $conf->{'global_plugin_module'}
            ),
            type  => "generic_proc",
            value => $value,
            desc  => $msg,
...
sub logger {
    ...
    print_error( $conf, "[ERROR] Log file is not defined.", 0, 1 )
      unless defined($file);
    ...
    if ( $log_aux_flag == 0 ) {
        ...
        # Log starts
        if ( !open( $logfile_fh, ">", $file ) ) {
            print_error( $conf, "[ERROR] Could not open logfile '$file'", 0,
                1 );
        }
        $log_aux_flag = 1;
    }
    ...
    print $logfile_fh strftime( "%Y-%m-%d %H:%M:%S", localtime() ) . " - "
      . $message . "\n";
    close($logfile_fh);
}
...
sub call_url {
    ...
    if ( empty( $_PluginTools_system->{ua} ) ) {
        return { error => "Uninitialized, please initialize UserAgent first" };
    }
...
sub init {
    ...
    eval {
        ...
        if ( defined( $options->{lwp_enable} ) ) {
            ...
            $conf->{'__system'}->{ua} =
              LWP::UserAgent->new( ( keep_alive => "10" ) );
            ...
            if (
                defined( $options->{ssl_verify} )
                && (   ( $options->{ssl_verify} eq "no" )
                    || ( !is_enabled( $options->{ssl_verify} ) ) )
              )
            {
...
sub join_by_field {
    ...
    foreach my $item ( @{$array_hashref} ) {
        $str .=
          ( defined( $item->{$field} ) ? $item->{$field} : '' ) . $separator;
    }
    chop($str);
...
sub read_configuration {
    ...
    if ( ( !empty(@ARGV) ) && ( -f $ARGV[0] ) ) {
        $config = merge_hashes( $config,
            parse_configuration( shift @ARGV, $separator, $custom_eval ) );
    }
    $config = merge_hashes( $config, parse_arguments( \@ARGV ) );

    if ( is_enabled( $config->{'as_agent_plugin'} ) ) {
        $config->{'as_server_plugin'} = 0
          if ( empty( $config->{'as_server_plugin'} ) );
    }
    else {
        $config->{'as_server_plugin'} = 1
          if ( empty( $config->{'as_server_plugin'} ) );
    }

    if ( is_enabled( $config->{'as_server_plugin'} ) ) {
        $config->{'as_agent_plugin'} = 0
          if ( empty( $config->{'as_agent_plugin'} ) );
    }
    else {
        $config->{'as_agent_plugin'} = 1
          if ( empty( $config->{'as_agent_plugin'} ) );
    }
...
sub parse_configuration {
    my ( $conf_file, $separator, $custom_eval, $detect_entities,
        $entities_list )
      = @_;
...
sub parse_php_configuration {
    ...
    open( my $_CFILE, "<", "$conf_file" ) or return;
...
sub process_performance {
    ...
    if ( empty($process) ) {
        ...
        my $out = trim(
`(FOR /F \"skip=2 tokens=2 delims=,\" %P IN ('typeperf \"\\Proceso($process)\\% de tiempo de procesador\" -sc 1') DO \@echo %P) | find /V /I \"...\"  2> $_PluginTools_system->{devnull}`
        );
        ...
        {
            $out = trim(
`(FOR /F \"skip=2 tokens=2 delims=,\" %P IN ('typeperf \"\\Process($process)\\% Processor Time\" -sc 1') DO \@echo %P) | find /V /I \"...\"  2> $_PluginTools_system->{devnull}`
            );
        }
        if (   ( $out =~ /member/i )
            || ( $out  =~ /error/i )
        ...
        $mem = (
            split /\s+/,
            trim(
`$_PluginTools_system->{ps} | $_PluginTools_system->{grep} \"$process\"`
            )
        )[-2];
        if ( !empty($mem) ) {
            $mem =~ s/,/./;
        ...
        $instances = trim(
            head(
`$_PluginTools_system->{ps} | $_PluginTools_system->{grep} "$process"| $_PluginTools_system->{wcl}`,
                1
            )
        );
        ...
        $cpu = trim(
`$_PluginTools_system->{ps} | $_PluginTools_system->{grep} -w "$process" | $_PluginTools_system->{grep} -v grep | awk 'BEGIN {sum=0} {sum+=\$2} END{print sum}'`
        );
        $mem = trim(
`$_PluginTools_system->{ps} | $_PluginTools_system->{grep} -w "$process" | $_PluginTools_system->{grep} -v grep | awk 'BEGIN {sum=0} {sum+=\$1} END{print sum}'`
        );
        $instances = trim(
`$_PluginTools_system->{ps} | $_PluginTools_system->{grep} -w "$process" | $_PluginTools_system->{grep} -v grep | $_PluginTools_system->{wcl}`
        );
    }
    elsif ( $^O =~ /hpux/ ) {
        $cpu = trim(
`UNIX95= ps -eo pcpu,comm | $_PluginTools_system->{grep} -w "$process" |  $_PluginTools_system->{grep} -v grep | awk 'BEGIN {sum=0} {sum+=\$1} END{printf("\%.2f",sum)}'`
        );
        $mem = trim(
`UNIX95= ps -eo vsz,comm | $_PluginTools_system->{grep} -w "$process" |  $_PluginTools_system->{grep} -v grep | awk 'BEGIN {sum=0} {sum+=(\$1*4096/1048576)} END{printf("\%.2f",sum)}'`
        );
        $instances = trim(
`$_PluginTools_system->{ps} | $_PluginTools_system->{grep} -w "$process" | $_PluginTools_system->{grep} -v grep | $_PluginTools_system->{wcl}`
        );
...
`UNIX95= ps -eo pcpu,comm | $_PluginTools_system->{grep} -w "$process" |  $_PluginTools_system->{grep} -v grep | awk 'BEGIN {sum=0} {sum+=\$1} END{printf("\%.2f",sum)}'`
        ...
        $mem = trim(
`UNIX95= ps -eo pmem,comm | $_PluginTools_system->{grep} -w "$process" |  $_PluginTools_system->{grep} -v grep | awk 'BEGIN {sum=0} {sum+=\$1} END{printf("\%.2f",sum)}'`
        );
        $instances = trim(
`$_PluginTools_system->{ps} | $_PluginTools_system->{grep} -w "$process" | $_PluginTools_system->{grep} -v grep | $_PluginTools_system->{wcl}`
        );
        ...
        $cpu = trim(
`ps -Ao comm,pcpu |grep $process | grep -v grep | awk 'BEGIN {sum=0} {sum+=\$2} END {print sum}'`
        );
        $mem = trim(
`ps au -A | grep $process |  grep -v grep | awk 'BEGIN {sum=0} {sum+=\$4} END {print sum}'`
        );
...
sub api_call {
    ...
    if ( ref $apidata eq "HASH" ) {
        ...
        $other_mode =
          "url_encode_separator_" . $apidata->{'url_encode_separator'}
          unless empty( $apidata->{'url_encode_separator'} );
...
sub api_create_custom_field {
    ...
    $call .=
      "apipass=" . $api_pass . "&user=" . $api_user . "&pass=" . $api_user_pass;
    ...
    if ( ref($rs) ne "HASH" ) {
        ...
        return {
            rs    => 1,
            error => 'Failed to reach API while creating custom field ['
              . $name . ']'
        };
    }
    ...
    if ( empty($rs) || ( $rs !~ /^\d+$/ || $rs eq "0" ) ) {
        return {
            rs    => 1,
            error => 'Failed while creating custom field ['
              . $name
              . '] => ['
              . $rs . ']'
        };
...
sub api_create_group {
    ...
    $other .= (
        empty( $group_config->{parent} )
        ? ''
        : $group_config->{parent} . '%7C&'
    );
    $other .=
      ( empty( $group_config->{desc} ) ? '' : $group_config->{desc} . '%7C&' );
    $other .= (
        empty( $group_config->{propagate} )
        ? ''
        : $group_config->{propagate} . '%7C&'
    );
    $other .= (
        empty( $group_config->{disabled} )
        ? ''
        : $group_config->{disabled} . '%7C&'
    );
    $other .= (
        empty( $group_config->{custom_id} )
        ? ''
        : $group_config->{custom_id} . '%7C&'
    );
    $other .= (
        empty( $group_config->{contact} )
        ? ''
        : $group_config->{contact} . '%7C&'
    );
    $other .= (
        empty( $group_config->{other} )
        ? ''
        : $group_config->{other} . '%7C&'
    );
...
sub snmp_walk {
    ...
    if ( $^O =~ /lin/i && "`which snmpwalk`" eq "" ) {
        return { 'error' => 'snmpwalk not found' };
    }

    $snmp->{extra} = '' unless defined $snmp->{extra};

    if (
        defined( $snmp->{version} )
        && (   ( $snmp->{version} eq "1" )
            || ( $snmp->{version} eq "2" )
            || ( $snmp->{version} eq "2c" ) )
      )
    {
        ...
        if ( defined $snmp->{port} ) {
            $cmd =
"snmpwalk -t $timeout $snmp->{extra} -On -v $snmp->{version} -c $snmp->{community} $snmp->{host}:$snmp->{port} $snmp->{oid}";
        }
        else {
            $cmd =
"snmpwalk -t $timeout $snmp->{extra} -On -v $snmp->{version} -c $snmp->{community} $snmp->{host} $snmp->{oid}";
        }
...
# Ex. snmpwalk -t $timeout $snmp->{extra} -On -v 3 -n "" -u MD5DESUser -a MD5 -A "The Net-SNMP Demo Password" -x DES -X "The Net-SNMP Demo Password" -l authPriv test.net-snmp.org system
        ...
        if ( $snmp->{securityLevel} =~ /^noAuthNoPriv$/i ) {
            ...
            if ( defined $snmp->{port} ) {
                $cmd =
"snmpwalk -t $timeout $snmp->{extra} -On -v $snmp->{version} -n \"$snmp->{context}\" -l $snmp->{securityLevel} $snmp->{host}:$snmp->{port} $snmp->{oid}";
            }
            else {
                $cmd =
"snmpwalk -t $timeout $snmp->{extra} -On -v $snmp->{version} -n \"$snmp->{context}\" -l $snmp->{securityLevel} $snmp->{host} $snmp->{oid}";
            }
        }
            ...
            if ( defined $snmp->{port} ) {
                $cmd =
"snmpwalk -t $timeout $snmp->{extra} -On -v $snmp->{version} -n \"$snmp->{context}\" -u $snmp->{securityName} -a $snmp->{authProtocol} -A $snmp->{authKey} -l $snmp->{securityLevel} $snmp->{host}:$snmp->{port} $snmp->{oid}";
            }
            else {
                $cmd =
"snmpwalk -t $timeout $snmp->{extra} -On -v $snmp->{version} -n \"$snmp->{context}\" -u $snmp->{securityName} -a $snmp->{authProtocol} -A $snmp->{authKey} -l $snmp->{securityLevel} $snmp->{host} $snmp->{oid}";
            }
        }
            ...
            if ( defined $snmp->{port} ) {
                $cmd =
"snmpwalk -t $timeout $snmp->{extra} -On -v $snmp->{version} -n \"$snmp->{context}\" -u $snmp->{securityName} -l $snmp->{securityLevel} -a $snmp->{authProtocol} -A $snmp->{authKey} -x $snmp->{privProtocol} -X $snmp->{privKey} $snmp->{host}:$snmp->{port} $snmp->{oid}";
            }
            else {
                $cmd =
"snmpwalk -t $timeout $snmp->{extra} -On -v $snmp->{version} -n \"$snmp->{context}\" -u $snmp->{securityName} -l $snmp->{securityLevel} -a $snmp->{authProtocol} -A $snmp->{authKey} -x $snmp->{privProtocol} -X $snmp->{privKey} $snmp->{host} $snmp->{oid}";
            }
        }
    }
    else {
        return { error => "Only SNMP 1 2 2c and 3 are supported." };
    }

    #print STDERR "Launching $cmd\n";
    my $result = `$cmd 2>/dev/null`;
    if ( $? != 0 ) {
        return { error => "No response from " . trim( $snmp->{host} ) };
    }
    return $result;
...
sub snmp_get {
    ...
    if ( !defined $snmp->{version} ) {
        ...
        return { 'error' => "Destination host must be defined." };
    }
    elsif ( !defined $snmp->{oid} ) {
        return { 'error' => "OID must be defined" };
    }
    else {
        $snmp->{extra}     = ''       unless defined $snmp->{extra};
            ...
            || ( $snmp->{version} eq "2c" ) )
        {
            ...
            if ( defined $snmp->{port} ) {
                $cmd =
"snmpget -r $retries -t $timeout $snmp->{extra} -On -v $snmp->{version} -c $snmp->{community} $snmp->{host}:$snmp->{port} $snmp->{oid}";
            }
            else {
                $cmd =
"snmpget -r $retries -t $timeout $snmp->{extra} -On -v $snmp->{version} -c $snmp->{community} $snmp->{host} $snmp->{oid}";
            }
...
# Ex. snmpget -r $retries -t $timeout $snmp->{extra} -On -v 3 -n "" -u MD5DESUser -a MD5 -A "The Net-SNMP Demo Password" -x DES -X "The Net-SNMP Demo Password" -l authPriv test.net-snmp.org system
            ...
            if ( $snmp->{securityLevel} =~ /^noAuthNoPriv$/i ) {
                ...
                if ( defined $snmp->{port} ) {
                    $cmd =
"snmpget -r $retries -t $timeout $snmp->{extra} -On -v $snmp->{version} -n \"$snmp->{context}\" -l $snmp->{securityLevel} $snmp->{host}:$snmp->{port} $snmp->{oid}";
                }
                else {
                    $cmd =
"snmpget -r $retries -t $timeout $snmp->{extra} -On -v $snmp->{version} -n \"$snmp->{context}\" -l $snmp->{securityLevel} $snmp->{host} $snmp->{oid}";
                }
            }
            elsif ( $snmp->{securityLevel} =~ /^authNoPriv$/i ) {
                ...
                if ( defined $snmp->{port} ) {
                    $cmd =
"snmpget -r $retries -t $timeout $snmp->{extra} -On -v $snmp->{version} -n \"$snmp->{context}\" -u $snmp->{securityName} -a $snmp->{authProtocol} -A $snmp->{authKey} -l $snmp->{securityLevel} $snmp->{host}:$snmp->{port} $snmp->{oid}";
                }
                else {
                    $cmd =
"snmpget -r $retries -t $timeout $snmp->{extra} -On -v $snmp->{version} -n \"$snmp->{context}\" -u $snmp->{securityName} -a $snmp->{authProtocol} -A $snmp->{authKey} -l $snmp->{securityLevel} $snmp->{host} $snmp->{oid}";
                }
            }
            elsif ( $snmp->{securityLevel} =~ /^authPriv$/i ) {
                ...
                if ( defined $snmp->{port} ) {
                    $cmd =
"snmpget -r $retries -t $timeout $snmp->{extra} -On -v $snmp->{version} -n \"$snmp->{context}\" -u $snmp->{securityName} -l $snmp->{securityLevel} -a $snmp->{authProtocol} -A $snmp->{authKey} -x $snmp->{privProtocol} -X $snmp->{privKey} $snmp->{host}:$snmp->{port} $snmp->{oid}";
                }
                else {
                    $cmd =
"snmpget -r $retries -t $timeout $snmp->{extra} -On -v $snmp->{version} -n \"$snmp->{context}\" -u $snmp->{securityName} -l $snmp->{securityLevel} -a $snmp->{authProtocol} -A $snmp->{authKey} -x $snmp->{privProtocol} -X $snmp->{privKey} $snmp->{host} $snmp->{oid}";
                }
            }
            else {
                return { 'error' => "Security Level not defined." };
            }
        }
    }
...
sub encrypt {
    ...
    return
      unless (
        load_perl_modules( 'Crypt::CBC', 'Crypt::OpenSSL::AES', 'Digest::SHA' )
        == 1 );
    ...
    my $processed_salt =
      substr( Digest::SHA::hmac_sha256_base64( $salt, '' ), 0, 16 );
...
sub get_unix_time {
    ...
    eval {
        ...
        my ( $mday, $mon, $year, $hour, $min, $sec ) =
          split( /[\s$separator_dates$separator_hours]+/, $str_time );
...
sub load_perl_modules {
    my @missing_modules = ();
    foreach (@_) {
        eval "require $_";  ## no critic (BuiltinFunctions::ProhibitStringyEval)
...
sub extract_dbpass {
    ...
    if ( !empty( $config->{'dbpass_file'} ) ) {
        if ( -f $config->{'dbpass_file'} ) {
            eval {
                open( my $pf, "<", $config->{'dbpass_file'} )
                  or die( "Cannot open file " . $config->{'dbpass_file'} );
        ...
        if ( empty( $config->{'dbpass'} ) ) {
            print_error( $config, "Failed to read password from environment",
                1 );
            exit;
        }
    }
...
sub get_addresses {
    ...
    if ( is_enabled( $config->{'local'} ) ) {
        ...
        if ( -x "/bin/ip" || -x "/sbin/ip" || -x "/usr/sbin/ip" ) {
            @address_list =
`ip addr show 2>$DevNull | sed -e '/127.0.0/d' -e '/[0-9]*\\.[0-9]*\\.[0-9]*/!d' -e 's/^[ \\t]*\\([^ \\t]*\\)[ \\t]*\\([^ \\t]*\\)[ \\t].*/\\2/' -e 's/\\/.*//'`;
        }
        else {
            @address_list =
`ifconfig -a 2>$DevNull | sed -e '/127.0.0/d' -e '/[0-9]*\\.[0-9]*\\.[0-9]*/!d' -e 's/^[ \\t]*\\([^ \\t]*\\)[ \\t]*\\([^ \\t]*\\)[ \\t].*/\\2/' -e 's/.*://'`;
        }
