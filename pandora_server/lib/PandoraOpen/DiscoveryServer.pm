package PandoraOpen::DiscoveryServer;
################################################################################
# Pandora Open Discovery Server.
# Pandora Open. the Flexible Monitoring System. http://www.pandorafms.org
################################################################################
...
use PandoraOpen::ProducerConsumerServer;
use PandoraOpen::GIS;
use PandoraOpen::Recon::Base;

# Inherits from PandoraOpen::ProducerConsumerServer
our @ISA = qw(PandoraOpen::ProducerConsumerServer);
...
sub new {
    ...
    return
      unless ( defined( $config->{'reconserver'} )
        && $config->{'reconserver'} == 1 )
      || ( defined( $config->{'discoveryserver'} )
        && $config->{'discoveryserver'} == 1 );
    ...
    db_do(
        $dbh,
'UPDATE trecon_task  SET utimestamp = 0 WHERE id_recon_server = ? AND status <> -1 AND interval_sweep > 0',
    ...
    db_do(
        $dbh,
'UPDATE trecon_task  SET status = -1, summary = "cancelled" WHERE id_recon_server = ? AND status <> -1 AND interval_sweep = 0',
        get_server_id( $dbh, $config->{'servername'}, DISCOVERYSERVER )
    );
    ...
    my $self = $class->SUPER::new(
        ...
        \&PandoraOpen::DiscoveryServer::data_producer,
        \&PandoraOpen::DiscoveryServer::data_consumer, $dbh
    );
...
sub data_producer {
    ...
    my $server_id =
      get_server_id( $dbh, $pa_config->{'servername'}, $self->getServerType() );
    ...
    if ( pandora_is_master( $pa_config, $dbh ) == 0 ) {
      ...
      AND ((utimestamp = 0 AND interval_sweep != 0 OR status = 1)
        OR (status < 0 AND interval_sweep > 0 AND (utimestamp + interval_sweep) < UNIX_TIMESTAMP())
        OR (status < 0 AND utimestamp = 0 AND interval_sweep = 0))',
            $server_id, DISCOVERYSERVER
        );
    }
...
sub data_consumer {
    ...
    # Get recon task data
    my $task =
      get_db_single_row( $dbh, 'SELECT * FROM trecon_task WHERE id_rt = ?',
        $task_id );
    ...
    # Is it a discovery app?
        ...
        logger( $pa_config,
            'Starting recon task for net ' . $task->{'subnet'} . '.', 10 );
    }
    ...
    eval {
        ...
        my @communities =
          split( /,/, safe_output( $task->{'snmp_community'} ) );
        my @auth_strings = ();
        if ( defined( $task->{'auth_strings'} ) ) {
            @auth_strings =
              split( /,/, safe_output( $task->{'auth_strings'} ) );
        }

        my $main_event =
          pandora_event( $pa_config, "[Discovery] Execution summary",
        ...
        if ( $task->{'type'} == DISCOVERY_APP_SAP ) {
            ...
            if ( defined( $task->{'field4'} ) && $task->{'field4'} ne "" ) {
                ...
                $task->{'sap_license'} =
                  pandora_get_config_value( $dbh, 'sap_license' );
            }
            ...
            {
                my $key = credential_store_get_key( $pa_config, $dbh,
                    $task->{'auth_strings'} );
        ...
        my $recon = new PandoraOpen::Recon::Base(
            parent                    => $self,
            ...
            %{$pa_config},
            task_data  => $task,
            public_url => PandoraOpen::Config::pandora_get_tconfig_token(
                $dbh, 'public_url', ''
            ),
            %cnf_extra
        );
    ...
    if ($@) {
        logger(
            $pa_config,
            'Cannot execute Discovery task: '
              . safe_output( $task->{'name'} )
              . $@,
            10
        );
...
sub update_recon_task {
    ...
    db_do( $dbh,
        'UPDATE trecon_task SET utimestamp = ?, status = ? WHERE id_rt = ?',
        time(), $status, $id_task );
}
...
sub exec_recon_script {
    ...
    my $script = get_db_single_row(
        $dbh,
        'SELECT * FROM trecon_script WHERE id_recon_script = ?',
        $task->{'id_recon_script'}
    );
    return -1 unless defined($script);

    logger( $pa_config,
        'Executing recon script ' . safe_output( $script->{'name'} ), 10 );
    ...
    if ($macros) {
        eval { $decoded_macros = p_decode_json( $pa_config, $macros ); };
    }
    ...
    if ( -x $command ) {
        ...
        log_execution( $pa_config, $task->{'id_rt'}, "$command $args",
            $exec_output );
...
# Only update the timestamp in case something went wrong. The script should set the status.
    db_do( $dbh, 'UPDATE trecon_task SET utimestamp = ? WHERE id_rt = ?',
        time(), $task->{'id_rt'} );

    logger( $pa_config,
        'Done executing recon script ' . safe_output( $script->{'name'} ), 10 );
    return 0;
}
...
sub exec_recon_app {
    ...
    my @executions =
      get_db_rows( $dbh,
        'SELECT * FROM tdiscovery_apps_executions WHERE id_app = ?',
        $task->{'id_app'} );
    my @scripts =
      get_db_rows( $dbh,
        'SELECT * FROM tdiscovery_apps_scripts WHERE id_app = ?',
        $task->{'id_app'} );
...
# NOTE: Add the redirection before calling subst_alert_macros to prevent it from escaping quotes.
        my $cmd =
            $pa_config->{'plugin_exec'} . ' '
          . $task->{'executions_timeout'} . ' '
          . subst_alert_macros(
            safe_output( $execution->{'execution'} ) . ' 2>&1', \%macros );
        logger(
            $pa_config,
            'Executing command for recon app ID '
              . $task->{'id_app'} . ': '
              . $cmd,
            10
        );
        ...
        if ( !defined($output_json) ) {
            push( @summary,
                "The execution returned no output. Is the server out of memory?"
            );
            next;
        }
        ...
        # Update the progress.
        update_recon_task( $dbh, $task->{'id_rt'},
            int( ( 100 * ( $i + 1 ) ) / scalar(@executions) ) );
    }
    ...
    if ( !defined($summary_json) ) {
        logger( $pa_config,
            'Invalid summary for recon app ID ' . $task->{'id_app'}, 10 );
    }
    else {
        db_do( $dbh, "UPDATE trecon_task SET summary=? WHERE id_rt=?",
            $summary_json, $task->{'id_rt'} );
        pandora_audit(
            $pa_config,
            'Discovery task'
              . ' Executed task '
...
sub get_recon_app_macros {
    ...
    my @macro_array =
      get_db_rows( $dbh,
        'SELECT * FROM tdiscovery_apps_tasks_macros WHERE id_task = ?',
        $task->{'id_rt'} );
    foreach my $macro_item (@macro_array) {
        ...
        if ( defined($value_array) && ref($value_array) eq 'ARRAY' ) {
            ...
            foreach my $value_item ( @{$value_array} ) {
                push(
                    ...
                    get_recon_macro_value(
                        $pa_config, $dbh, $macro_type, $value_item
                    )
                );
            }
            ...
            if ( !defined($computed_value) ) {
                logger(
                    $pa_config,
                    "Error encoding macro $macro_name for task ID "
                      . $task->{'id_rt'},
                    10
                );
                next;
            ...
            # Single value macro.
            $computed_value =
              get_recon_macro_value( $pa_config, $dbh, $macro_type,
                $macro_value );
        }
...
sub dump_recon_app_macros {
    ...
    my @macro_array = get_db_rows(
        $dbh,
'SELECT * FROM tdiscovery_apps_tasks_macros WHERE id_task = ? AND temp_conf = 1',
        $task->{'id_rt'}
    );
    foreach my $macro_item (@macro_array) {
        ...
        if ($@) {
            logger(
                $pa_config,
                "Error writing macro $macro_name for task ID "
                  . $task->{'id_rt'}
                  . " to disk: $@",
                10
            );
...
# Compose the full path to the script: <incoming dir>/discovery/<app short name>/<script>
        my $app =
          get_db_single_row( $dbh,
            'SELECT short_name FROM tdiscovery_apps WHERE id_app = ?',
            $task->{'id_app'} );
        if ( !defined($app) ) {
            logger( $pa_config,
                "Discovery app with ID " . $task->{'id_app'} . " not found.",
                10 );
            next;
        }
...
sub get_recon_macro_value {
    ...
    # Name of the module if it exists. Empty otherwise.
        ...
        my $module_id =
          get_db_value( $dbh,
            "SELECT id_agente_modulo FROM tagente_modulo WHERE nombre = ?",
            safe_input($value) );
        if ( $module_id > 0 ) {
            $ret = $value;
        }
...
sub get_recon_credential_macro {
    ...
    my $cred =
      get_db_single_row( $dbh,
        'SELECT * FROM tcredential_store WHERE identifier = ?',
        $credential_id );
    ...
    if ( !defined($cred_json) ) {
        ...
        if ( !defined($cred_json) ) {
            logger( $pa_config,
                "Error encoding credential $credential_id to JSON.", 10 );
            return '';
        }
    }
...
################################################################################
sub PandoraOpen::Recon::Base::guess_os {
    ...
    if ( -x $self->{'pa_config'}->{'nmap'} ) {
        my $return =
`"$self->{pa_config}->{nmap}" -sSU -T5 -F -O --osscan-limit $device 2>$DEVNULL`;
        return OS_OTHER if ( $? != 0 );
        my ( $str_os, $os_version );
        if ( $return =~
            /Aggressive OS guesses:(.*?)(?>\(\d+%\),)|^OS details:(.*?)$/mi )
        {
            if ( defined($1) && $1 ne "" ) {
                $str_os = $1;
            }
            ...
            my $pandora_os_name =
              pandora_get_os_by_id( $self->{'dbh'}, $pandora_os );

            if ( defined($return_version_only) && $return_version_only == 1 ) {
...
################################################################################
sub PandoraOpen::Recon::Base::tcp_scan {
    my ( $self, $host ) = @_;
...
################################################################################
sub PandoraOpen::Recon::Base::test_module {
    my ( $self, $addr, $module ) = @_;

    # Default values.
    my $test = { %{$module}, 'ip_target' => $addr, };
    ...
    if ( is_enabled( $module->{'__module_component'} ) ) {
        ...
        $module->{'type'} = $module->{'module_type'}
          if is_empty( $module->{'type'} );

        if ( defined( $module->{'type'} ) ) {
            if ( !defined( $self->{'module_types'}{ $module->{'type'} } ) ) {
                $self->{'module_types'}{ $module->{'type'} } =
                  get_module_id( $self->{'dbh'}, $module->{'type'} );
            }
    ...
    if ( $test->{'id_tipo_modulo'} >= 15 && $test->{'id_tipo_modulo'} <= 18 ) {
        ...
        # SNMP
        $value = $self->call( 'snmp_get_value', $test->{'ip_target'},
            $test->{'snmp_oid'} );
    }
        ...
        $value = pandora_ping_latency(
            $self->{'pa_config'},   $test->{'ip_target'},
            $test->{'max_timeout'}, $test->{'max_retries'},
        );

    }
    elsif (
        ( $test->{'id_tipo_modulo'} >= 1 && $test->{'id_tipo_modulo'} <= 5 )
        || (   $test->{'id_tipo_modulo'} >= 21
            && $test->{'id_tipo_modulo'} <= 23 )
      )
    {
...
'SELECT * FROM tplugin WHERE name = "Network&#x20;bandwidth&#x20;SNMP"',
                ...
                # Decode macros.
                my $macros = p_decode_json( $self->{'config'},
                    safe_output( $test->{'macros'} ) );

                my %macros = %{$macros};
                if ( ref($macros) eq "HASH" ) {
                    foreach my $macro_id ( keys(%macros) ) {
                        my $macro_field =
                          safe_output( $macros{$macro_id}{'macro'} );
                        my $macro_desc =
                          safe_output( $macros{$macro_id}{'desc'} );
                        my $macro_value =
                          ( defined( $macros{$macro_id}{'hide'} )
                              && $macros{$macro_id}{'hide'} eq '1' )
                          ? pandora_output_password( $self->{'config'},
                            safe_output( $macros{$macro_id}{'value'} ) )
                          : safe_output( $macros{$macro_id}{'value'} );

                        # build parameters to invoke plugin
                        $parameters =~
                          s/\'$macros{$macro_id}{'macro'}\'/$macro_value/g;
    ...
    elsif ( $test->{'id_tipo_modulo'} >= 8 && $test->{'id_tipo_modulo'} <= 11 )
    {
        # TCP
        return 0
          unless is_numeric( $test->{'tcp_port'} )
          && $test->{'tcp_port'} > 0
          && $test->{'tcp_port'} <= 65535;

        my $result;

        PandoraOpen::NetworkServer::pandora_query_tcp(
            $self->{'pa_config'},   $test->{'tcp_port'},
            $test->{'ip_target'},   \$result,
            \$value,                $test->{'tcp_send'},
            $test->{'tcp_rcv'},     $test->{'id_tipo_modulo'},
            $test->{'max_timeout'}, $test->{'max_retries'},
            '<Discovery testing>',
        );
    ...
    if (
        is_in_array(
            [
                1,  2,  4,  5,  6,  7,  8,  9,  11, 15, 16, 18,
                21, 22, 25, 30, 31, 32, 34, 35, 37
            ],
            $test->{'id_tipo_modulo'}
        )
      )
    {
        ...
        if (
            (
                !defined( $test->{'min_critical'} )
                || $test->{'min_critical'} == 0
            )
            && ( !defined( $test->{'max_critical'} )
                || $test->{'max_critical'} == 0 )
          )
        {
        ...
        if ( !is_enabled( $test->{'critical_inverse'} ) ) {
            return 0
              if !is_empty( $test->{'str_critical'} )
              && $value =~ /$test->{'str_critical'}/;
        }
        else {
            return 0
              if !is_empty( $test->{'str_critical'} )
...
################################################################################
sub PandoraOpen::Recon::Base::create_interface_modules {
    my ( $self, $device ) = @_;
    ...
    if ( defined( $self->{'snmp3_auth_key'}{$device} ) ) {
        $snmp3_creds =
          $self->snmp3_credentials( $self->{'snmp3_auth_key'}{$device} );
    }
    my $snmp3_params = {
        'custom_string_1'  => '',
        'custom_string_2'  => '',
    ...
    my @output = $self->snmp_get_value_array( $device,
        $PandoraOpen::Recon::Base::IFINDEX );
    foreach my $if_index (@output) {
        ...
        if ( $self->{'task_data'}{'snmp_skip_non_enabled_ifs'} == 1 ) {
            ...
            my $if_status = $self->snmp_get_value( $device,
                "$PandoraOpen::Recon::Base::IFOPERSTATUS.$if_index" );
        ...
        my $if_desc =
          ( $mac ne '' ? "MAC $mac " : '' ) . ( $ip ne '' ? "IP $ip" : '' );

        # Get the name of the network interface.
        my $if_name = $self->snmp_get_value( $device,
            "$PandoraOpen::Recon::Base::IFNAME.$if_index" );
...
'The current operational state of the interface: up(1), down(2), testing(3), unknown(4), dormant(5), notPresent(6), lowerLayerDown(7)',
        ...
        my $if_hc_in_octets = $self->snmp_get_value( $device,
            "$PandoraOpen::Recon::Base::IFHCINOCTECTS.$if_index" );
...
'The total number of octets received on the interface, including framing characters.'
                    ...
                    'snmp_community'   => $community,
                    'snmp_oid'         =>
                      "$PandoraOpen::Recon::Base::IFINOCTECTS.$if_index",
                    'unit' => safe_input('bytes/s')
        ...
        my $if_hc_out_octets = $self->snmp_get_value( $device,
            "$PandoraOpen::Recon::Base::IFHCOUTOCTECTS.$if_index" );
...
'The total number of octets transmitted out of the interface, including framing characters.'
                    ...
                    'snmp_community'   => $community,
                    'snmp_oid'         =>
                      "$PandoraOpen::Recon::Base::IFOUTOCTECTS.$if_index",
                    'unit' => safe_input('bytes/s')
                }
            );
        }
...
'SELECT id, macros FROM tplugin WHERE name = "Network&#x20;bandwidth&#x20;SNMP"',
        ...
        if ( ref($macros) eq "HASH" ) {
            ...
            # Hash identifier.
            $macros->{'13'}->{'value'} =
              PandoraOpen::Tools::generate_agent_name_hash( $if_name, $device );
...
################################################################################
sub PandoraOpen::Recon::Base::create_wmi_modules {
    my ( $self, $target ) = @_;
    ...
    # Add modules.
    # CPU.
    my @cpus = $self->wmi_get_value_array( $target,
        'SELECT DeviceId FROM Win32_Processor', 0 );
    foreach my $cpu (@cpus) {
        $self->add_module(
            $target,
            {
...
"SELECT LoadPercentage FROM Win32_Processor WHERE DeviceId=\'$cpu\'",
    ...
    # Memory.
    my $mem = $self->wmi_get_value( $target,
...
"SELECT FreePhysicalMemory, TotalVisibleMemorySize FROM Win32_OperatingSystem",
    ...
    # Disk.
    my @units = $self->wmi_get_value_array( $target,
        'SELECT DeviceID FROM Win32_LogicalDisk', 0 );
...
"SELECT FreeSpace FROM Win32_LogicalDisk WHERE DeviceID='$unit'",
                ...
                'name'        => "FreeDisk $unit",
                'descripcion' =>
                  safe_input('Available disk space in kilobytes'),
                'id_tipo_modulo' => 1,
                'id_modulo'      => 6,
                'unit'           => 'KB',
            }
...
################################################################################
sub PandoraOpen::Recon::Base::create_network_profile_modules {
    my ( $self, $device ) = @_;
    ...
    if ( is_enabled( $self->{'task_data'}{'auto_monitor'} ) ) {
        ...
        my @pen_templates =
          get_pen_templates( $self->{'dbh'}, $self->get_pen($device) );
    ...
    foreach my $t_id (@template_ids) {
        ...
        my @np_components = get_db_rows( $self->{'dbh'},
            'SELECT * FROM tnetwork_profile_component WHERE id_np = ?', $t_id );
        ...
        foreach my $np_component (@np_components) {
            ...
            # SNMP Modules
            if (   $self->is_snmp_discovered($device)
                && $component->{'type'} >= 15
                && $component->{'type'} <= 18 )
            {
                my $snmp3_creds = undef;
                ...
                if ( defined( $self->{'snmp3_auth_key'}{$device} ) ) {
                    $snmp3_creds = $self->snmp3_credentials(
                        $self->{'snmp3_auth_key'}{$device} );
                }
                my $snmp3_params = {
                    'custom_string_1'  => '',
                    'custom_string_2'  => '',
                ...
                if ( defined($snmp3_creds) ) {
                    ...
                    $snmp3_params = {
                        'custom_string_1' =>
                          $snmp3_creds->{'snmp_privacy_method'},
                        'custom_string_2' =>
                          $snmp3_creds->{'snmp_privacy_pass'},
                        'custom_string_3' =>
                          $snmp3_creds->{'snmp_security_level'},
                        'plugin_parameter' =>
                          $snmp3_creds->{'snmp_auth_method'},
                        'plugin_user' => $snmp3_creds->{'snmp_auth_user'},
                        'plugin_pass' => $snmp3_creds->{'snmp_auth_pass'}
                    };
                }
...
################################################################################
sub PandoraOpen::Recon::Base::get_credentials {
    my ( $self, $key_index, $product ) = @_;

    my $cred = credential_store_get_key( $self->{'pa_config'}, $self->{'dbh'},
        $key_index );
...
################################################################################
sub PandoraOpen::Recon::Base::report_scanned_agents {
    my ( $self, $force ) = @_;
      ...
      )
    {
        ...
        foreach my $row (@rows) {
            ...
            eval {
                local $SIG{__DIE__};
                $data = p_decode_json( $self->{'pa_config'},
                    decode_base64( $row->{'data'} ) );
            };
            if ($@) {
                ...
                || $force_creation )
            {
                ...
                $self->call( 'message',
                    "Agent accepted: " . $data->{'agent'}{'nombre'}, 5 );
                ...
                if ( defined($agent_id) && $agent_id > 0 ) {
                    $agent_data =
                      get_db_single_row( $self->{'dbh'},
                        'SELECT * FROM tagente WHERE id_agente = ?',
                        $agent_id );
                    $agent_learning = $agent_data->{'modo'}
                      if ref($agent_data) eq 'HASH';
                }
                ...
                if ( !defined($agent_learning) ) {
                   ...
                   # Check if has been created by another process, if not found.
                    $agent_data =
                      PandoraOpen::Core::locate_agent( $self->{'pa_config'},
                        $self->{'dbh'}, $data->{'agent'}{'direccion'} )
                      if ref($agent_data) ne 'HASH';

                    $agent_id = $agent_data->{'id_agente'}
                      if ref($agent_data) eq 'HASH';
                    if ( ref($agent_data) eq 'HASH'
                        && $agent_data->{'modo'} != 1 )
                    {
                        ...
                        || !defined($agent_data) )
                    {
                        ...
                        if ( ref( $data->{'other_ips'} ) eq 'ARRAY' ) {
                            foreach my $ip_addr ( @{ $data->{'other_ips'} } ) {
                                my $addr_id =
                                  get_addr_id( $self->{'dbh'}, $ip_addr );
                                $addr_id =
                                  add_address( $self->{'dbh'}, $ip_addr )
                                  unless ( $addr_id > 0 );
                                next unless ( $addr_id > 0 );

                                # Assign the new address to the agent
                                my $agent_addr_id =
                                  get_agent_addr_id( $self->{'dbh'}, $addr_id,
                                    $agent_id );
                                if ( $agent_addr_id <= 0 ) {
                                    db_do(
                                        $self->{'dbh'},
'INSERT INTO taddress_agent (`id_a`, `id_agent`)
...
'INSERT INTO taddress_agent (`id_a`, `id_agent`)
                ...
                $self->call( 'message',
                    "Agent id: " . $data->{'agent'}{'agent_id'}, 5 );
               ...
               # Do not create any modules if the agent is not in learning mode.
                        ...
                        if ( $module->{'name'} ne 'Host Alive' ) {
                            next
                              unless ( is_enabled( $module->{'checked'} )
                                || $force_creation );
                        }

                        $self->call( 'message',
                            "[$agent_id] Module: " . $module->{'name'}, 5 );
               ...
               WHERE id_agente = ? AND nombre = ?',
                        ...
                        if ( !is_enabled($agentmodule_id) ) {
                            ...
                            my $description =
                              safe_output( $module->{'descripcion'} );
        ...
        foreach my $agent (@agents) {
            ...
            # Get parent id.
            my $parent = PandoraOpen::Core::locate_agent( $self->{'pa_config'},
                $self->{'dbh'}, $agent->{'parent'} );
    ...
    if ( ( scalar(@hosts) ) > 0 ) {
        my ( $progress, $step ) =
          ( 90, 10.0 / scalar(@hosts) );    # From 90% to 100%.
        ...
        foreach my $addr ( keys %{ $self->{'agents_found'} } ) {
            ...
            $self->{'agents_found'}->{$addr}{'agent'}{'id_os'} =
              $self->guess_os($addr);
            ...
            $self->{'agents_found'}->{$addr}{'agent'}{'os_version'} =
              $os_version;
            ...
            eval {
                ...
                $encoded = encode_base64(
                    p_encode_json(
                        $self->{'pa_config'}, $self->{'agents_found'}->{$addr}
                    )
                );
            };
...
'index.php?sec=gservers&sec2=godmode/servers/discovery&wiz=tasklist#'
        ...
        $notification->{'mensaje'} =
          safe_input( 'Discovery task (host&devices) \''
              . safe_output( $self->{'task_data'}{'name'} )
        ...
        # Create message
        my $notification_id =
          db_process_insert( $self->{'dbh'}, 'id_mensaje', 'tmensajes',
            $notification );

        if ( is_enabled($notification_id) ) {
            my @users =
              notification_get_users( $self->{'dbh'}, 'System status' );
            my @groups =
              notification_get_groups( $self->{'dbh'}, 'System status' );
...
################################################################################
sub PandoraOpen::Recon::Base::apply_monitoring {
    my ($self) = @_;
...
################################################################################
# Connect the given devices in the Pandora Open database.
################################################################################
sub PandoraOpen::Recon::Base::connect_agents {
    ...
    my $module_name_1 =
      $if_1 eq 'Host Alive' ? 'Host Alive' : "${if_1}_ifOperStatus";
    my $module_name_2 =
      $if_2 eq 'Host Alive' ? 'Host Alive' : "${if_2}_ifOperStatus";
    my $module_id_1 = get_agent_module_id( $self->{'dbh'}, $module_name_1,
        $agent_1->{'id_agente'} );
    if ( $module_id_1 <= 0 ) {
        $self->call(
            'message',
            "ERROR: Module "
              . safe_output($module_name_1)
              . " does not exist for agent $dev_1.",
            5
        );
        return;
    }
    my $module_id_2 = get_agent_module_id( $self->{'dbh'}, $module_name_2,
        $agent_2->{'id_agente'} );
    if ( $module_id_2 <= 0 ) {
        $self->call(
            'message',
            "ERROR: Module "
              . safe_output($module_name_2)
              . " does not exist for agent $dev_2.",
            5
        );
        return;
    }
    ...
    my $connection_id = get_db_value(
        $self->{'dbh'},
'SELECT id FROM tmodule_relationship WHERE (module_a = ? AND module_b = ? AND `type` = "direct") OR (module_b = ? AND module_a = ? AND `type` = "direct")',
        $module_id_1,
        $module_id_2,
        $module_id_1,
        $module_id_2
    ...
    if ( !defined($connection_id) ) {
        db_do(
            $self->{'dbh'},
'INSERT INTO tmodule_relationship (`module_a`, `module_b`, `id_rt`) VALUES(?, ?, ?)',
            $module_id_1,
            $module_id_2,
            $self->{'task_id'}
        );
...
# ]
################################################################################
sub PandoraOpen::Recon::Base::create_agents {
    my ( $self, $data ) = @_;
    ...
    foreach my $information ( @{$data} ) {
        ...
        my $modules =
          defined( $information->{'module_data'} )
          ? $information->{'module_data'}
          : [];
        my $inventory =
          defined( $information->{'inventory_data'} )
          ? $information->{'inventory_data'}
          : [];
        my $force_processing = 0;

        # Search agent
        my $current_agent = PandoraOpen::Core::locate_agent( $pa_config, $dbh,
            $agent->{'agent_name'} );
        ...
        if ( defined( $agent->{'id_parent'} ) ) {
            ...
            $parent_id = PandoraOpen::Core::locate_agent( $pa_config, $dbh,
        ...
        if ( !$current_agent ) {
            ...
            # Create agent.
            $agent_id = pandora_create_agent(
                $pa_config,                       $pa_config->{'servername'},
                $agent->{'agent_name'},           $agent->{'address'},
                $agent->{'id_group'},             $parent_id,
                $os_id,                           $agent->{'description'},
                $agent->{'interval'},             $dbh,
                $agent->{'timezone_offset'},      $agent->{'longitude'},
                $agent->{'latitude'},             $agent->{'altitude'},
                $agent->{'position_description'}, $agent->{'custom_id'},
                $agent->{'url_address'},          $agent->{'agent_mode'},
                $agent->{'agent_alias'}
            );

            $current_agent = $parent_id =
              PandoraOpen::Core::locate_agent( $pa_config, $dbh,
                $agent->{'agent_name'} );
        ...
        if ( defined( $agent->{'address'} ) && $agent->{'address'} ne '' ) {
            pandora_add_agent_address( $pa_config, $agent_id,
                $agent->{'agent_name'},
                $agent->{'address'}, $dbh );
        }
        ...
        if ( ref($modules) eq "ARRAY" ) {
            foreach my $module ( @{$modules} ) {
                ...
                my %data_translated =
                  map { $_ => [ $module->{$_} ] } keys %{$module};

                # Process modules.
                PandoraOpen::DataServer::process_module_data(
                    $pa_config,
                    \%data_translated,
                    $server_id,
                    $current_agent,
        ...
        if ( ref($inventory) eq "HASH" ) {
            PandoraOpen::Core::process_inventory_data(
                $pa_config,
                $inventory,
...
################################################################################
sub PandoraOpen::Recon::Base::delete_connections {
    my ($self) = @_;

    $self->call( 'message', "Deleting connections...", 10 );
    db_do( $self->{'dbh'}, 'DELETE FROM tmodule_relationship WHERE id_rt=?',
        $self->{'task_id'} );
}
...
# Print log messages.
################################################################################
sub PandoraOpen::Recon::Base::message {
    my ( $self, $message, $verbosity ) = @_;

    if ( $verbosity <= 1 ) {
        my $label = "[Discovery task " . $self->{'task_id'} . "]";
        if ( ref( $self->{'task_data'} ) eq 'HASH'
            && defined( $self->{'task_data'}{'name'} ) )
        {
            $label = "[Discovery task " . $self->{'task_data'}{'name'} . "]";
        }

        PandoraOpen::Core::send_console_notification( $self->{'pa_config'},
            $self->{'parent'}->getDBH(),
            $label, $message, ['admin'] );
    ...
    logger( $self->{'pa_config'},
        "[Recon task " . $self->{'task_id'} . "] $message", $verbosity );
}

################################################################################
# Connect the given hosts to its parent.
################################################################################
sub PandoraOpen::Recon::Base::set_parent {
...
# Update recon task status.
################################################################################
sub PandoraOpen::Recon::Base::update_progress {
    my ( $self, $progress ) = @_;
    ...
    eval {
        ...
        db_do(
            $self->{'dbh'},
'UPDATE trecon_task SET utimestamp = ?, status = ?, summary = ? WHERE id_rt = ?',
            time(),
            $progress,
            p_encode_json( $self->{'pa_config'}, $stats ),
            $self->{'task_id'}
        );
    };
    if ($@) {
...
sub log_execution {
    ...
    my $discovery_log_path =
      dirname( $pa_config->{'log_file'} ) . '/discovery/';
