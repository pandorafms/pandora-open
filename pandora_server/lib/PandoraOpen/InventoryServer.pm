package PandoraOpen::InventoryServer;

##########################################################################
# Pandora Open Inventory Server.
##########################################################################
...
sub new {
    ...
    return unless $config->{'inventoryserver'} == 1;
    ...
    my $self = $class->SUPER::new(
        $config, INVENTORYSERVER,
        \&PandoraOpen::InventoryServer::data_producer,
        \&PandoraOpen::InventoryServer::data_consumer, $dbh
    );
...
'SELECT tagent_module_inventory.id_agent_module_inventory, tagent_module_inventory.flag, tagent_module_inventory.timestamp
				...
				WHERE tagente.server_name = ?
					...
					AND tagente.disabled = 0
					AND (tagent_module_inventory.timestamp = \'1970-01-01 00:00:00\'
						OR UNIX_TIMESTAMP(tagent_module_inventory.timestamp) + tagent_module_inventory.'
                  . ${RDBMS_QUOTE}
                  . 'interval'
                  . ${RDBMS_QUOTE}
                  . '< UNIX_TIMESTAMP()
...
'SELECT tagent_module_inventory.id_agent_module_inventory, tagent_module_inventory.flag, tagent_module_inventory.timestamp
    ...
    foreach my $row (@rows) {
        ...
        if ( $row->{'flag'} == 1 ) {
            db_do(
                $dbh,
'UPDATE tagent_module_inventory SET flag = 0 WHERE id_agent_module_inventory = ?',
...
sub data_consumer {
    ...
    if ( defined( $module->{'custom_fields'} )
        && $module->{'custom_fields'} ne '' )
    {
        my $decoded_cfields;

        eval {
            $decoded_cfields =
              decode_json( decode_base64( $module->{'custom_fields'} ) );
        };
        if ($@) {
            logger( $pa_config, "Failed to encode received inventory data",
                10 );
        }
        ...
        if ( !defined($decoded_cfields) ) {
            logger(
                ...
                "Remote inventory module "
                  . $module->{'name'}
                  . " has failed because the custom fields can't be read",
                6
            );
        ...
        foreach my $field ( @{$decoded_cfields} ) {
            if ( $field->{'secure'} ) {
                $command .= ' "'
                  . pandora_output_password( $pa_config, $field->{'value'} )
                  . '"';
            }
            else {
                $command .= ' "' . $field->{'value'} . '"';
    ...
    # Add the default user/password arguments to the command
        ...
        my $wmi_user = safe_output(
            subst_column_macros(
                $module->{"username"},
                \%macros, $pa_config, $dbh, undef, $module
            )
        );
        my $wmi_pass = safe_output(
