package PandoraOpen::SNMPServer;
##########################################################################
# Pandora Open SNMP Console.
...
sub new {
    ...
    return unless $config->{'snmpconsole'} == 1;
    ...
    if ( !open( $SNMPTRAPD->{'fd'}, "<", $SNMPTRAPD->{'log_file'} ) ) {
        logger(
            $config,
            ' [E] Could not open the SNMP log file '
              . $SNMPTRAPD->{'log_file'} . ".",
            1
        );
        print_message(
    ...
    if ( defined( $config->{'snmp_extlog'} ) && $config->{'snmp_extlog'} ne '' )
    {
        ...
        my $tmp_fh;
        open( $tmp_fh, '>', $DATASERVER->{'log_file'} ) && close($tmp_fh)
          if ( !-e $DATASERVER->{'log_file'} );
        if ( !open( $DATASERVER->{'fd'}, "<", $DATASERVER->{'log_file'} ) ) {
            logger(
                $config,
                ' [E] Could not open the Data Server SNMP log file '
                  . $DATASERVER->{'log_file'} . ".",
                1
            );
            print_message(
    ...
    my $self = $class->SUPER::new(
        $config, SNMPCONSOLE,
        \&PandoraOpen::SNMPServer::data_producer,
        \&PandoraOpen::SNMPServer::data_consumer, $dbh
    );
...
sub run {
    ...
    print_message( $pa_config,
        " [*] Starting " . $pa_config->{'rb_product_name'} . " SNMP Console.",
        2 );
...
sub data_producer {
    ...
    my $curr_time = time();
    if ( $pa_config->{"__storm_ref__"} + $pa_config->{"snmp_storm_timeout"} <
           $curr_time
...
s/(?:(?:TCP|UDP):\s*)?\[?([^] ]+)\]?(?::-?\d+)?(?:\s*->.*)?$/$1/;
            ...
            # Silence source.
            if (   ( defined( $SILENCEDSOURCES{$source} ) )
                && ( $SILENCEDSOURCES{$source} > $curr_time ) )
            {
                next;
            }
            if (   $pa_config->{'snmp_storm_protection'} > 0
                && $AGENTS{$source}{'count'} >
                $pa_config->{'snmp_storm_protection'} )
            {
                if ( $AGENTS{$source}{'event'} == 0 ) {
                    $SILENCEDSOURCES{$source} =
                      $curr_time + $pa_config->{'snmp_storm_silence_period'};
                    my $silenced_time = (
                          $pa_config->{'snmp_storm_silence_period'} eq 0
                        ? $pa_config->{"snmp_storm_timeout"}
                        : $pa_config->{'snmp_storm_silence_period'}
                    );
                    pandora_event(
                        $pa_config,
                        "Too many traps coming from $source. Silenced for "
                          . $silenced_time
                          . " seconds.",
                        0,
                        0,
...
sub data_consumer {
    my ( $self, $task ) = @_;
    my ( $pa_config, $server_id, $dbh ) =
      ( $self->getConfig(), $self->getServerID(), $self->getDBH() );
...
sub pandora_snmptrapd {
    ...
    if ( $trap_ver eq "SNMPv1" ) {
        ( $date, $time, $source, $oid, $type, $type_desc, $value, $data ) =
          split( /\[\*\*\]/, $line, 8 );
   ...
   # Try to save as much information as possible if the trap could not be parsed
        ...
        if ( !defined($oid) ) {
            logger( $pa_config,
                "[W] snmpTrapOID not found (Illegal SNMPv1 trap?)", 5 );
            return;
        }
        ...
        if ( !defined($oid) ) {
            logger( $pa_config,
                "[W] snmpTrapOID not found (Illegal SNMPv2 trap?)", 5 );
            return;
        }
        $oid =~ s/.* = OID: //;
...
#We loop through all the custom data of the received trap, creating the $trap_data_string string to forward the trap properly
        while ( $data =~ /([\.\d]+)\s=\s([^:]+):\s([\S ]+)/g ) {
            ...
            if ( $trap_type eq "INTEGER" ) {
                ...
                $trap_value =~ s/\D//g;
                $trap_data_string =
                  $trap_data_string . "$trap_data i $trap_value ";
            }
            elsif ( $trap_type eq "UNSIGNED" ) {
                $trap_data_string =
                  $trap_data_string . "$trap_data u $trap_value ";
            }
            elsif ( $trap_type eq "COUNTER32" ) {
                $trap_data_string =
                  $trap_data_string . "$trap_data c $trap_value ";
            }
            elsif ( $trap_type eq "STRING" ) {
                $trap_data_string =
                  $trap_data_string . "$trap_data s $trap_value ";
            }
            elsif ( $trap_type eq "HEX STRING" ) {
                $trap_data_string =
                  $trap_data_string . "$trap_data x $trap_value ";
            }
            elsif ( $trap_type eq "DECIMAL STRING" ) {
                $trap_data_string =
                  $trap_data_string . "$trap_data d $trap_value ";
            }
            elsif ( $trap_type eq "NULLOBJ" ) {
                $trap_data_string =
                  $trap_data_string . "$trap_data n $trap_value ";
            }
            elsif ( $trap_type eq "OBJID" ) {
                $trap_data_string =
                  $trap_data_string . "$trap_data o $trap_value ";
            }
            elsif ( $trap_type eq "TIMETICKS" ) {
                $trap_data_string =
                  $trap_data_string . "$trap_data t $trap_value ";
            }
            elsif ( $trap_type eq "IPADDRESS" ) {
                $trap_data_string =
                  $trap_data_string . "$trap_data a $trap_value ";
            }
            elsif ( $trap_type eq "BITS" ) {
                $trap_data_string =
                  $trap_data_string . "$trap_data b $trap_value ";
            }
        }
...
"snmptrap -v $pa_config->{'snmp_forward_version'} -n \"\" -a $pa_config->{'snmp_forward_authProtocol'} -A $pa_config->{'snmp_forward_authPassword'} -x $pa_config->{'snmp_forward_privProtocol'} -X $pa_config->{'snmp_forward_privPassword'} -l $pa_config->{'snmp_forward_secLevel'} -u $pa_config->{'snmp_forward_secName'} -e $pa_config->{'snmp_forward_engineid'} $pa_config->{'snmp_forward_ip'} '' $oid $trap_data_string"
        ...
        elsif ($pa_config->{'snmp_forward_version'} eq '2'
            || $pa_config->{'snmp_forward_version'} eq '2c' )
        {
            system(
"snmptrap -v 2c -n \"\" -c $pa_config->{'snmp_forward_community'} $pa_config->{'snmp_forward_ip'} '' $oid $trap_data_string"
            );
        }
...
#Because of tne SNMP v1 protocol, we must perform additional steps for creating the trap
            ...
            system(
"snmptrap -v 1 -c $pa_config->{'snmp_forward_community'} $pa_config->{'snmp_forward_ip'} $oid \"\" $type_sending $value_sending \"\" $trap_data_string"
            );
        }
    }

    # Insert the trap into the DB
    my $trap_id = db_insert(
        $dbh,
        'id_trap',
'INSERT INTO ttrap (timestamp, source, oid, type, value, oid_custom, value_custom,  type_custom, utimestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
        $timestamp,
        $source,
        $oid,
        $type,
    ...
    pandora_evaluate_snmp_alerts(
        $pa_config, $trap_id, $source,     $oid, $type,
        $oid,       $value,   $custom_oid, $dbh
    );
...
sub matches_filter {
    ...
    my @filter_unique_functions = get_db_rows( $dbh,
'SELECT DISTINCT(unified_filters_id) FROM tsnmp_filter ORDER BY unified_filters_id'
    );

    foreach my $filter_unique_func (@filter_unique_functions) {

        # Get filters
        my @filters = get_db_rows( $dbh,
            'SELECT filter FROM tsnmp_filter WHERE unified_filters_id = '
              . $filter_unique_func->{'unified_filters_id'} );
...
sub start_snmptrapd {
    ...
    if ( $config->{'snmp_trapd'} eq 'manual' ) {
        my $noSNMPTrap =
          "No SNMP trap daemon configured. Start snmptrapd manually.";
        logger( $config, $noSNMPTrap, 1 );
        print_message( $config, " [*] $noSNMPTrap", 1 );

        if ( !-f $config->{'snmp_logfile'} ) {
            my $noLogFile =
              "SNMP log file " . $config->{'snmp_logfile'} . " not found.";
    ...
    if ( -e $pid_file && open( my $pidfile_fh, "<", $pid_file ) ) {
        my $pid = <$pidfile_fh> + 0;
        close $pidfile_fh;
        ...
        if ( $snmptrapd_running = kill( 0, $pid ) ) {
            my $alreadyRunning =
"snmptrapd (pid $pid) is already running, attempting to kill it...";
    ...
    my $snmp_ignore_authfailure =
      ( $config->{'snmp_ignore_authfailure'} eq '1' ? ' -a' : '' );
    ...
    my $snmptrapd_args =
        ' -t -On -n'
      . $snmp_ignore_authfailure . ' -Lf '
      . $config->{'snmp_logfile'} . ' -p '
      . $pid_file;
    $snmptrapd_args .=
        ' --format1=SNMPv1[**]%4y-%02.2m-%l[**]%02.2h:%02.2j:%02.2k[**]'
      . $address_format
      . '[**]%N[**]%w[**]%W[**]%q[**]%v\\\n';
    $snmptrapd_args .=
' --format2=SNMPv2[**]%4y-%02.2m-%l[**]%02.2h:%02.2j:%02.2k[**]%b[**]%v\\\n';
    ...
    if (
        system( $config->{'snmp_trapd'} . $snmptrapd_args . " >$DEVNULL 2>&1" )
        != 0 )
    {
...
sub init_log_file {
    ...
    ( $fs->{'idx_file'}, $fs->{'last_line'}, $fs->{'last_size'} ) =
      ( $fs->{'log_file'} . '.index', 0, 0 );
    if ( -e $fs->{'idx_file'} ) {
...
sub reset_if_truncated {
    ...
    # New SNMP log file found
    if ( $log_size < $fs->{'last_size'} ) {
        logger( $pa_config, 'File ' . $fs->{'log_file'} . ' was truncated.',
            10 );
        unlink( $fs->{'idx_file'} );
        ( $fs->{'last_line'}, $fs->{'last_size'} ) = ( 0, 0 );
        seek( $fs->{'fd'}, 0, 0 );
