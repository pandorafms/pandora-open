package PandoraOpen::WMIServer;
##########################################################################
# Pandora Open WMI Server.
...
sub new {
    ...
    return unless $config->{'wmiserver'} == 1;
    ...
    if ( system( $config->{'wmi_client'} . " >$DEVNULL 2>&1" ) >> 8 == 127 ) {
        logger(
            ...
            ' [E] '
              ...
              . " WMI Server needs a DCOM/WMI client.",
            1
        );
        print_message(
            $config,
    ...
    my $self = $class->SUPER::new(
        $config, WMISERVER,
        \&PandoraOpen::WMIServer::data_producer,
        \&PandoraOpen::WMIServer::data_consumer, $dbh
    );
...
sub run {
    ...
    print_message( $pa_config,
        " [*] Starting " . $pa_config->{'rb_product_name'} . " WMI Server.",
        1 );
...
'SELECT DISTINCT(tagente_modulo.id_agente_modulo), tagente_modulo.flag, tagente_estado.current_interval + tagente_estado.last_execution_try AS time_left, last_execution_try
			...
			ORDER BY tagente_modulo.flag DESC, time_left ASC, last_execution_try ASC',
            safe_input( $pa_config->{'servername'} ), WMISERVER
        );
    }
...
sub data_consumer {
    ...
    if ( defined( $module->{'plugin_pass'} ) && $module->{'plugin_pass'} ne "" )
    {
        ...
        $wmi_command =
          $pa_config->{'wmi_client'} . ' -U "' . $user . '"%"' . $pass . '"';
    }
    elsif ( defined( $module->{'plugin_user'} )
        && $module->{'plugin_user'} ne "" )
    {
        my $user = safe_output(
            subst_column_macros(
                $module->{'plugin_user'},
    ...
    if ( $module->{'ip_target'} eq '_address_' ) {
        $module->{'ip_target'} = get_db_value(
            $dbh,
            "SELECT direccion FROM tagente WHERE id_agente=?",
            $module->{'id_agente'}
        );
    }
    ...
    logger( $pa_config, "Executing AM # $module_id WMI command '$wmi_command'",
        9 );
    ...
    if ( defined( $module->{'tcp_port'} ) ) {
        ...
        if ( !defined($selected_col) ) {
            logger(
                $pa_config,
                'Warning, WMI module '
                  . safe_output( $module->{'name'} )
                  . ' column missconfigured, using first available.',
                10
            );
        ...
        $module_data = $row[$col_number]
          if ( defined($col_number) && defined( $row[$col_number] ) );
