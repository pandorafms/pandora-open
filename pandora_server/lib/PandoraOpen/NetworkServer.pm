package PandoraOpen::NetworkServer;
##########################################################################
# Pandora Open Network Server.
...
sub new {
    ...
    return unless $config->{'networkserver'} == 1;
    ...
    my $self = $class->SUPER::new(
        ...
        \&PandoraOpen::NetworkServer::data_producer,
        \&PandoraOpen::NetworkServer::data_consumer, $dbh
    );
...
sub run {
    ...
    print_message(
        $pa_config,
        " [*] Starting " . $pa_config->{'rb_product_name'} . " Network Server.",
        1
    );
...
'SELECT tagente_modulo.id_agente_modulo, tagente_modulo.flag, tagente_estado.current_interval + tagente_estado.last_execution_try AS time_left, last_execution_try
        ...
        @rows = get_db_rows(
            $dbh,
'SELECT tagente_modulo.id_agente_modulo, tagente_modulo.flag, tagente_estado.last_execution_try, tagente_estado.current_interval + tagente_estado.last_execution_try AS time_left, last_execution_try
		FROM tagente, tagente_modulo, tagente_estado
		...
		AND (tagente_modulo.flag = 1 OR ((tagente_estado.last_execution_try + tagente_estado.current_interval) < UNIX_TIMESTAMP()))
		ORDER BY tagente_modulo.flag DESC, time_left ASC, tagente_estado.last_execution_try ASC',
            safe_input( $pa_config->{'servername'} ), NETWORKSERVER
        );
    }
    ...
    foreach my $row (@rows) {
        ...
        if ( $row->{'flag'} == 1 ) {
            db_do(
                $dbh,
                'UPDATE tagente_modulo SET flag = 0 WHERE id_agente_modulo = ?',
...
sub data_consumer {
    ...
    exec_network_module( $self->getConfig(), $task, $self->getServerID(),
        $self->getDBH() );
}
    ...
    for ( $counter = 0 ; $counter < $retries ; $counter++ ) {
        ...
        if ( defined($handle) ) {
          ...
          next_pair:
            ...
            {    # its Expected to sending data ?
                 # Send data
                logger(
                    $pa_config,
"[INFO] TCP query on port $tcp_port with target $ip_target by module with id $module_id.",
                    10
                );
                $handle->autoflush(1);
                $tcp_send =~ s/\^M/\r\n/g;
              ...
              )
            {
                ...
                for ( $tam = 0 ; $tam < $timeout ; $tam++ ) {
                    if ( $select->can_read(1) ) {
                        ...
                        last
                          ...
                          ;    # No more data or something went wrong
                        $temp2 = $temp2 . $temp;
                    }
                }
                ...
                elsif ( $id_tipo_modulo == 10 )
                {    # TCP String (no int conversion)!
                    $$module_data   = $temp2;
                    $$module_result = 0;
                }
            ...
            { # No expected data to receive, if connected and tcp_proc type successful
...
sub pandora_snmp_get_command {
    ...
    my (
        $snmpget_cmd,  $snmp_version,         $snmp_retries,
        $snmp_timeout, $snmp_community,       $snmp_target,
        $snmp_oid,     $snmp3_security_level, $snmp3_extra,
        $snmp_port,    $pa_config
    ) = @_;
    ...
    # Pandora Open's console MIB directory
...
# the call is the same than in linux
    if (   ( $OSNAME eq "MSWin32" )
        || ( $OSNAME eq "MSWin32-x64" )
        || ( $OSNAME eq "cygwin" ) )
    {
        if ( $snmp_version ne "3" ) {
            $output =
`$snmpget_cmd -v $snmp_version -r $snmp_retries -t $snmp_timeout -OUevqt -c $snmp_community $snmp_target $snmp_oid 2>$DEVNULL`;
        }
        else {
            $output =
`$snmpget_cmd -v $snmp_version -r $snmp_retries -t $snmp_timeout -OUevqt -l $snmp3_security_level $snmp3_extra $snmp_target $snmp_oid 2>$DEVNULL`;
        }
    }
    ...
    # by default LINUX/FreeBSD/Solaris calls
        ...
        if ( $snmp_version ne "3" ) {
            $output =
`$snmpget_cmd -M+"$mib_dir" -v $snmp_version -r $snmp_retries -t $snmp_timeout -OUevqt -c '$snmp_community' $snmp_target $snmp_oid 2>$DEVNULL`;
        }
        else {
            $output =
`$snmpget_cmd -M+"$mib_dir" -v $snmp_version -r $snmp_retries -t $snmp_timeout -OUevqt -l $snmp3_security_level $snmp3_extra $snmp_target $snmp_oid 2>$DEVNULL`;
        }
    }
...
# Check if output was defined because if the snmpget fails this variable will be undef
    if ( defined($output) ) {
        ...
        logger(
            $pa_config,
"[ERROR] Undefined value returned SNMP query. Is the server out of memory?",
            3
        );
        logger(
            $pa_config,
"[ERROR] Snmp Community: $snmp_community SNMP Target: $snmp_target OID: $snmp_oid",
            3
        );
        return "";
    }
...
sub pandora_query_snmp {
    ...
    my $snmp3_privacy_pass   = safe_output(
        pandora_output_password(
            $pa_config,
            subst_column_macros(
                $module->{"custom_string_2"},
                \%macros, $pa_config, $dbh, undef, $module
            )
        )
    );
    my $snmp3_security_level =
      $module->{"custom_string_3"};    # noAuthNoPriv|authNoPriv|authPriv
    my $snmp3_auth_user = safe_output(
        subst_column_macros(
            $module->{"plugin_user"},
            \%macros, $pa_config, $dbh, undef, $module
        )
    );
    my $snmp3_auth_pass = safe_output(
        pandora_output_password(
            $pa_config,
            subst_column_macros(
                $module->{"plugin_pass"},
    ...
    if ( $snmp_oid =~ m/[a-zA-Z]/ ) {
        ...
        if ( !defined($snmp_oid) || $snmp_oid eq '' ) {
            db_do(
                $dbh,
'UPDATE tagente_modulo SET disabled = 1 WHERE id_agente_modulo = ?',
        ...
        db_do(
            $dbh,
            'UPDATE tagente_modulo SET snmp_oid = ? WHERE id_agente_modulo = ?',
            $snmp_oid,
            $module->{"id_agente_modulo"}
        );
    }
    ...
    my $snmp_timeout =
      ...
      ? $module->{"max_timeout"}
      : $pa_config->{"snmp_timeout"};
    my $snmp_retries =
        $module->{"max_retries"} != 0
      ? $module->{"max_retries"}
      : $pa_config->{'snmp_checks'};
    ...
    if ( $snmp_version ne '3' ) {
        ...
        $output = pandora_snmp_get_command(
            $snmpget_cmd,    $snmp_version, $snmp_retries, $snmp_timeout,
            $snmp_community, $snmp_target,  $snmp_oid,     "",
            "",              $snmp_port,    $pa_config
        );
        ...
        if ( $snmp3_security_level eq "authNoPriv" ) {
            $snmp3_extra =
" -a $snmp3_auth_method -u '$snmp3_auth_user' -A '$snmp3_auth_pass' ";
        }

        # SNMP v3 privacy AND authentication
        if ( $snmp3_security_level eq "authPriv" ) {
            $snmp3_extra =
" -a $snmp3_auth_method -u '$snmp3_auth_user' -A '$snmp3_auth_pass' -x $snmp3_privacy_method -X '$snmp3_privacy_pass' ";
        }
...
sub exec_network_module {
    ...
    $ip_target = safe_output(
        subst_column_macros(
            $ip_target, \%macros, $pa_config, $dbh, $agent_row, $module
        )
    );
    ...
    if ( ( defined($ip_target) ) && ($ip_target) ) {
        ...
        if ( $id_tipo_modulo == 6 ) {    # ICMP (Connectivity only: Boolean)
            $module_data =
              pandora_ping( $pa_config, $ip_target, $timeout, $retries );
            $module_result = 0;          # Successful
        }
        elsif ( $id_tipo_modulo == 7 ) {    # ICMP (data for latency in ms)
            $module_data =
              pandora_ping_latency( $pa_config, $ip_target, $timeout,
                $retries );
            ...
            || ( $id_tipo_modulo == 17 ) )
        {
            ...
            ( $module_data, $module_result ) =
              pandora_query_snmp( $pa_config, $module, $ip_target, $dbh );
            ...
            if ( $module_result == 0 ) {    # A correct SNMP Query
                ...
                if ( $id_tipo_modulo == 18 ) {    #snmp_data_proc
                    ...
                    if ( $module_data ne '1' )
                    {    # up state is 1, down state in SNMP is 2 ....
                        $module_data = 0;
                    }
                }
        ...
        {    # TCP Module
            ...
            {    # Port check
                pandora_query_tcp(
                    $pa_config,      $tcp_port,       $ip_target,
                    \$module_result, \$module_data,   $tcp_send,
                    $tcp_rcv,        $id_tipo_modulo, $timeout,
                    $retries,        $id_agente_modulo
                );
            }
            else {
        ...
        if ( $module_result == 0 ) {
            ...
            pandora_process_module(
                ...
                '',         $timestamp, $utimestamp, $server_id,
                $dbh
            );
            ...
            pandora_update_agent( $pa_config, $timestamp, $id_agente, undef,
                undef, -1, $dbh );
