...
"You can download latest versions and documentation at official web page.\n\n";
    ...
    if ( $#ARGV == -1 ) {
        print "I need at least one parameter: Complete path to "
          . pandora_get_initial_product_name()
    ...
    $pa_config->{"quiet"}     = 0;     # Quiet 0 by default
    ...
    if ( !defined( $pa_config->{"pandora_path"} )
        || $pa_config->{"pandora_path"} eq "" )
    {
        print "[ERROR] I need at least one parameter: Complete path to "
          . pandora_get_initial_product_name()
          . " configuration file. \n";
        print
          "For example: ./pandora_server /etc/pandora/pandora_server.conf \n\n";
        exit;
    }
}
...
sub pandora_get_sharedconfig {
    ...
    $pa_config->{"realtimestats"} =
      pandora_get_tconfig_token( $dbh, 'realtimestats', 0 );

    # Stats_interval option
    $pa_config->{"stats_interval"} =
      pandora_get_tconfig_token( $dbh, 'stats_interval', 300 );

    # Sflow configuration options
    $pa_config->{"activate_sflow"} =
      pandora_get_tconfig_token( $dbh, 'activate_sflow', 0 );
    $pa_config->{"sflow_path"} = pandora_get_tconfig_token( $dbh, 'sflow_path',
        '/var/spool/pandora/data_in/sflow' );
    $pa_config->{"sflow_interval"} =
      pandora_get_tconfig_token( $dbh, 'sflow_interval', 300 );

    # Log module configuration
    $pa_config->{"log_dir"} = pandora_get_tconfig_token( $dbh, 'log_dir',
        '/var/spool/pandora/data_in/log' );
    $pa_config->{"log_interval"} =
      pandora_get_tconfig_token( $dbh, 'log_interval', 3600 );

    # Pandora Open Console's attachment directory
    $pa_config->{"attachment_dir"} =
      pandora_get_tconfig_token( $dbh, 'attachment_store',
        '/var/www/pandora_console/attachment' );

    #Public url
    $pa_config->{'public_url'} = pandora_get_tconfig_token( $dbh, 'public_url',
...
#$pa_config->{"node_metaconsole"} = pandora_get_tconfig_token ($dbh, 'node_metaconsole', 0);
    ...
    $pa_config->{"provisioning_mode"} =
      pandora_get_tconfig_token( $dbh, 'provisioning_mode', '' );

    $pa_config->{"event_storm_protection"} =
      pandora_get_tconfig_token( $dbh, 'event_storm_protection', 0 );

    $pa_config->{"use_custom_encoding"} =
      pandora_get_tconfig_token( $dbh, 'use_custom_encoding', 0 );

    # PandoraOpen product name
    $pa_config->{'rb_product_name'} = 'Pandora Open';

    # Mail transport agent configuration. Local configuration takes precedence.
    if ( $pa_config->{"mta_local"} eq 0 ) {
        $pa_config->{"mta_address"} =
          pandora_get_tconfig_token( $dbh, 'email_smtpServer', '' );
        $pa_config->{"mta_from"} = '"'
          . pandora_get_tconfig_token( $dbh, 'email_from_name', 'Pandora Open' )
          . '" <'
          . pandora_get_tconfig_token( $dbh, 'email_from_dir',
            'pandora@pandorafms.org' )
          . '>';
        $pa_config->{"mta_pass"} =
          pandora_get_tconfig_token( $dbh, 'email_password', '' );
        $pa_config->{"mta_port"} =
          pandora_get_tconfig_token( $dbh, 'email_smtpPort', '' );
        $pa_config->{"mta_user"} =
          pandora_get_tconfig_token( $dbh, 'email_username', '' );
        $pa_config->{"mta_encryption"} =
          pandora_get_tconfig_token( $dbh, 'email_encryption', '' );
   ...
   # 451 4.5.0 SMTP protocol violation, see RFC 2821
    ...
    # Server identifier
    $pa_config->{'server_unique_identifier'} =
      pandora_get_tconfig_token( $dbh, 'server_unique_identifier', '' );

    # Vulnerability scans
    $pa_config->{'agent_vulnerabilities'} =
      pandora_get_tconfig_token( $dbh, 'agent_vulnerabilities', 0 );
}
...
sub pandora_load_config {
    ...
    $pa_config->{"basepath"} =
      $pa_config->{'pandora_path'};    # Compatibility with Pandora 1.1
    $pa_config->{"incomingdir"} = "/var/spool/pandora/data_in";
    $pa_config->{"user"} =
      "pandora";   # environment settings default user owner for files generated
    $pa_config->{"group"} =
      "apache";   # environment settings default group owner for files generated
    $pa_config->{"umask"} =
      "0007";     # environment settings umask applied over chmod (A & (not B))
    ...
    $pa_config->{"transactional_pool"} =
      $pa_config->{"incomingdir"} . "/" . "trans";  # Default, introduced on 6.1
    ...
    $pa_config->{"elastic_query_size"}             = 10;
    ...
    $pa_config->{"mail_in_separate"} =
      1;    # 1: eMail deliver alert mail in separate mails.
    ...
    # Discovery SAP utils
    $pa_config->{"sap_utils"} =
      "/usr/share/pandora_server/util/recon_scripts/SAP";
...
# PsExec allows to exec commands on remote windows systems from windows servers (optional)
    $pa_config->{"psexec"} = 'C:\PandoraOpen\Pandora_Server\bin\PsExec.exe';

# plink allows to exec commands on remote linux systems from windows servers (optional)
    $pa_config->{"plink"} = 'C:\PandoraOpen\Pandora_Server\bin\plink.exe';
    ...
    # If 1 => uses timestamp from received XML
    ...
    $pa_config->{'self_monitoring_interval'} = 300;
    ...
    $pa_config->{"text_going_down_normal"} =
      "Module '_module_' is going to NORMAL (_data_)";
    $pa_config->{"text_going_up_critical"} =
      "Module '_module_' is going to CRITICAL (_data_)";
    $pa_config->{"text_going_up_warning"} =
      "Module '_module_' is going to WARNING (_data_)";
    $pa_config->{"text_going_down_warning"} =
      "Module '_module_' is going to WARNING (_data_)";
    $pa_config->{"text_going_unknown"} =
      "Module '_module_' is going to UNKNOWN";
    ...
    $pa_config->{'ncm_ssh_utility'} =
      '/usr/share/pandora_server/util/ncm_ssh_extension';

    $pa_config->{'agent_deployer_utility'} =
      '/usr/share/pandora_server/util/pandora_agent_deployer';
    ...
    if ( $pa_config->{"quiet"} != 0 ) {
        if ( $> == 0 ) {
            printf
" [W] Not all Pandora Open components need to be executed as root\n";
...
"	Standard configuration file is at /etc/pandora/pandora_server.conf \n";
    ...
    my $cfg_fh;
    if ( !open( $cfg_fh, "<:encoding(UTF-8)", $archivo_cfg ) ) {
...
m/([\w\-\_\.]+)\s([0-9\w\-\_\.\/\?\&\=\)\(\_\-\!\*\@\#\%\$\~\"\']+)/
    ...
    for ( $ax = 0 ; $ax <= $ltotal ; $ax++ ) {
        ...
        if ( $parametro =~ m/^transactional_pool\s(.*)/i ) {
            ...
            $pa_config->{'snmp_forward_community'} =
              safe_input( clean_blank($1) );
        }
            ...
            $pa_config->{'snmp_forward_version'} =
              safe_input( clean_blank($1) );
        }
        elsif ( $parametro =~ m/^snmp_forward_ip\s(.*)/i ) {
            $pa_config->{'snmp_forward_ip'} = safe_input( clean_blank($1) );
            if (
                $pa_config->{'snmp_forward_trap'} == 1
                && (   $pa_config->{'snmp_forward_ip'} eq '127.0.0.1'
                    || $pa_config->{'snmp_forward_ip'} eq 'localhost' )
              )
            {
                printf
"\n [ERROR] Cannot set snmp_forward_ip to localhost or 127.0.0.1 \n";
            ...
            $pa_config->{'claim_back_snmp_modules'} =
              safe_input( clean_blank($1) );
        }
        ...
        elsif ( $parametro =~ m/^unknown_interval\s+([0-9]*)/i ) {
            ...
            || $parametro =~ m/^warmup_alert_interval\s+([0-9]*)/i )
        {
            ...
            $pa_config->{'warmup_unknown_on'}       = 0
              if ( $pa_config->{'warmup_unknown_interval'} == 0 )
              ;    # On by default.
        }
    ...
    $pa_config->{'ha_dbuser'} = $pa_config->{'dbuser'}
      unless defined( $pa_config->{'ha_dbuser'} );
    $pa_config->{'ha_dbpass'} = $pa_config->{'dbpass'}
      unless defined( $pa_config->{'ha_dbpass'} );

    # repl_dbuser and repl_dbpass default to dbuser and dbpass respectively.
    $pa_config->{'repl_dbuser'} = $pa_config->{'dbuser'}
      unless defined( $pa_config->{'repl_dbuser'} );
    $pa_config->{'repl_dbpass'} = $pa_config->{'dbpass'}
      unless defined( $pa_config->{'repl_dbpass'} );
    ...
    if ( ( $pa_config->{"verbosity"} > 4 ) && ( $pa_config->{"quiet"} == 0 ) ) {
        ...
        print " [*] Server errorlogfile at "
          . $pa_config->{"errorlog_file"} . "\n";
        print " [*] Server incoming directory at "
          . $pa_config->{"incomingdir"} . "\n";
        print " [*] Server keepalive " . $pa_config->{"keepalive"} . "\n";
        print " [*] Server threshold "
          . $pa_config->{"server_threshold"} . "\n";
    }

    # Check for valid token token values
    if (   ( $pa_config->{"dbuser"} eq "" )
        || ( $pa_config->{"basepath"} eq "" )
        || ( $pa_config->{"incomingdir"} eq "" )
        || ( $pa_config->{"log_file"} eq "" )
        || ( $pa_config->{"dbhost"} eq "" )
        || ( $pa_config->{"pandora_master"} eq "" )
        || ( $pa_config->{"dbpass"} eq "" ) )
    {
        print
" [ERROR] Bad Config values. Be sure that $archivo_cfg is a valid setup file. \n\n";
    ...
    if ( ( $pa_config->{"quiet"} == 0 ) && ( $pa_config->{"verbosity"} > 4 ) ) {
        ...
        if ( $pa_config->{"pandora_master"} != 0 ) {
            print " [*] This server is running with MASTER priority "
              . $pa_config->{"pandora_master"} . "\n";
        }
    }

    logger( $pa_config, "Launching $pa_config->{'version'}", 1 );
    my $config_options =
        "Logfile at "
      . $pa_config->{"log_file"}
      . ", Basepath is "
      . $pa_config->{"basepath"}
      . ", Checksum is "
      . $pa_config->{"pandora_check"}
      . ", Master is "
      . $pa_config->{"pandora_master"}
      . ", SNMP Console is "
      . $pa_config->{"snmpconsole"}
      . ", Server Threshold at "
      . $pa_config->{"server_threshold"}
      . " sec, verbosity at "
      . $pa_config->{"verbosity"}
      . ", Alert Threshold at $pa_config->{'alert_threshold'}, ServerName is '"
...
sub pandora_start_log {
    ...
    my $file_mode = ( stat( $pa_config->{'errorlog_file'} ) )[2] & 777;
    my $min_mode  = 664;
    ...
    print STDERR strftime( "%Y-%m-%d %H:%M:%S", localtime() ) . ' - '
      ...
      . " Starting "
      . pandora_get_initial_product_name()
      . " Server. Error logging activated.\n";
}
...
sub pandora_set_tconfig_token {
    ...
    if ( defined($token_value) && $token_value ne '' ) {
        db_update( $dbh, 'UPDATE `tconfig` SET `value`=? WHERE `token`= ?',
            safe_input($value), $token );
    }
    else {
        db_insert( $dbh, 'id_config',
            'INSERT INTO `tconfig`(`token`, `value`) VALUES (?, ?)',
            $token, safe_input($value) );
    }
...
sub pandora_get_initial_product_name {
    ...
    return 'Pandora Open'
      unless ( defined($product_name) && $product_name ne '' );
    return $product_name;
}
...
sub pandora_get_initial_copyright_notice {
    ...
    return 'Pandora Open' unless ( defined($name) && $name ne '' );
    return $name;
}
