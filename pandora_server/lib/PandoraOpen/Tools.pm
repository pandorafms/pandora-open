...
if ( $OS eq 'linux' ) {
    $OS_VERSION =
      `cat /etc/*ease|grep PRETTY| cut -f 2 -d= | tr -d '"' 2>/dev/null`;
}
elsif ( $OS eq 'aix' ) {
...
# Threads started by the Pandora Open Server.
my @ServerThreads;
...
sub pandora_daemonize {
    ...
    if ( $pa_config->{'PID'} ne "" ) {
        ...
        umask 0022;
        open( $pidfile_fh, ">", $pa_config->{'PID'} )
          or die "[FATAL] Cannot open PIDfile at " . $pa_config->{'PID'};
        print $pidfile_fh "$$";
        close($pidfile_fh);
    }
    umask 0007;
...
sub credential_store_get_key {
    ...
    my $key = PandoraOpen::DB::get_db_single_row( $dbh, $sql, $identifier );

    if ( defined($key) ) {
        return {
            'product'  => $key->{'product'},
            'username' => PandoraOpen::Core::pandora_output_password(
                $pa_config, $key->{'username'}
            ),
            'password' => PandoraOpen::Core::pandora_output_password(
                $pa_config, $key->{'password'}
            ),
            'extra_1' => $key->{'extra_1'},
            'extra_2' => $key->{'extra_2'},
        };
...
sub pandora_sendmail {
    ...
    # Set the timeout.
    $PandoraOpen::Sendmail::mailcfg{'timeout'} = $pa_config->{"tcp_timeout"};

    # Enable debugging.
    $PandoraOpen::Sendmail::mailcfg{'debug'} = $pa_config->{"verbosity"};
    ...
    if ( $pa_config->{"mta_user"} ne "" ) {
        $mail{auth} = {
            ...
            password => PandoraOpen::Core::pandora_output_password(
                $pa_config, safe_output( $pa_config->{"mta_pass"} )
            ),
            method   => $pa_config->{"mta_auth"},
            required => 1
        };
    }
    ...
    eval {
        if ( !sendmail(%mail) ) {
            logger( $pa_config,
                "[ERROR] Sending email to $to_address with subject $subject",
                1 );
...
sub logger {
    ...
    if ( $file eq 'syslog' ) {
        ...
        my $file_fh;
        open( $file_fh, ">>", $file )
          or die "[FATAL] Could not open logfile '$file'";

        # Get an exclusive lock on the file (LOCK_EX)
        flock( $file_fh, 2 );
        print $file_fh strftime( "%Y-%m-%d %H:%M:%S", localtime() )
          . $parent_caller
          . (
            defined( $pa_config->{'servername'} )
...
sub pandora_rotate_logfile {
    ...
    if (   $file ne 'syslog'
        && -e $file
        && ( stat($file) )[7] > $pa_config->{'max_log_size'} )
    {
...
sub sqlWrap {
    ...
    if ( defined $toBeWrapped ) {
        $toBeWrapped =~ s/\'/\\\'/g;
        $toBeWrapped =~ s/\"/\\\'/g
          ;    # " This is for highlighters that don't understand escaped quotes
        return "'" . $toBeWrapped . "'";
    }
}
...
sub get_tag_value {
    ...
    return $def_value
      unless defined( $hash_ref->{$tag} )
      and ref( $hash_ref->{$tag} );
...
# Append bit /* bit, not byte */ length of unpadded message as 64-bit little-endian integer to message
    ...
    for ( my $i = 0 ; $i < length($msg) ; $i += 512 ) {
        ...
        for ( my $y = 0 ; $y < 64 ; $y++ ) {
            ...
            $b =
              ( $b +
                  leftrotate( ( $a + $f + $K[$y] + $w[$g] ) % MOD232, $R[$y] ) )
              % MOD232;
            $a = $temp;
        }
...
sub dateTimeToTimestamp {
    ...
    my ( $year, $mon, $day, $GMT, $hour, $min, $sec ) =
      ( $1, $2, $3, $4, $5, $6, $7 );
...
sub disk_free {
    ...
    my $command =
      "df -k -P " . $target . " | tail -1 | awk '{ print \$4/1024}'";
    my $output = `$command`;
    return $output;
}
...
sub load_average {
    ...
    if ( $OSNAME eq "freebsd" ) {
        ...
        $load_average =
`powershell "(Get-WmiObject win32_processor | Measure-Object -property LoadPercentage -Average).average"`;
        chop($load_average);
    }
...
sub free_mem {
    ...
    if ( $OSNAME eq "freebsd" ) {
        my ( $pages_free, $page_size ) =
          `/sbin/sysctl -n vm.stats.vm.v_page_size vm.stats.vm.v_free_count`;
...
sub cpu_load {
    ...
    if ( $OSNAME eq "MSWin32" ) {
        $cpu_load =
`wmic cpu get loadpercentage|find /I /V "Loadpercentage" | findstr /r "[0-9]" `;
    }
...
sub count_files_ext {
    ...
    if ( $OSNAME eq "MSWin32" ) {
        ...
        $count =
`wmic datafile where "drive=\'$drive\' and path=\'$folder\' and extension=\'$ext\'" get /value | find /c "="`;
        if ( $count =~ m/=(.*)$/gm ) {
            $count = $1;
        }
        $count = undef;
...
sub pandora_ping {
    ...
    my $packets =
      defined( $pa_config->{'icmp_packets'} )
      ? $pa_config->{'icmp_packets'}
      : 1;
...
sub pandora_ping_latency {
  ...
  # but always in english
    ...
    # by default LINUX calls
        ...
        # Ping the host
        my @output =
          `$ping_command -q -W $timeout -n -c $retries $host 2>$DEVNULL`;
...
sub ping {
    ...
    my ( $timeout, $retries, $packets ) =
      ( $pa_config->{'networktimeout'}, $pa_config->{'icmp_checks'}, 1 );
...
sub cron_next_execution {
    ...
    if ( $wday_down ne "*"
        && ( $wday_down > 6 || ( defined($wday_up) && $wday_up > 6 ) ) )
    {
        $wday = "*";
    }
...
sub cron_next_execution_date {
    ...
    return ($nex_time)
      if cron_is_in_cron( \@cron_array, \@curr_time_array ) == 1;
...
sub resolve_hostname {
    ...
    my $resolved_hostname = inet_aton($hostname);
...
sub uri_encode {
    ...
    # Encode a literal '%'
    $data =~
      s{(\%)(.*)}{uri_encode_literal_percent($1, $2, $enc_map, $dec_map)}gex;
...
sub uri_encode_literal_percent {
    ...
    $return_char ||=
      join( '', uri_encode_get_encoded_char( $char, $enc_map ), $post );
    return $return_char;
...
sub generate_agent_name_hash {
    ...
    return sha256(
        join(
            '|',
            (
                $agent_alias, $server_ip, time(), sprintf( "%04d", rand(10000) )
            )
        )
    );
}
...
# Extend the first 16 words into the remaining 48 words w[16..63] of the message schedule array:
        for ( my $i = 16 ; $i < 64 ; $i++ ) {
            my $s0 = rightrotate( $w[ $i - 15 ], 7 )
              ^ rightrotate( $w[ $i - 15 ], 18 ) ^ ( $w[ $i - 15 ] >> 3 );
            my $s1 =
              rightrotate( $w[ $i - 2 ], 17 ) ^ rightrotate( $w[ $i - 2 ], 19 )
              ^ ( $w[ $i - 2 ] >> 10 );
        ...
        for ( my $i = 0 ; $i < 64 ; $i++ ) {
            my $S1 = rightrotate( $e, 6 ) ^ rightrotate( $e, 11 )
              ^ rightrotate( $e, 25 );
            my $ch    = ( $e & $f ) ^ ( ( 0xFFFFFFFF & ( ~$e ) ) & $g );
            my $temp1 = ( $h + $S1 + $ch + $K2[$i] + $w[$i] ) % POW232;
            my $S0    = rightrotate( $a, 2 ) ^ rightrotate( $a, 13 )
              ^ rightrotate( $a, 22 );
            my $maj   = ( $a & $b ) ^ ( $a & $c ) ^ ( $b & $c );
...
sub check_cron_element {
    ...
    foreach my $elem (@elems) {
        ...
        if (   check_cron_interval( $elem, $elem_curr_time )
            || check_cron_skips( $elem, $elem_curr_time )
            || check_cron_value( $elem, $elem_curr_time ) )
        {
            $elem_res = 1;
...
sub apply_timezone_offset {
    ...
    # Nothing to be done.
    return $timestamp
      if ( !defined($timezone_offset) || $timezone_offset == 0 );
    ...
    $timestamp = strftime( "%Y-%m-%d %H:%M:%S",
        localtime( $utimestamp + ( $timezone_offset * 3600 ) ) );
