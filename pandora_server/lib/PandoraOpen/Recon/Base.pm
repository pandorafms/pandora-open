#!/usr/bin/perl
# (c) Pandora Open 2014-2023 <info@pandorafms.com>
# Module for network topology discovery.

package PandoraOpen::Recon::Base;
use strict;
use warnings;
...
use PandoraOpen::Recon::NmapParser;
use PandoraOpen::Recon::Util;
...
sub get_os_version {
  ...
  my $os_version = $self->snmp_get_value($device, "$PandoraOpen::Recon::Base::SYSDESCR");
...
sub find_ifaces {
  ...
  my @output = $self->snmp_get_value_array($device, $PandoraOpen::Recon::Base::IFINDEX);
  foreach my $if_index (@output) {
    ...
    my $if_name = $self->snmp_get_value($device, "$PandoraOpen::Recon::Base::IFNAME.$if_index");
...
sub get_pen {
  ...
  return unless ref($self->{'pen'}) eq 'HASH';
...
sub get_iface {
  ...
  return unless defined($self->{'ifaces'}->{$mac});
...
sub snmp3_credentials {
  ...
  return if !defined($cred);
  return if ref($cred) ne 'HASH';
...
sub traceroute_connectivity {
  ...
  my $np = PandoraOpen::Recon::NmapParser->new();
...
sub wmi_get_value {
  ...
  return unless defined($output[2]);

  my $line = $output[2];
  chomp($line);
  my @columns = split(/\|/, $line);
  return unless defined($columns[$column]);
