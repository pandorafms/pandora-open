package PandoraOpen::DataServer;
##########################################################################
# Pandora Open Data Server.
...
sub new {
    ...
    return unless $config->{'dataserver'} == 1;
    ...
    if ( $config->{'dataserver_smart_queue'} == 0 ) {
        $self = $class->SUPER::new(
            $config, DATASERVER,
            \&PandoraOpen::DataServer::data_producer,
            \&PandoraOpen::DataServer::data_consumer, $dbh
        );
    }
    else {
        logger( $config,
            "Smart queue enabled for the Pandora Open DataServer.", 3 );
        $self = $class->SUPER::new(
            $config, DATASERVER,
            \&PandoraOpen::DataServer::data_producer_smart_queue,
            \&PandoraOpen::DataServer::data_consumer, $dbh
        );
    }
    ...
    if ( $config->{'enc_dir'} ne '' ) {
        ...
        if ( $XML::Simple::PREFERRED_PARSER eq 'XML::SAX::ExpatXS' ) {
            no warnings 'once';
            push( @XML::SAX::ExpatXS::Encoding::Encoding_Path,
                $config->{'enc_dir'} );
        }
    }
    ...
    if ( $config->{'autocreate_group_name'} ne '' ) {
        if ( get_group_id( $dbh, $config->{'autocreate_group_name'} ) == -1 ) {
            my $msg =
                "Group '"
              . $config->{'autocreate_group_name'}
              . "' does not exist (check autocreate_group_name config token).";
            logger( $config, $msg, 3 );
        ...
        if ( !defined( get_group_name( $dbh, $config->{'autocreate_group'} ) ) )
        {
            my $msg =
                "Group id "
              . $config->{'autocreate_group'}
              . " does not exist (check autocreate_group config token).";
            logger( $config, $msg, 3 );
            print_message( $config, $msg, 1 );
            pandora_event( $config, $msg, 0, 0, 0, 0, 0, 'error', 0, $dbh );
        }
...
sub run {
    ...
    print_message( $pa_config,
        " [*] Starting " . $pa_config->{'rb_product_name'} . " Data Server.",
        1 );
...
sub data_producer {
    ...
    opendir( my $dir_fh, $pa_config->{'incomingdir'} )
    ...
    # Sort the queue
    {
        ...
        if ( $pa_config->{'dataserver_lifo'} == 0 ) {
            @sorted = sort {
                -M $pa_config->{'incomingdir'}
                  . "/$b" <=> -M $pa_config->{'incomingdir'} . "/$a"
                  || $a cmp $b
            } (@files);
        }
        else {
    ...
    foreach my $file (@sorted) {
        ...
        $AgentCounts{$agent_name} =
          defined( $AgentCounts{$agent_name} )
          ? $AgentCounts{$agent_name} + 1
          : 1;
        ...
        while ( my ( $agent_name, $xml_count ) = each(%AgentCounts) ) {
            if ( $xml_count > $pa_config->{'too_many_xml'} ) {
                pandora_timed_event(
                    ...
                    $pa_config,
                    "More than "
                      . $pa_config->{'too_many_xml'}
                      . " XML files queued for agent $agent_name",
                    0,
                    0,
                    0,
                    0,
...
sub data_consumer {
    ...
    for ( 0 .. 1 ) {
        ...
        $xml_data->{'timestamp'} =
          strftime( "%Y-%m-%d %H:%M:%S", localtime( ( stat($file_name) )[9] ) )
          if ( $pa_config->{'use_xml_timestamp'} eq '0'
            || !defined( $xml_data->{'timestamp'} ) );
        ...
        eval {
            if ( defined( $xml_data->{'server_name'} ) ) {
                process_xml_server( $self->getConfig(), $file_name, $xml_data,
                    $self->getDBH() );
            }
            else {
                process_xml_data( $self->getConfig(), $file_name, $xml_data,
                    $self->getServerID(), $self->getDBH() );
            }
        };
    ...
    pandora_event( $pa_config, "Unable to process XML data file '$task'.",
        0, 0, 0, 0, 0, 'error', 0, $dbh );
    agent_unlock( $pa_config, $agent_name );
}
...
sub process_xml_data {
    ...
    my (
        $agent_name, $agent_version,   $timestamp, $interval,
        $os_version, $timezone_offset, $custom_id, $url_address
      )
      = (
        $data->{'agent_name'}, $data->{'version'},
        $data->{'timestamp'},  $data->{'interval'},
        $data->{'os_version'}, $data->{'timezone_offset'},
    ...
    # Apply an offset to the timestamp
        ...
        logger(
            $pa_config,
            "Applied a timezone offset of $timestamp to agent "
              . $data->{'agent_name'},
            10
        );
...
# A module with No-learn mode (modo = 0) creates its modules on database only when it is created
    ...
    my $agent_id =
      get_db_value( $dbh, "SELECT id_agente FROM tagente WHERE nombre = ?",
    ...
    if ( $agent_id < 1 ) {
        if ( $pa_config->{'autocreate'} == 0 ) {
            logger( $pa_config,
                "ERROR: There is no agent defined with name $agent_name", 3 );
            return;
        }
        ...
        $group_id = pandora_get_agent_group( $pa_config, $dbh, $agent_name,
            $data->{'group'}, $data->{'group_password'} );
        if ( $group_id <= 0 ) {
            pandora_event(
                $pa_config,
        ...
        my $alias =
          ( defined( $data->{'agent_alias'} ) && $data->{'agent_alias'} ne '' )
          ? $data->{'agent_alias'}
          : $data->{'agent_name'};
        $agent_id = pandora_create_agent(
            ...
            $os,              $description,
            $interval,        $dbh,
            $timezone_offset, undef,
            undef,            undef,
            undef,            $custom_id,
            $url_address,     $agent_mode,
            $alias
        );
        if ( !defined($agent_id) ) {
            return;
        ...
        if ( $address ne '' ) {
            pandora_add_agent_address( $pa_config, $agent_id, $agent_name,
                $address, $dbh );
        }
        ...
        if ( defined( $data->{'custom_fields'} ) ) {
            foreach my $custom_fields ( @{ $data->{'custom_fields'} } ) {
                foreach my $custom_field ( @{ $custom_fields->{'field'} } ) {
                    ...
                    logger( $pa_config,
                        "Processing custom field '" . $cf_name . "'", 10 );

                    # Check if the custom field exists
                    my $custom_field_info =
                      get_db_single_row( $dbh,
                        'SELECT * FROM tagent_custom_fields WHERE name = ?',
                        safe_input($cf_name) );

                    # If it exists add the value to the agent
                    if ( defined($custom_field_info) ) {
                        my $cf_value = safe_input(
                            get_tag_value( $custom_field, 'value', '' ) );

                        my $field_agent;

                        $field_agent->{'id_agent'} = $agent_id;
                        $field_agent->{'id_field'} =
                          $custom_field_info->{'id_field'};
                        $field_agent->{'description'} = $cf_value;

                        db_process_insert( $dbh, 'id_field',
                            'tagent_custom_data', $field_agent );
                    }
                    else {
                        logger(
                            $pa_config,
                            "The custom field '"
                              . $cf_name
                              . "' does not exist. Discarded from XML",
                            5
                        );
                    }
                }
    ...
    return if ( PandoraOpen::Tools::is_metaconsole($pa_config) );
    ...
    if ( !defined($agent) ) {
        logger( $pa_config,
            "Error retrieving information for agent ID $agent_id", 10 );
        return;
    }
    ...
    if ( defined( $data->{'satellite_server'} ) ) {
        $satellite_server_id =
          get_server_id( $dbh, $data->{'satellite_server'}, SATELLITESERVER );
        if ( $satellite_server_id < 0 ) {
            logger(
                $pa_config,
                "Satellite Server '"
                  . $data->{'satellite_server'}
                  . "' does not exist.",
                10
            );
...
# But, if agent is in mode autodisable, put it enable and retrieve all data
    if ( $agent->{'disabled'} == 1 ) {
        ...
        logger( $pa_config,
            "Autodisable agent ID $agent_id is recovered to enable mode.", 10 );
        db_do( $dbh, 'UPDATE tagente SET disabled=0 WHERE id_agente=?',
            $agent_id );
    }
    ...
    # Learning mode
        ...
        if ( $address ne '' && $address ne $agent->{'direccion'} ) {
            pandora_update_agent_address( $pa_config, $agent_id, $agent_name,
                $address, $dbh )
              unless $agent->{'fixed_ip'} == 1;
            pandora_add_agent_address( $pa_config, $agent_id, $agent_name,
                $address, $dbh );
        }
        ...
        if ( $pa_config->{'update_parent'} == 1 && $parent_id != 0 ) {
            logger( $pa_config,
                "Updating agent $agent_name parent_id: $parent_id", 5 );
        }
        else {
            $parent_id = $agent->{'id_parent'};
        }
            ...
            foreach my $custom_fields ( @{ $data->{'custom_fields'} } ) {
                foreach my $custom_field ( @{ $custom_fields->{'field'} } ) {
                    ...
                    if ( defined($custom_field_info) ) {
                        ...
                        my $custom_field_data = get_db_single_row(
                            $dbh,
'SELECT * FROM tagent_custom_data WHERE id_field = ? AND id_agent = ?',
                            $custom_field_info->{"id_field"},
                            $agent->{"id_agente"}
                        );
...
"UPDATE tagent_custom_data SET description = ? WHERE id_field = ? AND id_agent = ?",
    ...
    pandora_update_agent(
        $pa_config,  $timestamp,       $agent_id,
        $os_version, $agent_version,   $interval,
        $dbh,        $timezone_offset, $parent_id,
        $satellite_server_id
    );
    ...
    if ( $pa_config->{'activate_gis'} != 0 && $agent->{'update_gis_data'} == 1 )
    {
        pandora_update_gis_data(
            ...
            $data->{'altitude'},  $data->{'position_description'},
            $timestamp
        );
    }

    # Update keep alive modules
    pandora_module_keep_alive( $pa_config, $agent_id, $agent_name, $server_id,
        $dbh );
    ...
    foreach my $module_data ( @{ $data->{'module'} } ) {
        ...
        if (
            defined(
                  ...
                  && $module_data->{'timestamp'} ne ''
            )
          )
        {
            $module_data->{'timestamp'} = strftime(
                "%Y-%m-%d %H:%M:%S",
                localtime(
                    $module_data->{'timestamp'} + ( $timezone_offset * 3600 )
                )
            );
        }
        ...
        if ( !defined( $module_data->{'datalist'} ) ) {
            my $data_timestamp =
              get_tag_value( $module_data, 'timestamp', $timestamp );
            if ( $pa_config->{'use_xml_timestamp'} eq '0'
                && defined($timestamp) )
            {
            ...
            $data_timestamp =
              apply_timezone_offset( $data_timestamp, $timezone_offset );

            process_module_data(
                $pa_config,   $module_data, $server_id, $agent,
                $module_name, $module_type, $interval,  $data_timestamp,
                $dbh,         $new_agent
            );
            next;
        }
        ...
        foreach my $list ( @{ $module_data->{'datalist'} } ) {
            ...
            foreach my $data ( @{ $list->{'data'} } ) {
                ...
                my $data_timestamp =
                  get_tag_value( $data, 'timestamp', $timestamp );
                if ( $pa_config->{'use_xml_timestamp'} eq '0'
                    && defined($timestamp) )
                {
                    $data_timestamp = $timestamp;
                ...
                process_module_data(
                    $pa_config, $module_data,    $server_id,
    ...
    foreach my $module_data ( @{ $data->{'module'} } ) {
        ...
        my $parent_module_name =
          get_tag_value( $module_data, 'module_parent', undef );
        my $parent_module_unlink =
          get_tag_value( $module_data, 'module_parent_unlink', undef );

        next
          if ( ( !defined($parent_module_name) )
            && ( !defined($parent_module_unlink) ) );

        link_modules( $pa_config, $dbh, $agent_id, $module_name,
            $parent_module_name )
          if ( defined($parent_module_name) && ( $parent_module_name ne '' ) );
        unlink_modules( $pa_config, $dbh, $agent_id, $module_name )
          if ( defined($parent_module_unlink)
            && ( $parent_module_unlink eq '1' ) );
    }

    # Process inventory modules
    process_inventory_data( $pa_config, $data, $server_id, $agent_name,
...
sub process_module_data {
    my (
        $pa_config,   $data,        $server_id, $agent,
    ...
    my $tags = {
        ...
        'cron_interval'         => 0,
        'ff_type'               => 0,
        'min_warning_forced'    => 0,
        'max_warning_forced'    => 0,
        'min_critical_forced'   => 0,
        'max_critical_forced'   => 0,
        'str_warning_forced'    => 0,
        'str_critical_forced'   => 0
    };
    ...
    while ( my ( $tag, $value ) = each( %{$data} ) ) {
        if ( defined( $tags->{$tag} ) ) {
            ...
            $module_conf->{'extended_info'} .=
              "$tag: " . get_tag_value( $data, $tag, '' ) . '<br/>';
        }
    }

    # Reload alert_template to get all alerts like an array
    $module_conf->{'alert_template'} =
      get_tag_value( $data, 'alert_template', '', 1 );
    ...
    if ( defined( $module_conf->{'cron_interval'} ) ) {
        ...
        $module_conf->{'module_interval'} =
          $interval * $module_conf->{'module_interval'};
    }
    else {
    ...
    $module_conf->{'post_process'} =~ s/,/./
      if ( defined( $module_conf->{'post_process'} ) );

    # avoid NULL columns
    $module_conf->{'critical_instructions'} = ''
      unless defined( $module_conf->{'critical_instructions'} );
    $module_conf->{'warning_instructions'} = ''
      unless defined( $module_conf->{'warning_instructions'} );
    $module_conf->{'unknown_instructions'} = ''
      unless defined( $module_conf->{'unknown_instructions'} );
    $module_conf->{'disabled_types_event'} = ''
      unless defined( $module_conf->{'disabled_types_event'} );
    $module_conf->{'module_macros'} = ''
      unless defined( $module_conf->{'module_macros'} );
    ...
    my $module = get_db_single_row(
        $dbh,
        'SELECT * FROM tagente_modulo WHERE id_agente = ? AND '
          . db_text('nombre') . ' = ?',
        $agent->{'id_agente'},
...
#}
        ...
        if ( ( $agent->{'modo'} == 0 ) && !($force_processing) ) {
            logger(
                $pa_config,
"Learning mode disabled. Skipping module '$module_name' agent '$agent_name'.",
                10
            );
            return;
        }
        ...
        if ( $module_conf->{'id_tipo_modulo'} <= 0 ) {
            logger(
                $pa_config,
"Invalid module type '$module_type' for module '$module_name' agent '$agent_name'.",
                3
            );
            return;
        }
        ...
        if ( defined $module_conf->{'module_group'} ) {
            my $id_group_module =
              get_module_group_id( $dbh, $module_conf->{'module_group'}, 1 );
...
# module_parent_unlink is a special case. It is not stored in the DB, but we will need it later.
        ...
        # Create the module
        my $module_id =
          pandora_create_module_from_hash( $pa_config, $module_conf, $dbh );
        ...
        if ( !defined($module) ) {
            logger(
                $pa_config,
"Could not create module '$module_name' for agent '$agent_name'.",
                3
            );
            return;
        }
        ...
        if ( defined($module_tags) ) {
            logger(
                $pa_config,
"Processing module tags '$module_tags' in module '$module_name' for agent '$agent_name'.",
                10
            );
            my @module_tags = split( /,/, $module_tags );
            for ( my $i = 0 ; $i <= $#module_tags ; $i++ ) {
                my $tag_info = get_db_single_row(
                    $dbh,
                    'SELECT * FROM ttag WHERE name = ?',
                    safe_input( $module_tags[$i] )
                );
                if ( defined($tag_info) ) {
                    my $tag_module;

                    $tag_module->{'id_tag'} = $tag_info->{'id_tag'};
                    $tag_module->{'id_agente_modulo'} =
                      $module->{'id_agente_modulo'};

                    db_process_insert( $dbh, 'id_tag', 'ttag_module',
                        $tag_module );
                }
            }
        }
        ...
        if ($initial_alert_template) {
            foreach my $individual_template ( @{$initial_alert_template} ) {
                my $id_alert_template = get_db_value(
                    $dbh,
'SELECT id FROM talert_templates WHERE talert_templates.name = ?',
                ...
                if ( defined($id_alert_template) ) {
                    pandora_create_template_module( $pa_config, $dbh,
                        $module->{'id_agente_modulo'},
                        $id_alert_template );
                }
            }
        ...
        $module->{'descripcion'} = ''
          unless defined( $module->{'descripcion'} );
        $module->{'extended_info'} = ''
          unless defined( $module->{'extended_info'} );

        # Set default values
        $module_conf->{'descripcion'} = $module->{'descripcion'}
          unless defined( $module_conf->{'descripcion'} );
        $module_conf->{'extended_info'} = $module->{'extended_info'}
          unless defined( $module_conf->{'extended_info'} );
        $module_conf->{'module_interval'} = $module->{'module_interval'}
          unless defined( $module_conf->{'module_interval'} );
    }

    # Update module configuration if in learning mode.
    if ( ( $agent->{'modo'} eq '1' ) || ( $agent->{'modo'} eq '2' ) ) {
    ...
    if ( $module->{'disabled'} eq '1' ) {
        logger( $pa_config,
            "Skipping disabled module '$module_name' agent '$agent_name'.",
            10 );
        return;
    }
...
sub update_module_configuration {
    ...
    foreach my $conf_token ( 'descripcion', 'extended_info', 'module_interval' )
    {
        if ( $module->{$conf_token} ne $module_conf->{$conf_token} ) {
            logger(
                $pa_config,
                "Updating configuration for module '"
            ...
            db_do(
                $dbh,
'UPDATE tagente_modulo SET descripcion = ?, extended_info = ?, module_interval = ?
				WHERE id_agente_modulo = ?',
                $module_conf->{'descripcion'} eq ''
                ? $module->{'descripcion'}
                : $module_conf->{'descripcion'},
                $module_conf->{'extended_info'},
    ...
    # Update module hash
    $module->{'extended_info'} = $module_conf->{'extended_info'}
      if ( defined( $module_conf->{'extended_info'} ) );
    $module->{'descripcion'} =
      ( $module_conf->{'descripcion'} eq '' )
      ? $module->{'descripcion'}
      : $module_conf->{'descripcion'};
    $module->{'module_interval'} =
      ( $module_conf->{'module_interval'} eq '' )
      ? $module->{'module_interval'}
      : $module_conf->{'module_interval'};
...
sub process_xml_server {
    ...
    my ( $server_name, $server_type, $version, $threads, $modules ) = (
        $data->{'server_name'}, $data->{'server_type'},
        $data->{'version'},     $data->{'threads'},
        $data->{'modules'}
    );
    ...
    pandora_update_server(
        ...
        1,                      $server_type,
        $threads,               $modules,
        $version,               $data->{'keepalive'},
        $data->{'disabled'},    $data->{'remote_config'}
    );
}
...
sub link_modules {
    ...
    logger(
        $pa_config,
"Linking module $child_name to module $parent_name for agent ID $agent_id",
        10
    );
    db_do(
        $dbh,
"UPDATE tagente_modulo SET parent_module_id = ? WHERE id_agente_modulo = ?",
        $parent_id,
        $child_id
    );
}
...
sub unlink_modules {
    ...
    # Link them.
    logger( $pa_config,
        "Unlinking parent from module $child_name agent ID $agent_id", 10 );
    db_do(
        $dbh,
"UPDATE tagente_modulo SET parent_module_id = 0 WHERE id_agente_modulo = ?",
        $child_id
    );
}
...
sub process_events_dataserver {
    ...
    foreach my $event ( @{ $data->{'events'}->[0]->{'event'} } ) {
        ...
        if ($@) {
            logger( $pa_config, "Error processing base64 event data '$event'.",
                5 );
            next;
        }
