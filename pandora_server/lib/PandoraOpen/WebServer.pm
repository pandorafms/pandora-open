package PandoraOpen::WebServer;
##########################################################################
# Pandora Open Web Server.
# Pandora Open. the Flexible Monitoring System. http://www.pandorafms.org
##########################################################################
# Copyright (c) 2007-2023 Pandora Open
#
...
use PandoraOpen::Goliat::GoliatTools;
use PandoraOpen::Goliat::GoliatConfig;
...
sub new {
    ...
    return
      unless defined( $config->{'webserver'} )
      and ( $config->{'webserver'} == 1 );

    # Use Goliat with CURL
    if ( $config->{'web_engine'} eq 'curl' ) {
        require PandoraOpen::Goliat::GoliatCURL;
        PandoraOpen::Goliat::GoliatCURL->import;
        ...
        if ( system("curl -V >/dev/null 2>&1") >> 8 != 0 ) {
            ...
            logger(
                $config,
' [E] CURL binary not found. Install CURL or uncomment the web_engine configuration token to use LWP.',
                1
            );
            print_message(
                $config,
' [E] CURL binary not found. Install CURL or uncomment the web_engine configuration token to use LWP.',
        ...
        if (
            system(
                "\"" . $config->{'plugin_exec'} . "\" 10 echo >/dev/null 2>&1"
            ) >> 8 != 0
          )
        {
            logger(
                ...
                ' [E] '
                  . $config->{'plugin_exec'}
                  . ' not found. Please install it or add it to the PATH.',
                1
            );
            print_message(
                $config,
    ...
    # Use LWP by default
        ...
        require PandoraOpen::Goliat::GoliatLWP;
        PandoraOpen::Goliat::GoliatLWP->import;

        if ( !LWP::UserAgent->can('ssl_opts') ) {
            logger(
                $config,
"LWP version $LWP::VERSION does not support SSL. Make sure version 6.0 or higher is installed.",
                1
            );
            print_message(
                $config,
" [W] LWP version $LWP::VERSION does not support SSL. Make sure version 6.0 or higher is installed.",
                1
            );
        }
    }
    ...
    my $self = $class->SUPER::new(
        $config, WEBSERVER,
        \&PandoraOpen::WebServer::data_producer,
        \&PandoraOpen::WebServer::data_consumer, $dbh
    );
...
sub run {
    ...
    print_message( $pa_config,
        " [*] Starting " . $pa_config->{'rb_product_name'} . " Web Server.",
        1 );
...
'SELECT tagente_modulo.id_agente_modulo, tagente_modulo.flag, tagente_estado.current_interval + tagente_estado.last_execution_try AS time_left, last_execution_try
        ...
        @rows = get_db_rows(
            $dbh,
'SELECT DISTINCT(tagente_modulo.id_agente_modulo), tagente_modulo.flag, tagente_estado.current_interval + tagente_estado.last_execution_try  AS time_left, last_execution_try
			...
			ORDER BY tagente_modulo.flag DESC, time_left ASC, last_execution_try ASC',
            $pa_config->{'servername'}, WEBSERVER
        );
    }
...
sub data_consumer {
    ...
    our ( @task_fails, @task_time, @task_ssec, @task_get_content )
      ;    # Defined in GoliatLWP.pm and GoliatCURL.

    # Retrieve module data
    my $module = get_db_single_row( $dbh,
        'SELECT * FROM tagente_modulo WHERE id_agente_modulo = ?', $module_id );
    ...
    my %macros = (
        ...
        _modulegroup_ => ( defined($module) )
        ? ( get_module_group_name( $dbh, $module->{'id_module_group'} ) || '' )
        : '',
        _moduledescription_ => ( defined($module) ) ? $module->{'descripcion'}
        : '',
        _modulestatus_ => ( defined($module) )
        ? get_agentmodule_status( $pa_config, $dbh,
            $module->{'id_agente_modulo'} )
        : '',
        _moduletags_ => ( defined($module) )
        ? pandora_get_module_url_tags( $pa_config, $dbh,
            $module->{'id_agente_modulo'} )
        : '',
    ...
    # Pandora Open variables passed to Goliat.
    ...
    my $module_data;
    {
        ...
        if ( $module_type eq 'web_proc' ) {
            $module_data =
              ( $task_fails[0] == 0 && $task_get_content[0] ne "" ) ? 1 : 0;
        }
    ...
    pandora_process_module(
        ...
        $utimestamp,  $self->getServerID(),
        $dbh
    );

    my $agent_os_version = get_db_value(
        $dbh,
        'SELECT os_version FROM tagente WHERE id_agente = ?',
        $module->{'id_agente'}
    );
