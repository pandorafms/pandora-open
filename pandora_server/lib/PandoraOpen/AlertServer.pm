package PandoraOpen::AlertServer;
################################################################################
# Pandora Open Alert Server.
...
sub new {
    ...
    return unless $config->{'alertserver'} == 1;
    ...
    my $self = $class->SUPER::new(
        $config, ALERTSERVER,
        \&PandoraOpen::AlertServer::data_producer,
        \&PandoraOpen::AlertServer::data_consumer, $dbh
    );
...
sub run {
    ...
    print_message( $pa_config,
        " [*] Starting " . $pa_config->{'rb_product_name'} . " Alert Server.",
        1 );
...
'SELECT COUNT(*) FROM `tserver` WHERE `server_type` = ? AND `status` = 1',
    ...
    my $i       = 0;
    my %servers = map { $_->{'name'} => $i++; } get_db_rows(
        $dbh,
'SELECT `name` FROM `tserver` WHERE `server_type` = ? AND `status` = 1 ORDER BY `name` ASC',
    ...
    foreach my $row (@rows) {
          ...
          )
        {
            ...
            pandora_event(
                $pa_config,
                "Alert execution delay has exceeded "
                  . $pa_config->{'alertserver_warn'}
                  . " seconds.",
                0,
                0,
                3,
    ...
    eval {
        {
            ...
            my $task = get_db_single_row( $dbh,
                'SELECT * FROM talert_execution_queue WHERE id = ?', $task_id );
            if ( !defined($task) ) {
                logger( $pa_config, "[ERROR] Executing invalid alert", 0 );
                last 0;
            }

            my $args = PandoraOpen::Tools::p_decode_json( $pa_config,
            ...
            my $execution_args =
              [ $pa_config, @args[ 0 .. 4 ], $dbh, @args[ 5 .. $#args ] ];

            # Execute.
            PandoraOpen::Core::pandora_execute_alert(@$execution_args);
        }
    };
    if ($@) {
