package PandoraOpen::PluginServer;
##########################################################################
# Pandora Open Plugin Server.
...
sub new {
    ...
    return unless $config->{'pluginserver'} == 1;
    ...
    if ( !-x $config->{'plugin_exec'} ) {
        logger(
            ...
            ' [E] '
              . $config->{'plugin_exec'}
              . ' not found. Plugin Server not started.',
            1
        );
        print_message(
            $config,
    ...
    my $self = $class->SUPER::new(
        $config, PLUGINSERVER,
        \&PandoraOpen::PluginServer::data_producer,
        \&PandoraOpen::PluginServer::data_consumer, $dbh
    );
...
sub run {
    ...
    print_message(
        $pa_config,
        " [*] Starting " . $pa_config->{'rb_product_name'} . " Plugin Server.",
        1
    );
...
'SELECT DISTINCT(tagente_modulo.id_agente_modulo), tagente_modulo.flag, tagente_estado.current_interval + tagente_estado.last_execution_try AS time_left, last_execution_try
			...
			AND (tagente_modulo.flag = 1 OR (tagente_estado.last_execution_try + tagente_estado.current_interval) < UNIX_TIMESTAMP())
			ORDER BY tagente_modulo.flag DESC, time_left ASC, last_execution_try ASC',
            safe_input( $pa_config->{'servername'} ), PLUGINSERVER
        );
    }
...
sub data_consumer {
    ...
    eval {
        if ( $module->{'macros'} ne '' ) {
            logger(
                $pa_config,
"Decoding json macros from # $module_id plugin command '$command'",
                10
            );
            my $macros =
              p_decode_json( $pa_config, encode_utf8( $module->{'macros'} ) );
            my %macros;
            ...
            if ( ref( \%macros ) eq "HASH" ) {
                foreach my $macro_id ( keys(%macros) ) {
                    ...
                    my $macro_value =
                      ...
                      ? pandora_output_password( $pa_config,
                    ...
                    $plugin_macros_for_alert_processing{$name_for_desc} =
                      $macro_desc;
                    $plugin_macros_for_alert_processing{$name_for_value} =
                      $macro_value;
                }
            }
        }
    ...
    if ($@) {
        logger( $pa_config,
            "Error reading macros from module # $module_id. Error: $@", 10 );
    }
    ...
    $parameters =
      subst_alert_macros( $parameters, \%macros, $pa_config, $dbh, $agent,
        $module );
    ...
    if ($@) {
        logger(
            $pa_config,
"Error reading macros from module # $module_id. Probably malformed json",
            10
        );
    }
    ...
    logger( $pa_config, "Executing AM # $module_id plugin command '$command'",
        9 );
    ...
    eval {
        ...
        if ( $? < 0 ) {
            logger(
                $pa_config,
"Error executing command from module # $module_id. Probably out of memory.",
                10
            );
            pandora_timed_event(
                300,
                $pa_config,
"Cannot process monitoring data. plug-in module \#$module_id failed to execute on server "
...
"[ERROR] Undefined value returned by plug-in module '%s' in agent whith name '%s' and alias '%s'. Is the server out of memory?",
    ...
    pandora_process_module( $pa_config, \%data, '', $module, '', $timestamp,
        $utimestamp, $self->getServerID(), $dbh,
        \%plugin_macros_for_alert_processing );
