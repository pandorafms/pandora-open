package PandoraOpen::PredictionServer;
########################################################################
# Pandora Open Prediction Server.
...
use PandoraOpen::Statistics::Regression;
...
sub new {
    ...
    return unless $config->{'predictionserver'} == 1;
    ...
    my $self = $class->SUPER::new(
        $config, PREDICTIONSERVER,
        \&PandoraOpen::PredictionServer::data_producer,
        \&PandoraOpen::PredictionServer::data_consumer, $dbh
    );
...
sub data_producer {
    ...
    if ( pandora_is_master( $pa_config, $dbh ) == 0 ) {
        @rows = get_db_rows(
            $dbh, 'SELECT tagente_modulo.id_agente_modulo,
        ...
        @rows = get_db_rows(
            $dbh, 'SELECT DISTINCT(tagente_modulo.id_agente_modulo),
				tagente_modulo.flag, last_execution_try
			FROM tagente, tagente_modulo, tagente_estado
			WHERE ((server_name = ?)
...
sub data_consumer {
    ...
    exec_prediction_module( $self->getConfig(), $task, $self->getServerID(),
        $self->getDBH() );
}
...
sub exec_prediction_module {
    ...
    my $agent_module = get_db_single_row(
        $dbh, 'SELECT *
		FROM tagente_modulo
    ...
    if ( $agent_module->{'prediction_module'} == 5 ) {
        logger( $pa_config,
            "Executing cluster status module " . $agent_module->{'nombre'},
            10 );
        exec_cluster_status_module( $pa_config, $agent_module, $server_id,
            $dbh );
        return;
    }
    ...
    if ( $agent_module->{'prediction_module'} == 6 ) {
        logger(
            $pa_config,
            "Executing cluster active-active module "
              . $agent_module->{'nombre'},
            10
        );
    ...
    if ( $agent_module->{'prediction_module'} == 7 ) {
        logger(
            $pa_config,
            "Executing cluster active-passive module "
              . $agent_module->{'nombre'},
            10
        );
    ...
    exec_capacity_planning_module( $pa_config, $agent_module, $server_id,
        $dbh );
}
...
sub exec_capacity_planning_module {
    ...
    my $target_module = get_db_single_row(
        $dbh,
    ...
    eval {
        ( $theta_0, $theta_1 ) =
          linear_regression( $target_module, $from, $now, $dbh );
    };
...
sub linear_regression {
    ...
    my @rows = get_db_rows(
        $dbh,
'SELECT datos, utimestamp FROM tagente_datos WHERE id_agente_modulo = ? AND utimestamp > ? AND utimestamp < ? ORDER BY utimestamp ASC',
    ...
    my $reg = PandoraOpen::Statistics::Regression->new( "linear regression",
        [ "const", "x" ] );
