## no critic
package PandoraOpen::Sendmail;

# Repackaged for work "by default" in PandoraOpen.
# Original library by:
...
sub _digest_md5 {
    ...
    return
qq(username="$user",realm="$ckey{realm}",nonce="$nonce",nc=$nc,cnonce="$cnonce",digest-uri="$uri",response=$hv,qop=$qop);
}
...
sub sendmail {
    ...
    my (
        %mail,           $k,         $smtp,     $server,
        $port,           $localhost, $fromaddr, $recip,
        @recipients,     $to,        $header,   %esmtp,
        @wanted_methods, $encryption
    );
        ...
        for $i ( 0 .. $#_ ) {
            ...
            eval {
                ...
                while ( $data_sent < length($$data) ) {
                    $data_sent +=
                      syswrite( $sockets[0], $$data,
                        length($$data) - $data_sent, $data_sent )
                      || die $!;
                }
            };
            if ($@) {
...
"Received false mail hash key: \'$k\'. Did you forget to put it in quotes?\n";
    ...
    {    # don't warn for undefined values below
        local $^W = 0;
        $mail{'Message'} =
          join( "", $mail{'Message'}, $mail{'Body'}, $mail{'Text'} );
    }
...
"Sending 8bit characters without encoding, hoping it will come across OK.\n"
    ...
    foreach $server ( @{ $mailcfg{'smtp'} } ) {
        ...
        if ( $encryption ne 'none' ) {
            eval "require IO::Socket::SSL"
              || return fail("IO::Socket::SSL is not available");
        }
    ...
    if ( defined($auth) && $auth->{'user'} ne '' ) {
        ...
        if (@methods) {
            ...
            _require_base64()
              || fail(
                "Could not use MIME::Base64 module required for authentication"
              );
            if ( $method eq "LOGIN" ) {
                ...
                _require_md5()
                  || fail(
"Could not use Digest::MD5 module required for authentication"
                  );
                ...
                $challenge =~ s/^\d+\s+//;
                my $response =
                  _hmac_md5( $auth->{password}, decode_base64($challenge) );
                socket_write( encode_base64( "$auth->{user} $response", "" ),
                    $CRLF )
                  || return fail("AUTH CRAM-MD5 failed: $server_reply");
                socket_read()
                  || return fail("AUTH CRAM-MD5 failed: $server_reply");
...
"Could not use Digest::MD5 module required for authentication"
                ...
                warn "\nCHALLENGE=", decode_base64($challenge), "\n"
                  if ( $mailcfg{debug} > 9 );
                my $response =
                  _digest_md5( $auth->{user}, $auth->{password},
                ...
                return fail(
"$method not supported (and wrongly advertised as supported by this silly module)\n"
                );
            }
            ...
            if ( $auth->{required} ) {
                return fail(
                        "Required AUTH method '$auth->{method}' not supported. "
                      . "(Server supports '$esmtp{'AUTH'}'. Module supports: '$auth_support')"
                );
            }
            else {
                warn
"No common authentication method! Requested: '$auth->{method}'. Server supports '$esmtp{'AUTH'}'. Module supports: '$auth_support'. Skipping authentication\n";
            }
        }
    }
