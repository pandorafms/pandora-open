...
# Defined in PandoraOpen::Core.pm
our @ServerSuffixes;
...
sub setEventStormProtection {
    ...
    $PandoraOpen::Core::EventStormProtection = $event_storm_protection;
}
...
sub update {
    ...
    eval {
        pandora_update_server(
            $self->{'_pa_config'},                 $self->{'_dbh'},
            $self->{'_pa_config'}->{'servername'}, $self->{'_server_id'},
            1,                                     $self->{'_server_type'},
            $self->{'_num_threads'},               $self->{'_queue_size'}
        );
    };
}
