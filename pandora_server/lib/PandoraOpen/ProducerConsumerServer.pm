package PandoraOpen::ProducerConsumerServer;
##########################################################################
# Pandora Open generic server.
...
use PandoraOpen::Server;
use PandoraOpen::Tools;

# Inherits from PandoraOpen::Server
our @ISA = qw(PandoraOpen::Server);
...
########################################################################################
sub new {
    my ( $class, $config, $server_type, $producer, $consumer, $dbh ) = @_;
    ...
    $self->{'_producer_wrapper'} =
      \&PandoraOpen::ProducerConsumerServer::data_producer;
    $self->{'_consumer_wrapper'} =
      \&PandoraOpen::ProducerConsumerServer::data_consumer;
    ...
    for ( 1 .. $self->getNumThreads() ) {
        ...
        my $thr = threads->create(
            ...
            sub {
                ...
                $self->{'_consumer_stats'}->{ threads->tid() } =
                  $consumer_stats;
    ...
    # Launch producer thread
        ...
        my $thr = threads->create(
            ...
            sub {
                ...
                $self->{'_producer_stats'}->{ threads->tid() } =
                  $producer_stats;
...
sub data_consumer {
    ...
    my $dbh;
    my $sem_timeout =
        $pa_config->{'self_monitoring_interval'} > 0
