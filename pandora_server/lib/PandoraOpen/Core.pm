package PandoraOpen::Core;
##########################################################################
# Core Pandora Open functions.
# Pandora Open.
##########################################################################
# Copyright (c) 2005-2023 Pandora Open
#
...
PandoraOpen::Core - Core functions of Pandora Open
...
=head1 SYNOPSIS
 ...
 use PandoraOpen::Core;

=head1 DESCRIPTION

This module contains all the base functions of B<Pandora Open>, the most basic operations of the system are done here.
...
if ( $^O =~ /win/i ) {
    eval {
        require POSIX::strftime::GNU;
        POSIX::strftime::GNU->import();
        1;
    };
}
...
eval {
    local $SIG{__DIE__};
    require XML::SAX::ExpatXS;
    XML::SAX::ExpatXS->import();
    1;
};
if ( !$@ ) {
...
use PandoraOpen::DB;
use PandoraOpen::Config;
use PandoraOpen::Tools;
use PandoraOpen::GIS qw(distance_moved);
...
our @AlertStatus = (
    'Execute the alert',
    'Do not execute the alert',
    'Do not execute the alert, but increment its internal counter',
    'Cease the alert',
    'Recover the alert',
...
sub get_agent_from_alias {
    ...
    return if ( !defined($alias) || $alias eq '' );
    if ($relative) {
        return get_db_single_row( $dbh,
            'SELECT * FROM tagente WHERE tagente.alias like ?',
            safe_input($alias) );
    }

    return get_db_single_row( $dbh,
        'SELECT * FROM tagente WHERE tagente.alias = ?',
        safe_input($alias) );
}
...
sub get_agent_from_addr {
    ...
    my $agent = get_db_single_row(
        $dbh, 'SELECT * FROM taddress, taddress_agent, tagente
...
sub get_agent_from_name {
    ...
    if ($relative) {
        return get_db_single_row( $dbh,
            'SELECT * FROM tagente WHERE tagente.nombre like ?',
            safe_input($name) );
    }

    return get_db_single_row( $dbh,
        'SELECT * FROM tagente WHERE tagente.nombre = ?',
        safe_input($name) );
}
...
sub get_agent_from_id {
    ...
    return get_db_single_row( $dbh,
        'SELECT * FROM tagente WHERE tagente.id_agente = ?', $id );
}
...
sub pandora_generate_alerts {
    my (
        $pa_config,    $data,            $status, $agent,
        $module,       $utimestamp,      $dbh,    $timestamp,
        $extra_macros, $last_data_value, $alert_type
    ) = @_;
    ...
    if ( $pa_config->{'warmup_alert_on'} == 1 ) {
        ...
        return
          if (
            time() < $pa_config->{'__start_utimestamp__'} +
            $pa_config->{'warmup_alert_interval'} );

        $pa_config->{'warmup_alert_on'} = 0;
        logger( $pa_config, "Warmup mode for alerts ended.", 10 );
        pandora_event( $pa_config, "Warmup mode for alerts ended.",
    ...
    if ( defined($alert_type) ) {
        ...
        $alert_type_filter =
          $alert_type eq 'unknown'
          ? " AND (type = 'unknown' OR type = 'not_normal')"
          : " AND type = '$alert_type'";
    }
    my @alerts = get_db_rows(
        $dbh, '
    ...
    foreach my $alert (@alerts) {
        ...
        pandora_process_alert(
            $pa_config, $data,      $agent,
            $module,    $alert,     $rc,
            $dbh,       $timestamp, $extra_macros
        );
    }
}
...
sub pandora_evaluate_alert {
    my ( $pa_config, $agent, $data, $last_status, $alert, $utimestamp, $dbh,
        $last_data_value, $correlated_items, $event, $log )
      = @_;
    ...
    if ( defined($agent) ) {
        logger(
            $pa_config,
            "Evaluating alert '"
              . safe_output( $alert->{'name'} )
              . "' for agent '"
              . safe_output( $agent->{'nombre'} ) . "'.",
            10
        );
    }
    else {
        logger( $pa_config,
            "Evaluating alert '" . safe_output( $alert->{'name'} ) . "'.", 10 );
    }
    ...
    if ( $alert->{'special_day'} ) {
        logger( $pa_config,
            "Checking special days '" . $alert->{'name'} . "'.", 10 );
        ...
        $special_day = get_db_value(
            $dbh,
'SELECT day_code FROM talert_special_days WHERE (date = ? OR date = ?) AND (id_group = 0 OR id_group = ?) AND (id_calendar = ?) ORDER BY date DESC',
            $date,
            $date_every_year,
            $alert->{'id_group'},
            $alert->{'special_day'}
        ...
        if ( $special_day != 0 ) {
            ...
            return $status
              if ( !defined( $alert->{ $weeks[$special_day] } )
                || $alert->{ $weeks[$special_day] } == 0 );
        }
        else {
            logger( $pa_config,
                $date . " is *NOT* a special day for " . $alert->{'name'}, 10 );
    ...
    if ( defined( $alert->{'schedule'} ) && $alert->{'schedule'} ne '' ) {
        $schedule =
          PandoraOpen::Tools::p_decode_json( $pa_config, $alert->{'schedule'} );
        if ( !defined($special_day) ) {
            $special_day = 0;
        }
        ...
        if ( $special_day != 0 ) {
            return $status
              if ( !defined( $schedule->{ $weeks[$special_day] } ) );
        }
    }
    ...
    if ( defined($schedule) ) {
        ...
        if ( $special_day != 0
            && defined( $schedule->{ $weeks[$special_day] } ) )
        {
        ...
        foreach my $timeBlock ( @{ $schedule->{$schedule_day} } ) {
            if ( $timeBlock->{'start'} eq $timeBlock->{'end'} ) {
                ...
                $timeBlock->{'start'} le $time
                && (   ( $timeBlock->{'end'} eq '00:00:00' )
                    || ( $timeBlock->{'end'} ge $time ) )
              )
            {
                # In range.
                $inSlot = 1;
        ...
        if ( ( $alert->{'time_from'} ne $alert->{'time_to'} ) ) {
            if ( $alert->{'time_from'} lt $alert->{'time_to'} ) {
                return $status
                  if ( ( $time le $alert->{'time_from'} )
                    || ( $time ge $alert->{'time_to'} ) );
            }
            else {
                return $status
                  if ( ( $time le $alert->{'time_from'} )
    ...
    my $limit_utimestamp =
      $alert->{'last_reference'} + $alert->{'time_threshold'};
    ...
    if ( $alert->{'times_fired'} > 0 ) {
        ...
        if ( $utimestamp > $limit_utimestamp ) {
            ...
            $status = 4
              if ( $alert->{'recovery_notify'} == 1
                && !defined( $alert->{'id_template_module'} ) );

            # Always reset
            ( $alert->{'internal_counter'}, $alert->{'times_fired'} ) =
              ( 0, 0 );
        }
    ...
    # Simple alert
    return $status
      if ( $alert->{'type'} eq "min" && $data >= $alert->{'min_value'} );
    return $status
      if ( $alert->{'type'} eq "max" && $data <= $alert->{'max_value'} );

    if ( $alert->{'type'} eq "max_min" ) {
        if ( $alert->{'matches_value'} == 1 ) {
            return $status
              if ( $data <= $alert->{'min_value'}
    ...
    return $status
      if ( $alert->{'type'} eq "equal" && $data != $alert->{'value'} );
    return $status
      if ( $alert->{'type'} eq "not_equal" && $data == $alert->{'value'} );
    ...
    if ( $alert->{'type'} eq "complex" ) {
        ...
        my %time_windows_map = (
            ...
            this_month  =>
              sub { return timelocal( 0, 0, 0, 1, (localtime)[ 4, 5 ] ) },
            seven_days => sub { return time - 7 * 24 * 60 * 60 },
            this_week  =>
              sub { return time - ( (localtime)[6] % 7 ) * 24 * 60 * 60 },
            one_day => sub { return time - 1 * 24 * 60 * 60 },
            today   =>
              sub { return timelocal( 0, 0, 0, (localtime)[ 3, 4, 5 ] ) },
        );

        my $function  = $alert->{'math_function'};
        my $condition = $condition_map{ $alert->{'condition'} };
        my $window    = $time_windows_map{ $alert->{'time_window'} };
        my $value     = defined $alert->{'value'}
          && $alert->{'value'} ne "" ? $alert->{'value'} : 0;

        if (   ( grep { $_ eq $function } @allowed_functions ) == 1
            && defined($condition)
            && defined($window) )
        {
            ...
            my $historical_value =
              get_db_value( $dbh, $query, $alert->{"id_agent_module"},
                $window->() );
            ...
            if ( $function eq "avg" ) {
				...
				# <<<
                eval {
                    $activate_alert = eval("$data $condition $historical_value");    ## no critic (BuiltinFunctions::ProhibitStringyEval)
                };
				# >>>
            }
            else {
                ...
                eval {
                    $activate_alert =
                      eval("$historical_value $condition $value");    ## no critic (BuiltinFunctions::ProhibitStringyEval)
                };
				# >>>
            }
    ...
    return 2
      if ( ( $alert->{'internal_counter'} < $alert->{'min_alerts'} )
...
sub pandora_process_alert {
    my (
        $pa_config, $data, $agent,     $module, $alert,
        $rc,        $dbh,  $timestamp, $extra_macros
    ) = @_;
    ...
    if ( $rc == 3 ) {
        ...
        my $critical_instructions = get_db_value(
            $dbh,
'SELECT critical_instructions FROM tagente_modulo WHERE id_agente_modulo = ?',
            $alert->{'id_agent_module'}
        );
        my $warning_instructions = get_db_value(
            $dbh,
'SELECT warning_instructions FROM tagente_modulo WHERE id_agente_modulo = ?',
            $alert->{'id_agent_module'}
        );
        my $unknown_instructions = get_db_value(
            $dbh,
'SELECT unknown_instructions FROM tagente_modulo WHERE id_agente_modulo = ?',
            $alert->{'id_agent_module'}
        );
        ...
        return
          if ( ( ref($module) eq 'HASH' && $module->{'quiet'} != "0" )
        ...
        pandora_event(
            $pa_config,
            "Alert ceased (" . safe_output( $alert->{'name'} ) . ")",
            $agent->{'id_grupo'},
            $agent->{'id_agente'},
            $alert->{'priority'},
            $id,
            (
                defined( $alert->{'id_agent_module'} )
                ? $alert->{'id_agent_module'}
                : 0
            ),
            "alert_ceased",
    ...
    if ( $rc == 4 ) {
        ...
        if (   $pa_config->{'alertserver'} == 1
            || $pa_config->{'alertserver_queue'} == 1 )
        {
            pandora_queue_alert(
                $pa_config,
                $dbh,
                [
                    $data, $agent,     $module, $alert,
                    0,     $timestamp, 0,       $extra_macros
                ]
            );
        }
        else {
            pandora_execute_alert(
                $pa_config, $data, $agent, $module,
                $alert,     0,     $dbh,   $timestamp,
                0,          $extra_macros
            );
        }
        return;
    ...
    if ( $rc == 5 ) {
        db_do( $dbh,
            'UPDATE ' . $table . ' SET internal_counter = 0 WHERE id = ?',
            $id );
        return;
    }
    ...
    if ( $rc == 0 ) {
        ...
        db_do(
            ...
            $alert->{'times_fired'}, $utimestamp, $alert->{'internal_counter'},
            $id
        );
...
sub pandora_execute_alert {
    my (
        $pa_config,    $data,       $agent, $module,
        $alert,        $alert_mode, $dbh,   $timestamp,
        $forced_alert, $extra_macros
    ) = @_;

    # 'in-process' events can inhibit alers too.
    if (   $pa_config->{'event_inhibit_alerts'} == 1
        && $alert_mode != RECOVERED_ALERT )
    {
        my $status = get_db_value(
            $dbh,
'SELECT estado FROM tevento WHERE id_alert_am = ? ORDER BY utimestamp DESC LIMIT 1',
        ...
        if ( defined($status) && $status == 2 ) {
            logger(
                ...
                "Alert '"
                  . safe_output( $alert->{'name'} )
                  . "' inhibited by in-process events.",
                10
            );
            return;
        }
    ...
    if ( defined($module) ) {
        ...
        logger( $pa_config,
            "Executing alert '" . safe_output( $alert->{'name'} ) . "'.", 10 );
    }
...
'SELECT taa.name as action_name, taa.*, tac.*, tatma.id AS id_alert_templ_module_actions,
                    ...
                    $alert->{'id_template_module'}, $alert->{'times_fired'},
                    $alert->{'times_fired'},        $alert->{'times_fired'}
                );
            }
        }
...
'SELECT talert_actions.name as action_name, talert_actions.*, talert_commands.*
    ...
    # Log alert.
        ...
        if ( $alert_mode == RECOVERED_ALERT ) {
            @actions = get_db_rows(
                $dbh,
'SELECT talert_actions.name as action_name, tlog_alert_action.*, talert_actions.*, talert_commands.*
...
# Check the action threshold (template_action_threshold takes precedence over action_threshold)
        ...
        $action->{'last_execution'} = 0
          unless defined( $action->{'last_execution'} );
        $action->{'recovered'} = 0 unless defined( $action->{'recovered'} );

        $threshold = $action->{'action_threshold'}
          if ( defined( $action->{'action_threshold'} )
            && $action->{'action_threshold'} > 0 );
        $threshold = $action->{'module_action_threshold'}
          if ( defined( $action->{'module_action_threshold'} )
            && $action->{'module_action_threshold'} > 0 );
        if (
               ( time() >= ( $action->{'last_execution'} + $threshold ) )
            || ( $alert_mode == RECOVERED_ALERT && $action->{'recovered'} == 0 )
          )
        {
            ...
            push(
                @{ $custom_data->{'actions'} },
                safe_output( $action->{'action_name'} )
            );
              ...
              )
            {
                pandora_execute_action(
                    ...
                    $alert_mode, $action,
                    $module,     $dbh,
                    $timestamp,  $extra_macros,
                    $monitoring_event_custom_data
                );
            }
            else {
                logger(
                    $pa_config,
                    "Skipping recover action "
                      . safe_output( $action->{'name'} )
                      . " for alert '"
                      . safe_output( $alert->{'name'} )
                      . "' module '"
                      . safe_output( $module->{'nombre'} ) . "'.",
            ...
            if ( $alert_mode == RECOVERED_ALERT ) {
                ...
                if ( defined( $alert->{'id_template_module'} ) ) {
                    db_do(
                        $dbh,
'UPDATE talert_template_module_actions SET recovered = 1 WHERE id = ?',
                ...
                db_do(
                    $dbh,
'UPDATE talert_template_module_actions SET recovered = 0 WHERE id = ?',
                    $action->{'id_alert_templ_module_actions'}
                );
            }
        }
...
# Generate an event	only if an event has not already been generated by an alert action
    if (
        $event_generated == 0
        && (
            !defined( $alert->{'disable_event'} )
            || ( defined( $alert->{'disable_event'} )
                && $alert->{'disable_event'} == 0 )
        )
      )
    {
#If we've spotted an alert recovered, we set the new event's severity to 2 (NORMAL), otherwise the original value is maintained.
        my ( $text, $event, $severity ) =
            ( $alert_mode == RECOVERED_ALERT )
          ? ( 'recovered', 'alert_recovered', 2 )
          : ( 'fired', 'alert_fired', $alert->{'priority'} );
...
sub pandora_queue_alert {
    ...
    my $json_arguments =
      PandoraOpen::Tools::p_encode_json( $pa_config, $arguments );

    $json_arguments = encode_base64($json_arguments);

    db_do(
        $dbh, "INSERT INTO talert_execution_queue (data, utimestamp)
...
sub pandora_execute_action {
    my (
        $pa_config,  $data,         $agent,  $alert,
        $alert_mode, $action,       $module, $dbh,
        $timestamp,  $extra_macros, $custom_data
    ) = @_;
    ...
    logger(
        ...
        "Executing action '"
          ...
          . safe_output( $alert->{'name'} )
          . "' agent '"
          . ( defined($agent) ? safe_output( $agent->{'nombre'} ) : 'N/A' )
          . "'.",
        10
    );
    ...
    if ( !defined( $alert->{'snmp_alert'} ) ) {
        ...
        # Regular alerts
        $field1 =
          defined( $action->{'field1'} )
          && $action->{'field1'} ne ""
          ? $action->{'field1'}
          : $alert->{'field1'};
        $field2 =
          defined( $action->{'field2'} )
          && $action->{'field2'} ne ""
          ? $action->{'field2'}
          : $alert->{'field2'};
        $field3 =
          defined( $action->{'field3'} )
          && $action->{'field3'} ne ""
          ? $action->{'field3'}
          : $alert->{'field3'};
        $field4 =
          defined( $action->{'field4'} )
          && $action->{'field4'} ne ""
          ? $action->{'field4'}
          : $alert->{'field4'};
        $field5 =
          defined( $action->{'field5'} )
          && $action->{'field5'} ne ""
          ? $action->{'field5'}
          : $alert->{'field5'};
        $field6 =
          defined( $action->{'field6'} )
          && $action->{'field6'} ne ""
          ? $action->{'field6'}
          : $alert->{'field6'};
        $field7 =
          defined( $action->{'field7'} )
          && $action->{'field7'} ne ""
          ? $action->{'field7'}
          : $alert->{'field7'};
        $field8 =
          defined( $action->{'field8'} )
          && $action->{'field8'} ne ""
          ? $action->{'field8'}
          : $alert->{'field8'};
        $field9 =
          defined( $action->{'field9'} )
          && $action->{'field9'} ne ""
          ? $action->{'field9'}
          : $alert->{'field9'};
        $field10 =
          defined( $action->{'field10'} )
          && $action->{'field10'} ne ""
          ? $action->{'field10'}
          : $alert->{'field10'};
        $field11 =
          defined( $action->{'field11'} )
          && $action->{'field11'} ne ""
          ? $action->{'field11'}
          : $alert->{'field11'};
        $field12 =
          defined( $action->{'field12'} )
          && $action->{'field12'} ne ""
          ? $action->{'field12'}
          : $alert->{'field12'};
        $field13 =
          defined( $action->{'field13'} )
          && $action->{'field13'} ne ""
          ? $action->{'field13'}
          : $alert->{'field13'};
        $field14 =
          defined( $action->{'field14'} )
          && $action->{'field14'} ne ""
          ? $action->{'field14'}
          : $alert->{'field14'};
        $field15 =
          defined( $action->{'field15'} )
          && $action->{'field15'} ne ""
          ? $action->{'field15'}
          : $alert->{'field15'};
        $field16 =
          defined( $action->{'field16'} )
          && $action->{'field16'} ne ""
          ? $action->{'field16'}
          : $alert->{'field16'};
        $field17 =
          defined( $action->{'field17'} )
          && $action->{'field17'} ne ""
          ? $action->{'field17'}
          : $alert->{'field17'};
        $field18 =
          defined( $action->{'field18'} )
          && $action->{'field18'} ne ""
          ? $action->{'field18'}
          : $alert->{'field18'};
        $field19 =
          defined( $action->{'field19'} )
          && $action->{'field19'} ne ""
          ? $action->{'field19'}
          : $alert->{'field19'};
        $field20 =
          defined( $action->{'field20'} )
          && $action->{'field20'} ne ""
          ? $action->{'field20'}
          : $alert->{'field20'};
    }
        ...
        foreach my $field (@command_fields) {
            if ( !defined( $action->{ 'field' . $index } )
                || $action->{ 'field' . $index } eq "" )
            {
        ...
        $field1 =
          defined( $alert->{'field1'} )
          && $alert->{'field1'} ne ""
          ? $alert->{'field1'}
          : $action->{'field1'};
        $field2 =
          defined( $alert->{'field2'} )
          && $alert->{'field2'} ne ""
          ? $alert->{'field2'}
          : $action->{'field2'};
        $field3 =
          defined( $alert->{'field3'} )
          && $alert->{'field3'} ne ""
          ? $alert->{'field3'}
          : $action->{'field3'};
        $field4 =
          defined( $alert->{'field4'} )
          && $alert->{'field4'} ne ""
          ? $alert->{'field4'}
          : $action->{'field4'};
        $field5 =
          defined( $alert->{'field5'} )
          && $alert->{'field5'} ne ""
          ? $alert->{'field5'}
          : $action->{'field5'};
        $field6 =
          defined( $alert->{'field6'} )
          && $alert->{'field6'} ne ""
          ? $alert->{'field6'}
          : $action->{'field6'};
        $field7 =
          defined( $alert->{'field7'} )
          && $alert->{'field7'} ne ""
          ? $alert->{'field7'}
          : $action->{'field7'};
        $field8 =
          defined( $alert->{'field8'} )
          && $alert->{'field8'} ne ""
          ? $alert->{'field8'}
          : $action->{'field8'};
        $field9 =
          defined( $alert->{'field9'} )
          && $alert->{'field9'} ne ""
          ? $alert->{'field9'}
          : $action->{'field9'};
        $field10 =
          defined( $alert->{'field10'} )
          && $alert->{'field10'} ne ""
          ? $alert->{'field10'}
          : $action->{'field10'};
        $field11 =
          defined( $alert->{'field11'} )
          && $alert->{'field11'} ne ""
          ? $alert->{'field11'}
          : $action->{'field11'};
        $field12 =
          defined( $alert->{'field12'} )
          && $alert->{'field12'} ne ""
          ? $alert->{'field12'}
          : $action->{'field12'};
        $field13 =
          defined( $alert->{'field13'} )
          && $alert->{'field13'} ne ""
          ? $alert->{'field13'}
          : $action->{'field13'};
        $field14 =
          defined( $alert->{'field14'} )
          && $alert->{'field14'} ne ""
          ? $alert->{'field14'}
          : $action->{'field14'};
        $field15 =
          defined( $alert->{'field15'} )
          && $alert->{'field15'} ne ""
          ? $alert->{'field15'}
          : $action->{'field15'};
        $field16 =
          defined( $alert->{'field16'} )
          && $alert->{'field16'} ne ""
          ? $alert->{'field16'}
          : $action->{'field16'};
        $field17 =
          defined( $alert->{'field17'} )
          && $alert->{'field17'} ne ""
          ? $alert->{'field17'}
          : $action->{'field17'};
        $field18 =
          defined( $alert->{'field18'} )
          && $alert->{'field18'} ne ""
          ? $alert->{'field18'}
          : $action->{'field18'};
        $field19 =
          defined( $alert->{'field19'} )
          && $alert->{'field19'} ne ""
          ? $alert->{'field19'}
          : $action->{'field19'};
        $field20 =
          defined( $alert->{'field20'} )
          && $alert->{'field20'} ne ""
          ? $alert->{'field20'}
          : $action->{'field20'};
    }
...
# Field 1 is a special case where [RECOVER] prefix is not added even when it is defined
        $field1 =
          defined( $alert->{'field1_recovery'} )
          && $alert->{'field1_recovery'} ne ""
          ? $alert->{'field1_recovery'}
          : $field1;
        $field1 =
          defined( $action->{'field1_recovery'} )
          && $action->{'field1_recovery'} ne ""
          ? $action->{'field1_recovery'}
          : $field1;

        $field2 =
          defined($field2) && $field2 ne "" ? "[RECOVER]" . $field2 : "";
        $field2 =
          defined( $alert->{'field2_recovery'} )
          && $alert->{'field2_recovery'} ne ""
          ? $alert->{'field2_recovery'}
          : $field2;
        $field2 =
          defined( $action->{'field2_recovery'} )
          && $action->{'field2_recovery'} ne ""
          ? $action->{'field2_recovery'}
          : $field2;

        $field3 =
          defined($field3) && $field3 ne "" ? "[RECOVER]" . $field3 : "";
        $field3 =
          defined( $alert->{'field3_recovery'} )
          && $alert->{'field3_recovery'} ne ""
          ? $alert->{'field3_recovery'}
          : $field3;
        $field3 =
          defined( $action->{'field3_recovery'} )
          && $action->{'field3_recovery'} ne ""
          ? $action->{'field3_recovery'}
          : $field3;

        $field4 =
          defined($field4) && $field4 ne "" ? "[RECOVER]" . $field4 : "";
        $field4 =
          defined( $alert->{'field4_recovery'} )
          && $alert->{'field4_recovery'} ne ""
          ? $alert->{'field4_recovery'}
          : $field4;
        $field4 =
          defined( $action->{'field4_recovery'} )
          && $action->{'field4_recovery'} ne ""
          ? $action->{'field4_recovery'}
          : $field4;

        $field5 =
          defined($field5) && $field5 ne "" ? "[RECOVER]" . $field5 : "";
        $field5 =
          defined( $alert->{'field5_recovery'} )
          && $alert->{'field5_recovery'} ne ""
          ? $alert->{'field5_recovery'}
          : $field5;
        $field5 =
          defined( $action->{'field5_recovery'} )
          && $action->{'field5_recovery'} ne ""
          ? $action->{'field5_recovery'}
          : $field5;

        $field6 =
          defined($field6) && $field6 ne "" ? "[RECOVER]" . $field6 : "";
        $field6 =
          defined( $alert->{'field6_recovery'} )
          && $alert->{'field6_recovery'} ne ""
          ? $alert->{'field6_recovery'}
          : $field6;
        $field6 =
          defined( $action->{'field6_recovery'} )
          && $action->{'field6_recovery'} ne ""
          ? $action->{'field6_recovery'}
          : $field6;

        $field7 =
          defined($field7) && $field7 ne "" ? "[RECOVER]" . $field7 : "";
        $field7 =
          defined( $alert->{'field7_recovery'} )
          && $alert->{'field7_recovery'} ne ""
          ? $alert->{'field7_recovery'}
          : $field7;
        $field7 =
          defined( $action->{'field7_recovery'} )
          && $action->{'field7_recovery'} ne ""
          ? $action->{'field7_recovery'}
          : $field7;

        $field8 =
          defined($field8) && $field8 ne "" ? "[RECOVER]" . $field8 : "";
        $field8 =
          defined( $alert->{'field8_recovery'} )
          && $alert->{'field8_recovery'} ne ""
          ? $alert->{'field8_recovery'}
          : $field8;
        $field8 =
          defined( $action->{'field8_recovery'} )
          && $action->{'field8_recovery'} ne ""
          ? $action->{'field8_recovery'}
          : $field8;

        $field9 =
          defined($field9) && $field9 ne "" ? "[RECOVER]" . $field9 : "";
        $field9 =
          defined( $alert->{'field9_recovery'} )
          && $alert->{'field9_recovery'} ne ""
          ? $alert->{'field9_recovery'}
          : $field9;
        $field9 =
          defined( $action->{'field9_recovery'} )
          && $action->{'field9_recovery'} ne ""
          ? $action->{'field9_recovery'}
          : $field9;

        $field10 =
          defined($field10) && $field10 ne "" ? "[RECOVER]" . $field10 : "";
        $field10 =
          defined( $alert->{'field10_recovery'} )
          && $alert->{'field10_recovery'} ne ""
          ? $alert->{'field10_recovery'}
          : $field10;
        $field10 =
          defined( $action->{'field10_recovery'} )
          && $action->{'field10_recovery'} ne ""
          ? $action->{'field10_recovery'}
          : $field10;

        $field11 =
          defined($field11) && $field11 ne "" ? "[RECOVER]" . $field11 : "";
        $field11 =
          defined( $alert->{'field11_recovery'} )
          && $alert->{'field11_recovery'} ne ""
          ? $alert->{'field11_recovery'}
          : $field11;
        $field11 =
          defined( $action->{'field11_recovery'} )
          && $action->{'field11_recovery'} ne ""
          ? $action->{'field11_recovery'}
          : $field11;

        $field12 =
          defined($field12) && $field12 ne "" ? "[RECOVER]" . $field12 : "";
        $field12 =
          defined( $alert->{'field12_recovery'} )
          && $alert->{'field12_recovery'} ne ""
          ? $alert->{'field12_recovery'}
          : $field12;
        $field12 =
          defined( $action->{'field12_recovery'} )
          && $action->{'field12_recovery'} ne ""
          ? $action->{'field12_recovery'}
          : $field12;

        $field13 =
          defined($field13) && $field13 ne "" ? "[RECOVER]" . $field13 : "";
        $field13 =
          defined( $alert->{'field13_recovery'} )
          && $alert->{'field13_recovery'} ne ""
          ? $alert->{'field13_recovery'}
          : $field13;
        $field13 =
          defined( $action->{'field13_recovery'} )
          && $action->{'field13_recovery'} ne ""
          ? $action->{'field13_recovery'}
          : $field13;

        $field14 =
          defined($field14) && $field14 ne "" ? "[RECOVER]" . $field14 : "";
        $field14 =
          defined( $alert->{'field14_recovery'} )
          && $alert->{'field14_recovery'} ne ""
          ? $alert->{'field14_recovery'}
          : $field14;
        $field14 =
          defined( $action->{'field14_recovery'} )
          && $action->{'field14_recovery'} ne ""
          ? $action->{'field14_recovery'}
          : $field14;

        $field15 =
          defined($field15) && $field15 ne "" ? "[RECOVER]" . $field15 : "";
        $field15 =
          defined( $alert->{'field15_recovery'} )
          && $alert->{'field15_recovery'} ne ""
          ? $alert->{'field15_recovery'}
          : $field15;
        $field15 =
          defined( $action->{'field15_recovery'} )
          && $action->{'field15_recovery'} ne ""
          ? $action->{'field15_recovery'}
          : $field15;

        $field16 =
          defined($field16) && $field16 ne "" ? "[RECOVER]" . $field16 : "";
        $field16 =
          defined( $alert->{'field16_recovery'} )
          && $alert->{'field16_recovery'} ne ""
          ? $alert->{'field16_recovery'}
          : $field16;
        $field16 =
          defined( $action->{'field16_recovery'} )
          && $action->{'field16_recovery'} ne ""
          ? $action->{'field16_recovery'}
          : $field16;

        $field17 =
          defined($field17) && $field17 ne "" ? "[RECOVER]" . $field17 : "";
        $field17 =
          defined( $alert->{'field17_recovery'} )
          && $alert->{'field17_recovery'} ne ""
          ? $alert->{'field17_recovery'}
          : $field17;
        $field17 =
          defined( $action->{'field17_recovery'} )
          && $action->{'field17_recovery'} ne ""
          ? $action->{'field17_recovery'}
          : $field17;

        $field18 =
          defined($field18) && $field18 ne "" ? "[RECOVER]" . $field18 : "";
        $field18 =
          defined( $alert->{'field18_recovery'} )
          && $alert->{'field18_recovery'} ne ""
          ? $alert->{'field18_recovery'}
          : $field18;
        $field18 =
          defined( $action->{'field18_recovery'} )
          && $action->{'field18_recovery'} ne ""
          ? $action->{'field18_recovery'}
          : $field18;

        $field19 =
          defined($field19) && $field19 ne "" ? "[RECOVER]" . $field19 : "";
        $field19 =
          defined( $alert->{'field19_recovery'} )
          && $alert->{'field19_recovery'} ne ""
          ? $alert->{'field19_recovery'}
          : $field19;
        $field19 =
          defined( $action->{'field19_recovery'} )
          && $action->{'field19_recovery'} ne ""
          ? $action->{'field19_recovery'}
          : $field19;

        $field20 =
          defined($field20) && $field20 ne "" ? "[RECOVER]" . $field20 : "";
        $field20 =
          defined( $alert->{'field20_recovery'} )
          && $alert->{'field20_recovery'} ne ""
          ? $alert->{'field20_recovery'}
          : $field20;
        $field20 =
          defined( $action->{'field20_recovery'} )
          && $action->{'field20_recovery'} ne ""
          ? $action->{'field20_recovery'}
          : $field20;
    }
    ...
    $field10 =
      defined($field10) && $field10 ne "" ? decode_entities($field10) : "";
    $field11 =
      defined($field11) && $field11 ne "" ? decode_entities($field11) : "";
    $field12 =
      defined($field12) && $field12 ne "" ? decode_entities($field12) : "";
    $field13 =
      defined($field13) && $field13 ne "" ? decode_entities($field13) : "";
    $field14 =
      defined($field14) && $field14 ne "" ? decode_entities($field14) : "";
    $field15 =
      defined($field15) && $field15 ne "" ? decode_entities($field15) : "";
    $field16 =
      defined($field16) && $field16 ne "" ? decode_entities($field16) : "";
    $field17 =
      defined($field17) && $field17 ne "" ? decode_entities($field17) : "";
    $field18 =
      defined($field18) && $field18 ne "" ? decode_entities($field18) : "";
    $field19 =
      defined($field19) && $field19 ne "" ? decode_entities($field19) : "";
    $field20 =
      defined($field20) && $field20 ne "" ? decode_entities($field20) : "";

    # Get group info
    my $group = undef;
    if ( defined($agent) ) {
        $group =
          get_db_single_row( $dbh, 'SELECT * FROM tgrupo WHERE id_grupo = ?',
            $agent->{'id_grupo'} );
    }

    my $time_down;
    if ( $alert_mode == RECOVERED_ALERT
        && defined( $extra_macros->{'_modulelaststatuschange_'} ) )
    {
        ...
        if ( ref($module) eq "HASH" ) {
            $agent_status = get_db_single_row(
                $dbh,
        ...
        $time_down =
            ( defined($agent_status) )
          ? ( time() - $agent_status->{'last_status_change'} )
          : undef;
    }
...
# Alert macros
    my %macros = (
        ...
        _agent_      => ( defined($agent) )
        ? ( $agent->{'alias'} ? $agent->{'alias'} : $agent->{'nombre'} )
        : '',
        _agentcustomid_ => ( defined($agent) ) ? $agent->{'custom_id'} : '',
        '_agentcustomfield_\d+_' => undef,
        _agentdescription_       => ( defined($agent) )
        ? $agent->{'comentarios'}
        : '',
        _agentgroup_  => ( defined($group) ) ? $group->{'nombre'} : '',
        _agentstatus_ => undef,
        _agentos_     => ( defined($agent) )
        ? get_os_name( $dbh, $agent->{'id_os'} )
        : '',
        _address_   => ( defined($agent) ) ? $agent->{'direccion'} : '',
        _timestamp_ => ( defined($timestamp) )
        ? $timestamp
        ...
        _modulegroup_       => undef,
        _moduledescription_ => ( defined($module) )
        ? $module->{'descripcion'}
        : '',
        ...
        _id_module_ => ( defined($module) )
        ? $module->{'id_agente_modulo'}
        : '',
        _id_group_ => ( defined($group) ) ? $group->{'id_grupo'} : '',
        _id_alert_ => ( defined( $alert->{'id_template_module'} ) )
        ? $alert->{'id_template_module'}
        : '',
        _interval_ => ( defined($module) && $module->{'module_interval'} != 0 )
        ? $module->{'module_interval'}
        : ( defined($agent) ) ? $agent->{'intervalo'}
        : '',
        _server_ip_ => ( defined($agent) )
        ? get_db_value(
            $dbh,
            "SELECT ip_address FROM tserver WHERE name = ?",
            $agent->{'server_name'}
          )
        : '',
        ...
        _plugin_parameters_ => ( defined($module) )
        ...
        _warning_threshold_min_ => ( defined( $module->{'min_warning'} ) )
        ? $module->{'min_warning'}
        : '',
        _warning_threshold_max_ => ( defined( $module->{'max_warning'} ) )
        ? $module->{'max_warning'}
        : '',
        _critical_threshold_min_ => ( defined( $module->{'min_critical'} ) )
        ? $module->{'min_critical'}
        : '',
        _critical_threshold_max_ => ( defined( $module->{'max_critical'} ) )
        ? $module->{'max_critical'}
        : '',
    );
    ...
    if ( $action->{'internal'} == 0 ) {
        $macros{_field1_} =
          subst_alert_macros( $field1, \%macros, $pa_config, $dbh, $agent,
            $module, $alert );
        $macros{_field2_} =
          subst_alert_macros( $field2, \%macros, $pa_config, $dbh, $agent,
            $module, $alert );
        $macros{_field3_} =
          subst_alert_macros( $field3, \%macros, $pa_config, $dbh, $agent,
            $module, $alert );
        $macros{_field4_} =
          subst_alert_macros( $field4, \%macros, $pa_config, $dbh, $agent,
            $module, $alert );
        $macros{_field5_} =
          subst_alert_macros( $field5, \%macros, $pa_config, $dbh, $agent,
            $module, $alert );
        $macros{_field6_} =
          subst_alert_macros( $field6, \%macros, $pa_config, $dbh, $agent,
            $module, $alert );
        $macros{_field7_} =
          subst_alert_macros( $field7, \%macros, $pa_config, $dbh, $agent,
            $module, $alert );
        $macros{_field8_} =
          subst_alert_macros( $field8, \%macros, $pa_config, $dbh, $agent,
            $module, $alert );
        $macros{_field9_} =
          subst_alert_macros( $field9, \%macros, $pa_config, $dbh, $agent,
            $module, $alert );
        $macros{_field10_} =
          subst_alert_macros( $field10, \%macros, $pa_config, $dbh, $agent,
            $module, $alert );
        $macros{_field11_} =
          subst_alert_macros( $field11, \%macros, $pa_config, $dbh, $agent,
            $module, $alert );
        $macros{_field12_} =
          subst_alert_macros( $field12, \%macros, $pa_config, $dbh, $agent,
            $module, $alert );
        $macros{_field13_} =
          subst_alert_macros( $field13, \%macros, $pa_config, $dbh, $agent,
            $module, $alert );
        $macros{_field14_} =
          subst_alert_macros( $field14, \%macros, $pa_config, $dbh, $agent,
            $module, $alert );
        $macros{_field15_} =
          subst_alert_macros( $field15, \%macros, $pa_config, $dbh, $agent,
            $module, $alert );
        $macros{_field16_} =
          subst_alert_macros( $field16, \%macros, $pa_config, $dbh, $agent,
            $module, $alert );
        $macros{_field17_} =
          subst_alert_macros( $field17, \%macros, $pa_config, $dbh, $agent,
            $module, $alert );
        $macros{_field18_} =
          subst_alert_macros( $field18, \%macros, $pa_config, $dbh, $agent,
            $module, $alert );
        $macros{_field19_} =
          subst_alert_macros( $field19, \%macros, $pa_config, $dbh, $agent,
            $module, $alert );
        $macros{_field20_} =
          subst_alert_macros( $field20, \%macros, $pa_config, $dbh, $agent,
            $module, $alert );
        ...
        foreach my $word (
            quotewords( '\s+', 1, ( decode_entities( $action->{'command'} ) ) )
          )
        {
            push @command_args,
              subst_alert_macros( $word, \%macros, $pa_config, $dbh, $agent,
                $module );
        }
        my $command = join( ' ', @command_args );
        logger(
            $pa_config,
            "Executing command '$command' for action '"
              . safe_output( $action->{'name'} )
              . "' alert '"
              . safe_output( $alert->{'name'} )
              . "' agent '"
              . ( defined($agent) ? safe_output( $agent->{'alias'} ) : 'N/A' )
              . "'.",
            8
        );
            ...
            if ( $pa_config->{'global_alert_timeout'} == 0 ) {
                ...
                logger(
                    ...
                    "Command '$command' for action '"
                      ...
                      . (
                        defined($agent)
                        ? safe_output( $agent->{'alias'} )
                        : 'N/A'
                      )
                      . "' returned with errorlevel "
                      . ( $? >> 8 ),
                    8
                );
            }
                ...
                my $command_timeout =
                    safe_output( $pa_config->{'plugin_exec'} ) . " "
                  . $pa_config->{'global_alert_timeout'} . " "
                  . $command;
                system($command_timeout);
                my $return_code = ( $? >> 8 ) & 0xff;
                logger(
                    $pa_config,
                    "Command '$command_timeout' for action '"
                      . safe_output( $action->{'name'} )
                      . "' alert '"
                      . safe_output( $alert->{'name'} )
                      . "' agent '"
                ...
                if ( $return_code != 0 ) {
                    logger(
                        ...
                        "Action '"
                          ...
                          . "' exceeded the global alert timeout "
                          . $pa_config->{'global_alert_timeout'}
                          . " seconds",
                        3
                    );
        ...
        if ($@) {
            logger(
                $pa_config,
                "Error $@ executing command '$command' for action '"
                  . safe_output( $action->{'name'} )
                  . "' alert '"
                  . safe_output( $alert->{'name'} )
                  . "' agent '"
        ...
        pandora_audit(
            ...
            $field1,
            defined($agent) ? safe_output( $agent->{'alias'} ) : 'N/A',
            'Alert (' . safe_output( $alert->{'description'} ) . ')',
            $dbh
        );
        ...
        if ( defined($data) && $data =~ /^data:image\/png;base64, / ) {
            ...
            my $_cid =
              '<img style="height: 150px;" src="cid:' . $cid_data . '"/>';
...
# Simple email address validation. Prevents connections to the SMTP server when no address is provided.
        if ( index( $field1, '@' ) == -1 ) {
            logger(
                $pa_config,
                "No valid email address provided for action '"
                  . $action->{'name'}
                  . "' alert '"
                  . $alert->{'name'}
                  . "' agent '"
                  . ( defined($agent) ? $agent->{'alias'} : 'N/A' ) . "'.",
                10
            );
            return;
        }
        ...
        if ($@) {
            logger( $pa_config,
                "Failed to limit ssl security on console link: " . $@, 10 );
        }
        ...
        my $subst_func = sub {
            ...
            if ( defined($module) ) {
                $params->{"id"} =
                  get_agent_module_id( $dbh, $module, $agent->{'id_agente'} );
            }
        ...
        if ( $attach_data_as_image == 1 ) {
            ...
            my $base64_data = substr( $data, 23 )
              ;    # remove first 23 characters: 'data:image/png;base64, '
        ...
        if ( $field3 =~ /cid:moduledata_/ ) {
            ...
            foreach my $module_id (@matches) {
                ...
                my $module_data = get_db_value(
                    $dbh,
'SELECT datos FROM tagente_estado WHERE id_agente_modulo = ?',
                    $module_id
                );
        ...
        $agent_name =
          subst_alert_macros( $agent_name, \%macros, $pa_config, $dbh, $agent,
            $module );
            ...
            && ( $field1 ne "" || $field2 ne "" ) )
        {
            ...
            send_console_notification( $pa_config, $dbh, $field3, $field4,
    ...
    if (   $alert_mode != RECOVERED_ALERT
        && defined( $action->{'last_execution'} )
        && defined( $action->{'id_alert_templ_module_actions'} ) )
    {
        db_do(
            $dbh, 'UPDATE talert_template_module_actions SET last_execution = ?
...
sub send_console_notification {
    ...
    # Create message
    my $notification_id =
      db_process_insert( $dbh, 'id_mensaje', 'tmensajes', $notification );
    if ( !$notification_id ) {
        logger( $pa_config, "Cannot send notification '" . $subject . "'", 3 );
    }
    else {
        notification_set_targets( $pa_config, $dbh, $notification_id,
            $user_list, $group_list );
    }
}
...
sub pandora_process_module {
    my (
        $pa_config,   $data_object, $agent,      $module,
        $module_type, $timestamp,   $utimestamp, $server_id,
        $dbh,         $extra_macros
    ) = @_;
    ...
    $module->{'module_interval'} = 0
      unless defined( $module->{'module_interval'} );

    if ( ref($agent) eq 'HASH' ) {
        if (  !defined( $agent->{'interval'} )
            && defined( $agent->{'interval'} ) )
        {
    ...
    if ( !defined($agent) || $agent eq '' ) {
        ...
        if ( !defined($agent) ) {
            logger(
                ...
                "Agent ID "
                  . $module->{'id_agente'}
                  . " not found while processing module '"
                  . safe_output( $module->{'nombre'} ) . "'.",
                3
            );
    ...
    # Process data
    my $processed_data =
      process_data( $pa_config, $data_object, $agent, $module, $module_type,
        $utimestamp, $dbh );
    ...
    # Export data
    export_module_data( $pa_config, $processed_data, $agent, $module,
        $module_type, $timestamp, $dbh );

    # Get previous status
    my $agent_status = get_db_single_row(
        $dbh,
        'SELECT * FROM tagente_estado WHERE id_agente_modulo = ?',
        $module->{'id_agente_modulo'}
    );
...
# tagente_estado.last_try defaults to NULL, should default to '1970-01-01 00:00:00'
    $agent_status->{'last_try'} = '1970-01-01 00:00:00'
      unless defined( $agent_status->{'last_try'} );
    $agent_status->{'datos'} = "" unless defined( $agent_status->{'datos'} );

    # Do we have to save module data?
    if ( $agent_status->{'last_try'} !~
        /(\d+)\-(\d+)\-(\d+) +(\d+):(\d+):(\d+)/ )
    {
        logger(
            $pa_config,
            "Invalid last try timestamp '"
              . $agent_status->{'last_try'}
              . "' for agent '"
              . $agent->{'nombre'}
              . "' not found while processing module '"
              . $module->{'nombre'} . "'.",
    ...
    my $last_try =
      ( $1 == 0 ) ? 0 : strftime( "%s", $6, $5, $4, $3, $2 - 1, $1 - 1900 );

    my $save = (
        $module->{'history_data'} == 1
          && ( $agent_status->{'datos'} ne $processed_data
            || $last_try < ( $utimestamp - 86400 ) )
    ) ? 1 : 0;

    # Received stale data. Save module data if needed and return.
    if (   $pa_config->{'dataserver_lifo'} == 1
        && $utimestamp <= $agent_status->{'utimestamp'} )
    {
        logger(
            $pa_config,
            "Received stale data from agent "
              . (
                defined($agent)
                ? "'" . $agent->{'nombre'} . "'"
                : 'ID ' . $module->{'id_agente'}
        ...
        if ( $module->{'history_data'} == 1 ) {
            save_module_data( $data_object, $module, $module_type, $utimestamp,
                $dbh );
        }
    ...
    $new_status = escalate_warning(
        $pa_config,    $agent,      $module,
        $agent_status, $new_status, $known_status
    );
...
# Set the last status change macro. Even if its value changes later, whe want the original value.
    ...
    my $current_interval;
    if (   defined( $module->{'cron_interval'} )
        && $module->{'cron_interval'} ne ''
        && $module->{'cron_interval'} ne '* * * * *' )
    {
    ...
    if ( $module->{'each_ff'} ) {
        $min_ff_event = $module->{'min_ff_event_normal'}
          if ( $new_status == 0 );
        $min_ff_event = $module->{'min_ff_event_critical'}
          if ( $new_status == 1 );
        $min_ff_event = $module->{'min_ff_event_warning'}
          if ( $new_status == 2 );
    }
    ...
    $module->{'module_ff_interval'} = 0
      unless defined( $module->{'module_ff_interval'} );

    if ( $last_known_status == $new_status ) {

        # Avoid overflows
        $status_changes = $min_ff_event
          if ( $status_changes > $min_ff_event && $module->{'ff_type'} == 0 );

        $status_changes++;
        if (   $module_type =~ m/async/
            && $min_ff_event != 0
            && $ff_timeout != 0
            && ( $utimestamp - $ff_start_utimestamp ) > $ff_timeout )
        {
    ...
    if ( $module->{'ff_type'} == 0 ) {
        ...
        {
            generate_status_event( $pa_config, $processed_data, $agent,
                $module, $new_status, $status, $known_status, $dbh );
            $status = $new_status;
            ...
            {
                safe_mode( $pa_config, $agent, $module, $new_status,
                    $known_status, $dbh );
            }
        }
        elsif ($status_changes >= $min_ff_event
            && $known_status == $new_status
            && $new_status == 1 )
        {
            # Safe mode execution.
            if ( $agent->{'safe_mode_module'} == $module->{'id_agente_modulo'} )
 ...
 # Normal.
        if (
            (
                $new_status != $status
                && ( $new_status == 0 && $ff_normal > $min_ff_event )
            )
...
# if critical or warning status, just pass through here and wait the time min_ff_event will be matched.
    if ( $status == 4 ) {
        generate_status_event( $pa_config, $processed_data, $agent, $module, 0,
            $status, $known_status, $dbh );
        $status = 0;
...
# If unknown modules receive data, restore status even if min_ff_event is not matched.
        ...
        generate_status_event( $pa_config, $processed_data, $agent, $module,
            $known_status, $status, $known_status, $dbh );
        $status = $known_status;
    ...
    if ( $utimestamp >= $last_try ) {
        db_do(
			...
			WHERE id_agente_modulo = ?', $processed_data, $status, $status, $new_status,
            $new_status,         $status_changes,
            $current_utimestamp, $timestamp, $module->{'id_agente'},
            $current_interval,   $server_id,
            $utimestamp,
            ( $save == 1 ) ? $timestamp : $agent_status->{'last_try'},
            $last_error, $ff_start_utimestamp,
            $ff_normal,  $ff_warning, $ff_critical, $last_status_change,
            $agent_status->{'warning_count'}, $module->{'id_agente_modulo'}
        );
    }
        ...
        && pandora_cps_enabled( $agent, $module ) == 0 )
    {
        pandora_generate_alerts(
            ...
            $module,       $utimestamp,     $dbh,    $timestamp,
            $extra_macros, $last_data_value
        );
    }
    else {
        logger( $pa_config,
            "Alerts inhibited for agent '" . $agent->{'nombre'} . "'.", 10 );
    }
...
sub pandora_planned_downtime_cron_start {
    ...
    foreach my $downtime (@downtimes) {
        my $start_downtime =
          PandoraOpen::Tools::cron_check( $downtime->{'cron_interval_from'},
            $utimestamp );
        ...
        if ($start_downtime) {
            ...
            logger( $pa_config,
                "Starting planned downtime '" . $downtime->{'name'} . "'.",
                10 );

            db_do(
                $dbh, 'UPDATE tplanned_downtime
				SET executed = 1
            ...
            pandora_event(
                ...
                "Server "
                  ...
                  . " started planned downtime: "
                  . safe_output( $downtime->{'name'} ),
                0,
                0,
                1,
                0,
...
sub pandora_planned_downtime_cron_stop {
    ...
    foreach my $downtime (@downtimes) {
        my $stop_downtime =
          PandoraOpen::Tools::cron_check( $downtime->{'cron_interval_to'},
            $utimestamp );
        ...
        if ($stop_downtime) {
            ...
            logger(
                $pa_config,
                "Stopping planned cron downtime '" . $downtime->{'name'} . "'.",
                10
            );
...
sub pandora_planned_downtime_disabled_once_stop {
    ...
    foreach my $downtime (@downtimes) {
        ...
        logger( $pa_config,
            "Ending planned downtime '" . $downtime->{'name'} . "'.", 10 );
...
'UPDATE tagente_modulo tam, tagente ta, tplanned_downtime_modules tpdm
            ...
            db_do(
                $dbh, 'UPDATE tplanned_downtime_agents tp, tagente ta
				SET tp.manually_disabled = ta.disabled
				WHERE tp.id_agent = ta.id_agente AND tp.id_downtime = ?', $downtime->{'id'}
            );

            db_do(
                $dbh, 'UPDATE tagente ta, tplanned_downtime_agents tpa
        ...
        my @downtime_agents = get_db_rows(
            $dbh, 'SELECT *
        ...
        foreach my $downtime_agent (@downtime_agents) {
            db_do(
                $dbh, 'UPDATE talert_template_modules tat, tagente_modulo tam
...
sub pandora_planned_downtime_set_quiet_elements {
    ...
    foreach my $downtime_agent (@downtime_agents) {
        my @downtime_modules = get_db_rows(
            $dbh, 'SELECT *
...
sub pandora_planned_downtime_monthly_start {
    ...
    my $time = sprintf( "%02d:%02d:%02d",
        $var_localtime[2], $var_localtime[1], $var_localtime[0] );
    ...
    my @downtimes = get_db_rows(
		...
		WHERE type_periodicity = ?
			AND executed = 0
			AND type_execution <> '
          . $RDBMS_QUOTE_STRING . 'once' . $RDBMS_QUOTE_STRING . '
			AND type_execution <> '
          . $RDBMS_QUOTE_STRING . 'cron' . $RDBMS_QUOTE_STRING . '
    ...
    foreach my $downtime (@downtimes) {
        ...
        logger(
            $pa_config,
            "Starting planned monthly downtime '" . $downtime->{'name'} . "'.",
            10
        );
        ...
        if ( $downtime->{'type_downtime'} eq "quiet" ) {
            pandora_planned_downtime_set_quiet_elements( $pa_config, $dbh,
                $downtime->{'id'} );
        }
        elsif (( $downtime->{'type_downtime'} eq "disable_agents" )
            || ( $downtime->{'type_downtime'} eq "disable_agents_alerts" )
            || ( $downtime->{'type_downtime'} eq "disable_agent_modules" ) )
        {
            ...
            pandora_planned_downtime_set_disabled_elements( $pa_config, $dbh,
                $downtime );
        }
    }
}
...
sub pandora_planned_downtime_monthly_stop {
    ...
    foreach my $downtime (@downtimes) {
        ...
        logger(
            $pa_config,
            "Stopping planned monthly downtime '" . $downtime->{'name'} . "'.",
            10
        );
...
sub pandora_planned_downtime_weekly_start {
    ...
    foreach my $downtime (@downtimes) {
        my $across_date = $downtime->{'periodically_time_from'}
          gt $downtime->{'periodically_time_to'} ? 1 : 0;
        $found = 0;
        ...
        if ($start_downtime) {
            ...
            logger(
                $pa_config,
                "Starting planned weekly downtime '"
                  . $downtime->{'name'} . "'.",
                10
            );
...
sub pandora_planned_downtime_weekly_stop {
    ...
    foreach my $downtime (@downtimes) {
        ...
        if ($stop_downtime) {
            ...
            logger(
                $pa_config,
                "Stopping planned weekly downtime '"
                  . $downtime->{'name'} . "'.",
                10
            );
...
sub pandora_update_server {
    my (
        $pa_config, $dbh,         $server_name, $server_id,
        $status,    $server_type, $num_threads, $queue_size,
        $version,   $keepalive,   $disabled,    $remote_config
    ) = @_;
    ...
    $version = $pa_config->{'version'} unless defined($version);

    my $master =
      ( $server_type == SATELLITESERVER ) ? 0 : $pa_config->{'pandora_master'};

    my ( $year, $month, $day, $hour, $minute, $second ) = split /[- :]/,
      $timestamp;

    my $keepalive_utimestamp =
    ...
    if ( $server_id == 0 ) {
        ...
        my $server = get_db_single_row(
            $dbh,
'SELECT id_server FROM tserver WHERE BINARY name = ? AND server_type = ?',
            $server_name,
            $server_type
        );
        if ( !defined($server) ) {
            $server_id = db_insert(
                $dbh, 'id_server',
'INSERT INTO tserver (name, server_type, description, version, threads, queued_modules, server_keepalive, server_keepalive_utimestamp, disabled)
						VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', $server_name, $server_type,
                'Autocreated at startup', $version, $num_threads, $queue_size,
                $keepalive, $keepalive_utimestamp, $disabled
            );

            $server = get_db_single_row( $dbh,
                'SELECT status FROM tserver WHERE id_server = ?', $server_id );
            if ( !defined($server) ) {
                logger( $pa_config,
                    "Server '" . $pa_config->{'servername'} . "' not found.",
                    3 );
                return;
            }
        }
            ...
            if ( !$remote_config ) {
                db_do( $dbh,
                    'UPDATE tserver SET disabled = ? WHERE id_server = ?',
                    $disabled, $server_id );
            }
        }
        ...
        db_do(
            $dbh,
'UPDATE tserver SET status = ?, keepalive = ?, master = ?, laststart = ?, version = ?, threads = ?, queued_modules = ?, server_keepalive = ?, server_keepalive_utimestamp = ?
				WHERE id_server = ?',
            1, $timestamp, $master, $timestamp, $version, $num_threads,
            $queue_size, $keepalive, $keepalive_utimestamp, $server_id
        );
        return;
    }
    ...
    db_do(
        $dbh,
'UPDATE tserver SET status = ?, keepalive = ?, master = ?, version = ?, threads = ?, queued_modules = ?, server_keepalive = ?, server_keepalive_utimestamp = ?
...
sub pandora_update_agent {
    my (
        $pa_config,  $agent_timestamp, $agent_id,
        $os_version, $agent_version,   $agent_interval,
        $dbh,        $timezone_offset, $parent_agent_id,
        $satellite_server_id
    ) = @_;

# No access update for data without interval.
# Single modules from network server, for example. This could be very Heavy for Pandora Open
    if ( $agent_interval == -1 ) {
    ...
    # Update tagente
    my $timestamp = strftime( "%Y-%m-%d %H:%M:%S", localtime() );
    my ( $set, $values ) = db_update_get_values(
        {
    ...
    db_do( $dbh, "UPDATE tagente SET $set WHERE id_agente = ?",
        @{$values}, $agent_id );
}
...
sub pandora_update_gis_data {
    my (
        $pa_config,  $dbh,                  $agent_id,
        $agent_name, $longitude,            $latitude,
        $altitude,   $position_description, $timestamp
    ) = @_;
    ...
    logger(
        $pa_config,
"Updating GIS data for agent $agent_name (long: $longitude lat: $latitude alt: $altitude)",
        10
    );
...
# Other methods for reverse geocoding are OpenStreetmaps, in nternet or in a local server
        ...
        if ( $pa_config->{'google_maps_description'} ) {
            my $content =
              get(  'http://maps.google.com/maps/geo?q='
                  . $latitude . ','
                  . $longitude
            ...
            my $content =
              get(  'http://nominatim.openstreetmap.org/reverse?format=csv&lat='
                  . $latitude . '&lon='
                  . $longitude
                  . '&zoom=18&addressdetails=1&email=user@domain.org' );
        ...
        logger(
            $pa_config,
"Getting GIS Data=longitude=$longitude latitude=$latitude altitude=$altitude position_description=$position_description",
            10
        );
    }

    # Get the last position to see if it has moved.
    my $last_agent_position =
      get_db_single_row( $dbh,
        'SELECT * FROM tgis_data_status WHERE tagente_id_agente = ?',
        $agent_id );
    if ( defined($last_agent_position) ) {

        logger(
            $pa_config,
            "Old Agent data: current_longitude="
              . $last_agent_position->{'current_longitude'}
              . " current_latitude="
              . $last_agent_position->{'current_latitude'}
              . " current_altitude="
        ...
        if (
            distance_moved(
                $pa_config,
                $last_agent_position->{'stored_longitude'},
                $last_agent_position->{'stored_latitude'},
                $last_agent_position->{'stored_altitude'},
                $longitude,
                $latitude,
                $altitude
          ...
          )
        {
            ...
            archive_agent_position(
                $pa_config,
                $last_agent_position->{'start_timestamp'},
                $timestamp,
                $last_agent_position->{'stored_longitude'},
                $last_agent_position->{'stored_latitude'},
                $last_agent_position->{'stored_altitude'},
                $last_agent_position->{'description'},
                $last_agent_position->{'number_of_packages'},
                $agent_id,
                $dbh
            );
            ...
            update_agent_position(
                $pa_config, $longitude, $latitude,
                $altitude,  $agent_id,  $dbh,
        ...
        logger(
            $pa_config,
"There was not previous positional data, storing first positioal status",
            10
        );
        save_agent_position( $pa_config, $longitude, $latitude, $altitude,
...
sub pandora_create_template_module {
    my ( $pa_config, $dbh, $id_agent_module, $id_alert_template,
        $id_policy_alerts, $disabled, $standby )
      = @_;
    ...
    return db_insert(
        ...
        "INSERT INTO talert_template_modules(id_agent_module,
		                                     ...
		                                     last_reference)
		VALUES (?, ?, ?, ?, ?, ?)",
        $id_agent_module, $id_alert_template, $id_policy_alerts, $disabled,
        $standby,         time
    );
}
...
sub pandora_update_template_module {
    my ( $pa_config, $dbh, $id_alert, $id_policy_alerts, $disabled, $standby )
      = @_;
...
sub pandora_create_template_module_action {
    ...
    logger(
        $pa_config,
"Creating module alert action to alert '$parameters->{'id_alert_template_module'}'.",
        10
    );

    my $action_id =
      db_process_insert( $dbh, 'id', 'talert_template_module_actions',
        $parameters );
...
sub pandora_delete_all_template_module_actions {
    ...
    return db_do(
        $dbh,
'DELETE FROM talert_template_module_actions WHERE id_alert_template_module = ?',
        $template_module_id
    );
}
...
sub pandora_create_alert_command {
    ...
    my $command_id =
      db_process_insert( $dbh, 'id', 'talert_commands', $parameters );
...
sub pandora_update_agent_address {
    ...
    logger( $pa_config,
        'Updating address for agent ' . $agent_name . ' (' . $address . ')',
        10 );
...
sub pandora_module_keep_alive {
    ...
    logger(
        $pa_config,
        "Updating keep_alive modules for agent '"
          . safe_output($agent_name) . "'.",
        10
    );

    # Update keepalive modules
    my @modules = get_db_rows(
        $dbh,
'SELECT * FROM tagente_modulo WHERE id_agente = ? AND delete_pending = 0 AND id_tipo_modulo = 100',
        $id_agent
    );
    my %data = ( 'data' => 1 );
    foreach my $module (@modules) {
        pandora_process_module(
            $pa_config,   \%data, '',     $module,
            'keep_alive', '',     time(), $server_id,
            $dbh
        );
    }
}
...
sub pandora_audit {
    ...
    logger( $pa_config,
        "Creating audit entry '$description' name '$name' action '$action'.",
        10 );
    ...
    db_do(
        $dbh,
'INSERT INTO tsesion (id_usuario, ip_origen, accion, fecha, descripcion, utimestamp) 
			VALUES (?, ?, ?, ?, ?, ?)', 'SYSTEM', $name, $action, $timestamp,
        $description, $utimestamp
    );
...
sub pandora_create_module {
    my (
        $pa_config, $agent_id, $module_type_id, $module_name,
        $max,       $min,      $post_process,   $description,
        $interval,  $dbh
    ) = @_;

    logger( $pa_config,
        "Creating module '$module_name' for agent ID $agent_id.", 10 );
...
'INSERT INTO tagente_modulo (id_agente, id_tipo_modulo, nombre, max, min, post_process, descripcion, module_interval, id_modulo, critical_instructions, warning_instructions, unknown_instructions, disabled_types_event, module_macros)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, \'\', \'\', \'\', \'\', \'\')',
        $agent_id,     $module_type_id, safe_input($module_name), $max, $min,
        $post_process, $description,    $interval
    );
    db_do(
        $dbh,
'INSERT INTO tagente_estado (id_agente_modulo, id_agente, estado, known_status, last_status, last_known_status, last_try, datos)
...
sub pandora_delete_module {
    my $dbh       = shift;
    my $module_id = shift;
    my $conf      = @_ ? shift : undef;
    my $cascade   = @_ ? shift : undef;

    # Recursively delete descendants (delete in cascade)
    if ( defined($cascade) && $cascade eq 1 ) {
        my @id_children_modules = get_db_rows(
            $dbh,
'SELECT id_agente_modulo FROM tagente_modulo WHERE parent_module_id = ?',
            $module_id
        );

        foreach my $id_child_module (@id_children_modules) {
            pandora_delete_module( $dbh, $id_child_module->{'id_agente_modulo'},
                $conf, 1 );
        }
    }

    # Get module data
    my $module = get_db_single_row(
        $dbh,
'SELECT * FROM tagente_modulo, tagente_estado WHERE tagente_modulo.id_agente_modulo = tagente_estado.id_agente_modulo AND tagente_modulo.id_agente_modulo=?',
        $module_id
    );
    return unless defined($module);

    # Delete Graphs, layouts & reports
    db_do( $dbh, 'DELETE FROM tgraph_source WHERE id_agent_module = ?',
        $module_id );
    db_do( $dbh, 'DELETE FROM tlayout_data WHERE id_agente_modulo = ?',
        $module_id );
    db_do( $dbh, 'DELETE FROM treport_content WHERE id_agent_module = ?',
        $module_id );

    # Delete the module state
    db_do( $dbh, 'DELETE FROM tagente_estado WHERE id_agente_modulo = ?',
        $module_id );

    # Delete templates asociated to the module
    db_do( $dbh,
        'DELETE FROM talert_template_modules WHERE id_agent_module = ?',
        $module_id );

    # Delete tags asociated to the module
    db_do( $dbh, 'DELETE FROM ttag_module WHERE id_agente_modulo = ?',
        $module_id );

    # Set pending delete the module
    db_do(
        $dbh,
'UPDATE tagente_modulo SET disabled = 1, delete_pending = 1, nombre = "delete_pending" WHERE id_agente_modulo = ?',
        $module_id
    );
...
# The modules are created enabled and with the flag activated to force first execution
    ...
    my $module_id =
      pandora_create_module_from_hash( $pa_config, $component, $dbh );
    ...
    logger(
        $pa_config,
        'Creating module '
          . safe_output( $component->{'nombre'} )
          . " (ID $module_id) for agent $addr from network component.",
        10
    );
...
sub pandora_create_module_from_hash {
    ...
    logger(
        $pa_config,
"Creating module '$parameters->{'nombre'}' for agent ID $parameters->{'id_agente'}.",
        10
    );
    ...
    if ( defined( $parameters->{'plugin_pass'} ) ) {
        $parameters->{'plugin_pass'} =
          pandora_input_password( $pa_config, $parameters->{'plugin_pass'} );
    }
    ...
    {
        $parameters->{'custom_string_2'} = pandora_input_password( $pa_config,
            $parameters->{'custom_string_2'} );
    }
    ...
    db_do(
        $dbh,
'INSERT INTO tagente_estado (id_agente_modulo, id_agente, estado, known_status, last_status, last_known_status, last_try, datos) VALUES (?, ?, ?, ?, ?, ?, \'1970-01-01 00:00:00\', \'\')',
        $module_id,
        $parameters->{'id_agente'},
        $status,
        $status,
        $status,
        $status
...
sub pandora_update_module_from_hash {
    ...
    my $module_id = db_process_update( $dbh, 'tagente_modulo', $parameters,
...
sub pandora_update_table_from_hash {
    my ( $pa_config, $parameters, $where_column, $where_value, $table, $dbh ) =
      @_;

    my $module_id = db_process_update( $dbh, $table, $parameters,
        { $where_column => $where_value } );
    return $module_id;
}
...
sub pandora_create_group {
    my (
        $name,      $icon,    $parent,      $propagate, $disabled,
        $custom_id, $id_skin, $description, $dbh
    ) = @_;

    my $group_id = db_insert(
        $dbh,
        'id_grupo',
'INSERT INTO tgrupo (nombre, icon, parent, propagate, disabled, custom_id, id_skin, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        safe_input($name),
        $icon,
        $parent,
        $propagate,
...
sub pandora_update_config_token {
    ...
    if ( $config_value ne '' ) {
        $result =
          db_update( $dbh, 'UPDATE tconfig SET value = ? WHERE token = ?',
            $value, $token );
    }
    else {
        $result =
...
sub pandora_select_id_custom_field {
    ...
    $result =
      get_db_single_row( $dbh,
        'SELECT id_field FROM tagent_custom_fields WHERE name = ? ',
        safe_input($field) );
...
sub pandora_select_combo_custom_field {
    ...
    $result =
      get_db_single_row( $dbh,
        'SELECT combo_values FROM tagent_custom_fields WHERE id_field = ? ',
        $field );
...
sub pandora_get_custom_fields {
    ...
    my @result = get_db_rows( $dbh,
        'select tagent_custom_fields.* FROM tagent_custom_fields' );
...
sub pandora_get_custom_field_for_itsm {
    ...
    my $agent_custom_field_data =
      pandora_get_agent_custom_field_data( $dbh, $id_agent );
    ...
    foreach my $custom_field ( @{$custom_fields} ) {
        if ( $agent_custom_field_data_reducer{ $custom_field->{'name'} } ) {
            ...
            $result{ safe_output( $custom_field->{'name'} ) } =
              pandora_check_type_custom_field_for_itsm($custom_field);
        }
    }
...
sub pandora_update_agent_custom_field {
    ...
    my $exist_field = get_db_value(
        $dbh,
'SELECT count(*) FROM tagent_custom_data WHERE id_field = ? AND id_agent = ?',
        $field,
        $id_agent
    );
    my $result = undef;
    ...
    if ( !$exist_field ) {
        $result = defined(
            db_insert(
                $dbh,
                'id_field',
'INSERT INTO tagent_custom_data (`description`, `id_field`, `id_agent`) VALUES (?, ?, ?)',
                $token,
                $field,
                $id_agent
            )
        ...
        $result = db_update(
            $dbh,
'UPDATE tagent_custom_data SET description = ? WHERE id_field = ? AND id_agent = ?',
            $token,
            $field,
            $id_agent
        );
...
sub pandora_get_config_value {
    ...
    my $config_value =
      get_db_value( $dbh, 'SELECT value FROM tconfig WHERE token = ?', $token );
...
sub pandora_get_credential {
    ...
    my $key = get_db_single_row( $dbh,
        'SELECT * FROM tcredential_store WHERE identifier = ?', $identifier );

    $key->{'username'} =
      pandora_output_password( $pa_config, safe_output( $key->{'username'} ) );
    $key->{'password'} =
      pandora_output_password( $pa_config, safe_output( $key->{'password'} ) );
    $key->{'extra_1'} =
      pandora_output_password( $pa_config, safe_output( $key->{'extra_1'} ) );
    $key->{'extra_2'} =
      pandora_output_password( $pa_config, safe_output( $key->{'extra_2'} ) );
...
sub pandora_create_agent {
    ...
    my (
        $pa_config, $server_name,     $agent_name,
        $address,   $group_id,        $parent_id,
        $os_id,     $description,     $interval,
        $dbh,       $timezone_offset, $longitude,
        $latitude,  $altitude,        $position_description,
        $custom_id, $url_address,     $agent_mode,
        $alias,     $event_id,        $os_version
    ) = @_;

    logger(
        $pa_config,
"Server '$server_name' creating agent '$agent_name' address '$address'.",
        10
    );
    ...
    my ( $columns, $values ) = db_insert_get_values(
        {
            'nombre'              => safe_input($agent_name),
            'direccion'           => $address,
    ...
    my $agent_id = db_insert( $dbh, 'id_agente', "INSERT INTO tagente $columns",
        @{$values} );

    # Save GIS data
    if (   defined($longitude)
        && defined($latitude)
        && $pa_config->{'activate_gis'} == 1 )
    {
    ...
    logger(
        $pa_config,
        "Server '$server_name' CREATED agent '$agent_name' address '$address'.",
        10
    );
    if ( !defined($event_id) ) {
        pandora_event( $pa_config,
            "Agent [" . safe_output($alias) . "] created by $server_name",
            $group_id, $agent_id, 2, 0, 0, 'new_agent', 0, $dbh );
    }
    else {
        pandora_extended_event( $pa_config, $dbh, $event_id,
                "Agent ["
              . safe_output($alias) . "][#"
              . $agent_id
              . "] created by $server_name" );
    }
    return $agent_id;
}
...
sub pandora_add_agent_address {
    ...
    if ( $addr_id <= 0 ) {
        logger( $pa_config, 'Adding address ' . $addr . ' to the address list',
            10 );
    ...
    if ( $addr_id <= 0 ) {
        logger( $pa_config,
            "Could not add address '$addr' for host '$agent_name'", 3 );
    }
...
sub pandora_delete_agent {
    ...
    my @modules =
      get_db_rows( $dbh, 'SELECT * FROM tagente_modulo WHERE id_agente = ?',
        $agent_id );
    ...
    if ( defined $conf ) {
        ...
        my $conf_fname =
            $conf->{incomingdir}
          . '/conf/'
          . md5( encode_utf8( safe_output($agent_name) ) ) . '.conf';
        unlink($conf_fname) if ( -f $conf_fname );

        my $md5_fname = $conf->{incomingdir} . '/md5/'
          . md5( encode_utf8( safe_output($agent_name) ) ) . '.md5';
...
sub pandora_event {
    my (
        $pa_config,             $evento,               $id_grupo,
        $id_agente,             $severity,             $id_alert_am,
        $id_agentmodule,        $event_type,           $event_status,
        $dbh,                   $source,               $user_name,
        $comment,               $id_extra,             $tags,
        $critical_instructions, $warning_instructions, $unknown_instructions,
        $custom_data,           $module_data,          $module_status,
        $server_id,             $event_custom_id
    ) = @_;
    ...
    if ( defined($id_agentmodule) && $id_agentmodule != 0 ) {
        $module = get_db_single_row(
            $dbh, 'SELECT *, tagente_estado.datos, tagente_estado.estado
    ...
    if ( defined($tags) && ( $tags ne '' ) ) {
        ...
        if ( defined($id_agentmodule) && $id_agentmodule > 0 ) {
            $module_tags =
              pandora_get_module_tags( $pa_config, $dbh, $id_agentmodule );
        }
    }
    ...
    $module_data           = defined($module) ? $module->{'datos'} : ''
      unless defined($module_data);
    $module_status = defined($module) ? $module->{'estado'} : 0
      unless defined($module_status);

    # If the event is created with validated status, assign ack_utimestamp
    my $ack_utimestamp =
      ( $event_status == 1 || $event_status == 2 ) ? time() : 0;
    ...
    if ( defined($id_extra) && $id_extra ne '' ) {
        my $keep_in_process_status_extra_id =
          pandora_get_tconfig_token( $dbh, 'keep_in_process_status_extra_id',
            0 );

        if ( defined($keep_in_process_status_extra_id)
            && $keep_in_process_status_extra_id == 1 )
        {
            # Keep status if the latest event was In process
            logger(
                $pa_config,
                "Checking status of latest event with extended id " . $id_extra,
                10
            );

# Check if there is a previous event with that extra ID and it is currently in "in process" state
            my $id_extra_inprocess_count =
              get_db_value( $dbh,
                'SELECT COUNT(*) FROM tevento WHERE id_extra=? AND estado=2',
                $id_extra );

            # Only when the event comes as New. Validated events are excluded
            if (   defined($id_extra_inprocess_count)
                && $id_extra_inprocess_count > 0
                && $event_status == 0 )
            {
                logger(
                    $pa_config,
"Keeping In process status from last event with extended id '$id_extra'.",
                    10
                );
                $ack_utimestamp = get_db_value(
                    $dbh,
'SELECT ack_utimestamp FROM tevento WHERE id_extra=? AND estado=2',
                    $id_extra
                );
                $event_status    = 2;
                $event_custom_id = get_db_value(
                    $dbh,
'SELECT event_custom_id FROM tevento WHERE id_extra=? AND estado=2',
                    $id_extra
                );
            }
        }

        logger( $pa_config, "Updating events with extended id '$id_extra'.",
            10 );
        db_do(
            $dbh,
'UPDATE tevento SET estado = 1, ack_utimestamp = ? WHERE estado IN (0,2) AND id_extra=?',
    ...
    logger(
        $pa_config,
"Generating event '$evento' for agent ID $id_agente module ID $id_agentmodule.",
        10
    );
    $event_id = db_insert(
        $dbh, 'id_evento',
'INSERT INTO tevento (id_agente, id_grupo, evento, timestamp, estado, utimestamp, event_type, id_agentmodule, id_alert_am, criticity, tags, source, id_extra, id_usuario, critical_instructions, warning_instructions, unknown_instructions, ack_utimestamp, custom_data, data, module_status, event_custom_id)
	              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        $id_agente,   $id_grupo, safe_input($evento), $timestamp, $event_status,
        $utimestamp,  $event_type, $id_agentmodule,   $id_alert_am, $severity,
        $module_tags, $source, $id_extra, $user_name, $critical_instructions,
        $warning_instructions, $unknown_instructions, $ack_utimestamp,
        $custom_data, safe_input($module_data), $module_status, $event_custom_id
    );

    if ( defined($event_id) && $comment ne '' ) {
        my $comment_id = db_insert(
            $dbh, 'id',
'INSERT INTO tevent_comment (id_event, utimestamp, comment, id_user, action)
											VALUES (?, ?, ?, ?, ?)', $event_id, $utimestamp,
            safe_input($comment), $user_name, "Added comment"
        );
    }
    ...
    if ( !-f $pa_config->{'event_file'} ) {
        $header =
"agent_name,group_name,evento,timestamp,estado,utimestamp,event_type,module_name,alert_name,criticity,tags,source,id_extra,id_usuario,critical_instructions,warning_instructions,unknown_instructions,ack_utimestamp";
    }

    # Open the event file for writing
    my $event_file_fh;
    if ( !open( $event_file_fh, ">>", $pa_config->{'event_file'} ) ) {
        logger( $pa_config,
            "Error opening event file " . $pa_config->{'event_file'} . ": $!",
            10 );
        return $event_id;
    }
    ...
    my $module_name =
      defined($module) ? safe_output( $module->{'nombre'} ) : '';
    my $alert_name = get_db_value(
        $dbh,
'SELECT name FROM talert_templates, talert_template_modules WHERE talert_templates.id = talert_template_modules.id_alert_template AND talert_template_modules.id = ?',
        $id_alert_am
    );
    ...
    flock( $event_file_fh, 2 );

    # Write the event
    print $event_file_fh "$header\n" if ( defined($header) );
    print $event_file_fh "$agent_name,"
      . safe_output($group_name) . ","
      . safe_output($evento)
      . ",$timestamp,$event_status,$utimestamp,$event_type,"
      . safe_output($module_name) . ","
      . safe_output($alert_name)
      . ",$severity,"
      . safe_output($comment) . ","
      . safe_output($module_tags)
      . ",$source,$id_extra,$user_name,"
      . safe_output($critical_instructions) . ","
      . safe_output($warning_instructions) . ","
      . safe_output($unknown_instructions)
      . ",$ack_utimestamp\n";
...
sub pandora_timed_event {
    ...
    my $now = time();
    if ( !defined( $TIMED_EVENTS{$event_msg} )
        || $TIMED_EVENTS{$event_msg} + $time_limit < $now )
    {
...
sub pandora_get_agent_group {
    my ( $pa_config, $dbh, $agent_name, $agent_group, $agent_group_password ) =
      @_;

    my $group_id;
    my $auto_group =
        $pa_config->{'autocreate_group_name'} ne ''
      ? $pa_config->{'autocreate_group_name'}
      : $pa_config->{'autocreate_group'};
    my @groups =
      $pa_config->{'autocreate_group_force'} == 1
      ? ( $auto_group, $agent_group )
    ...
    foreach my $group (@groups) {
        ...
        if ( $group eq $pa_config->{'autocreate_group'} ) {
            ...
            if ( !defined( get_group_name( $dbh, $group_id ) ) ) {
                logger( $pa_config,
                    "Group ID " . $group_id . " does not exist.", 10 );
                next;
            }
        }
        else {
            ...
            if ( $group_id <= 0 ) {
                logger( $pa_config, "Group " . $group . " does not exist.",
                    10 );
                next;
            }
        }
...
sub pandora_update_module_on_error {
    ...
    db_do(
        $dbh,
        'UPDATE tagente_estado SET last_execution_try = ?, current_interval = ?
		WHERE id_agente_modulo = ?', time(), $current_interval,
        $module->{'id_agente_modulo'}
    );
}
...
sub pandora_exec_forced_alerts {
    ...
    my @alerts = get_db_rows(
        $dbh, 'SELECT talert_template_modules.id as id_template_module,
				talert_template_modules.*, talert_templates.*
				FROM talert_template_modules, talert_templates
				WHERE talert_template_modules.id_alert_template = talert_templates.id
    ...
    foreach my $alert (@alerts) {
        ...
        my $module = get_db_single_row(
            $dbh,
            'SELECT * FROM tagente_modulo WHERE id_agente_modulo = ?',
            $alert->{'id_agent_module'}
        );
        if ( !defined($module) ) {
            logger(
        ...
        pandora_execute_alert(
            $pa_config, 'N/A', $agent, $module, $alert, 1,
            $dbh,       undef, 1,      undef
        );

       # Reset the force_execution flag, even if the alert could not be executed
        db_do( $dbh,
            "UPDATE talert_template_modules SET force_execution = 0 WHERE id = "
              . $alert->{'id_template_module'} );
    }
}
...
sub pandora_module_keep_alive_nd {
    ...
    if ( $pa_config->{'warmup_unknown_on'} == 1 ) {
        ...
        return
          if (
            time() < $pa_config->{'__start_utimestamp__'} +
            $pa_config->{'warmup_unknown_interval'} );

        # Disabled from pandora_module_unknown.
    }

    my @modules = get_db_rows(
        $dbh, 'SELECT tagente_modulo.*
					...
					AND ( tagente_estado.utimestamp + (tagente.intervalo * ?) < UNIX_TIMESTAMP())',
        $pa_config->{'unknown_interval'}
    );
    ...
    foreach my $module (@modules) {
        logger(
            $pa_config,
            "Updating keep_alive module for module '"
              . $module->{'nombre'}
              . "' agent ID "
              . $module->{'id_agente'}
              . " (agent without data).",
...
sub pandora_evaluate_snmp_alerts {
    my (
        ...
        $trap_oid,   $trap_type,       $trap_oid_text,
        $trap_value, $trap_custom_oid, $dbh
    ) = @_;

    # Get all SNMP alerts
    my @snmp_alerts =
      get_db_rows( $dbh, 'SELECT * FROM talert_snmp ORDER BY position ASC' );
    my $fired_position;
...
# Check if one alert has been thrown. If there is another with same position, tries to throw it.
        ...
        if ( $oid ne '' ) {
            ...
            # Partial match.
                ...
                next
                  if ( index( $trap_oid, $oid ) == -1
                    && index( $trap_oid_text, $oid ) == -1 );
            }
            $alert_data .= "OID: $oid ";
        }
        ...
        if ( $single_value ne '' ) {
            ...
            # No match
            next
              if ( valid_regex($single_value) == 0
                || $trap_value !~ m/^$single_value$/i );
        ...
        if ( $custom_oid ne '' ) {
            ...
            # No match
            next
              if ( valid_regex($custom_oid) == 0
                || $trap_custom_oid !~ m/^$custom_oid$/i );
        ...
        # Replace macros
        $alert->{'al_field1'} =
          subst_alert_macros( $alert->{'al_field1'}, \%macros );
        $alert->{'al_field2'} =
          subst_alert_macros( $alert->{'al_field2'}, \%macros );
        $alert->{'al_field3'} =
          subst_alert_macros( $alert->{'al_field3'}, \%macros );
        $alert->{'al_field4'} =
          subst_alert_macros( $alert->{'al_field4'}, \%macros );
        $alert->{'al_field5'} =
          subst_alert_macros( $alert->{'al_field5'}, \%macros );
        $alert->{'al_field6'} =
          subst_alert_macros( $alert->{'al_field6'}, \%macros );
        $alert->{'al_field7'} =
          subst_alert_macros( $alert->{'al_field7'}, \%macros );
        $alert->{'al_field8'} =
          subst_alert_macros( $alert->{'al_field8'}, \%macros );
        $alert->{'al_field9'} =
          subst_alert_macros( $alert->{'al_field9'}, \%macros );
        $alert->{'al_field10'} =
          subst_alert_macros( $alert->{'al_field10'}, \%macros );
        $alert->{'al_field11'} =
          subst_alert_macros( $alert->{'al_field11'}, \%macros );
        $alert->{'al_field12'} =
          subst_alert_macros( $alert->{'al_field12'}, \%macros );
        $alert->{'al_field13'} =
          subst_alert_macros( $alert->{'al_field13'}, \%macros );
        $alert->{'al_field14'} =
          subst_alert_macros( $alert->{'al_field14'}, \%macros );
        $alert->{'al_field15'} =
          subst_alert_macros( $alert->{'al_field15'}, \%macros );
        $alert->{'al_field16'} =
          subst_alert_macros( $alert->{'al_field16'}, \%macros );
        $alert->{'al_field17'} =
          subst_alert_macros( $alert->{'al_field17'}, \%macros );
        $alert->{'al_field18'} =
          subst_alert_macros( $alert->{'al_field18'}, \%macros );
        $alert->{'al_field19'} =
          subst_alert_macros( $alert->{'al_field19'}, \%macros );
        $alert->{'al_field20'} =
          subst_alert_macros( $alert->{'al_field20'}, \%macros );

        # Check time threshold
        $alert->{'last_fired'} = '1970-01-01 00:00:00'
          unless defined( $alert->{'last_fired'} );
        return
          unless ( $alert->{'last_fired'} =~
            /(\d+)\-(\d+)\-(\d+) +(\d+):(\d+):(\d+)/ );
        my $last_fired =
          ( $1 > 0 ) ? strftime( "%s", $6, $5, $4, $3, $2 - 1, $1 - 1900 ) : 0;

        my $utimestamp = time();
        my $timestamp = strftime( "%Y-%m-%d %H:%M:%S", localtime($utimestamp) );

        # Out of limits, start a new interval
        ( $times_fired, $internal_counter ) = ( 0, 0 )
          if ( $utimestamp >= ( $last_fired + $alert->{'time_threshold'} ) );

        # Execute the alert
        my ( $min_alerts, $max_alerts ) =
          ( $alert->{'min_alerts'}, $alert->{'max_alerts'} );
        if (   ( $internal_counter + 1 >= $min_alerts )
            && ( $times_fired + 1 <= $max_alerts ) )
        {
...
'SELECT talert_actions.name as action_name, talert_actions.*, talert_commands.*
            ...
            pandora_execute_action(
                $pa_config, $trap_rcv_full, \%agent, \%alert,
                1,          $action,        undef,   $dbh,
                $timestamp, \%macros
            ) if ( defined($action) );
            push(
...
# Generate an event, ONLY if our alert action is different from generate an event.
            ...
            db_do(
                $dbh,
'UPDATE talert_snmp SET times_fired = ?, last_fired = ?, internal_counter = ? WHERE id_as = ?',
                $times_fired,
                $timestamp,
                $internal_counter,
                $alert->{'id_as'}
            ...
            db_do(
                $dbh,
                'UPDATE ttrap SET alerted = 1, priority = ? WHERE id_trap = ?',
                $alert->{'priority'},
                $trap_id
            );
            ...
            @more_actions_snmp =
              get_db_rows( $dbh,
                'SELECT * FROM talert_snmp_action WHERE id_alert_snmp = ?',
                $alert->{'id_as'} );

            foreach my $other_alert (@more_actions_snmp) {
                my $other_action = get_db_single_row(
                    $dbh,
'SELECT talert_actions.name as action_name, talert_actions.*, talert_commands.*
					FROM talert_actions, talert_commands
					WHERE talert_actions.id_alert_command = talert_commands.id
                ...
                pandora_execute_action(
                    $pa_config, $trap_rcv_full, \%agent, \%alert_action,
                    1,          $other_action,  undef,   $dbh,
                    $timestamp, \%macros
                ) if ( defined($other_action) );
                push(
                    @{ $custom_data->{'actions'} },
                    safe_output( $other_action->{'action_name'} )
                );
...
# Now update the new value for times_fired & last_fired if we are below min limit for triggering this alert
                db_do(
                    $dbh,
'UPDATE talert_snmp SET internal_counter = ?, times_fired = ?, last_fired = ? WHERE id_as = ?',
                    $internal_counter,
                    $times_fired,
                    $timestamp,
                    $alert->{'id_as'}
                ...
                db_do(
                    $dbh,
'UPDATE talert_snmp SET times_fired = ?, internal_counter = ? WHERE id_as = ?',
                    $times_fired,
                    $internal_counter,
                    $alert->{'id_as'}
                );
...
sub subst_alert_macros {
    ...
    if ( defined($string) && $string =~ m/^(?:(")(?:.*)"|(')(?:.*)')$/ ) {
        ...
        $subst_func = sub {
            my $macro =
              on_demand_macro( $pa_config, $dbh, shift, $macros, $agent,
                $module, $alert );
            $macro =~ s/'/'\\''/g;    # close, escape, open
            return decode_entities( $quote . "'" . $macro . "'" . $quote )
              ;                       # close, quote, open
        };
    }
    else {
...
sub subst_column_macros {
    ...
    return subst_alert_macros( $string, $macros, $pa_config, $dbh, $agent,
        $module );
}
...
sub on_demand_macro {
    ...
    if ( $macro eq '_agentstatus_' ) {
        return ( defined($agent) )
          ? get_agent_status( $pa_config, $dbh, $agent->{'id_agente'} )
          : '';
    }
    elsif ( $macro eq '_modulegroup_' ) {
        return ( defined($module) )
          ? ( get_module_group_name( $dbh, $module->{'id_module_group'} )
              || '' )
          : '';
    }
    elsif ( $macro eq '_modulestatus_' ) {
        return ( defined($module) )
          ? get_agentmodule_status_str( $pa_config, $dbh,
            $module->{'id_agente_modulo'} )
          : '';
    }
    elsif ( $macro eq '_moduletags_' ) {
        return ( defined($module) )
          ? pandora_get_module_tags( $pa_config, $dbh,
            $module->{'id_agente_modulo'} )
          : '';
    }
    elsif ( $macro eq '_email_tag_' ) {
        return ( defined($module) )
          ? pandora_get_module_email_tags( $pa_config, $dbh,
            $module->{'id_agente_modulo'} )
          : '';
    }
    elsif ( $macro eq '_phone_tag_' ) {
        return ( defined($module) )
          ? pandora_get_module_phone_tags( $pa_config, $dbh,
            $module->{'id_agente_modulo'} )
          : '';
    }
        ...
        my $field_value  = get_db_value(
            $dbh,
'SELECT description FROM tagent_custom_data WHERE id_field=? AND id_agent=?',
            $field_number,
            $agent_id
        );
        ...
        my $field_value = get_db_value(
            $dbh,
'SELECT unit FROM tagente_modulo where id_agente_modulo = ? limit 1',
            $module->{'id_agente_modulo'}
        );
    }
        ...
        if ( $module->{'id_tipo_modulo'} eq 3 ) {
            my $field_value = get_db_value(
                $dbh,
'SELECT datos FROM tagente_datos_string where id_agente_modulo = ? order by utimestamp desc limit 1 offset 1',
                $module->{'id_agente_modulo'}
            );
        }
        else {
            my $field_value = get_db_value(
                $dbh,
'SELECT datos FROM tagente_datos where id_agente_modulo = ? order by utimestamp desc limit 1 offset 1',
                $module->{'id_agente_modulo'}
            );
        }
    }
        ...
        my @rows = get_db_rows(
            $dbh,
'SELECT ip FROM taddress_agent taag, taddress ta WHERE ta.id_a = taag.id_a AND id_agent = ?',
            $module->{'id_agente'}
        );
        ...
        my @rows         = get_db_rows(
            $dbh,
'SELECT ip FROM taddress_agent taag, taddress ta WHERE ta.id_a = taag.id_a AND id_agent = ? ORDER BY ip ASC',
            $module->{'id_agente'}
        );
        ...
        my $id_mod = get_db_value(
            $dbh,
'SELECT id_agente_modulo FROM tagente_modulo WHERE id_agente = ? AND nombre = ?',
            $module->{'id_agente'},
            $field_number
        );
        my $module_data = get_db_single_row(
            $dbh,
'SELECT id_tipo_modulo, unit FROM tagente_modulo WHERE id_agente_modulo = ?',
            $id_mod
        );
        ...
        if ( $field_value =~ /^data:image\/png;base64, / ) {
            ...
            # macro _data_ substitution in case is image.
            $field_value = '<img style="height: 150px;" src="cid:moduledata_'
              . $id_mod . '"/>';
        }
        ...
        my @groups = get_db_rows(
            $dbh,
'SELECT tg.nombre from tagent_secondary_group as tsg INNER JOIN tgrupo tg ON tsg.id_group = tg.id_grupo WHERE tsg.id_agent = ?',
            $module->{'id_agente'}
        );
...
sub process_data {
    ...
    if ( $module_type =~ m/_inc$/ ) {
        $data =
          process_inc_data( $pa_config, $data, $module, $agent, $utimestamp,
            $dbh );
    ...
    # Process absolute INC modules
        ...
        $data = process_inc_abs_data( $pa_config, $data, $module, $agent,
            $utimestamp, $dbh );
    ...
    # Process the rest of modules
        ...
        return unless check_min_max( $pa_config, $data, $module, $agent );
    }
...
sub post_process {
    ...
    return ( is_numeric( $module->{'post_process'} )
          && $module->{'post_process'} != 0 )
      ? $data * $module->{'post_process'}
      : $data;
}
...
sub check_min_max {
        ...
        && ( $data > $module->{'max'} || $data < $module->{'min'} ) )
    {
        if ( $module->{'max'} < $module->{'min'} ) {
            ...
            return 1
              unless ( ( $module->{'max'} == 0 && $data < $module->{'min'} )
                || ( $module->{'min'} == 0 && $data > $module->{'max'} ) );
...
sub process_inc_data {
    ...
    my $data_inc = get_db_single_row(
        $dbh,
        'SELECT * FROM tagente_datos_inc WHERE id_agente_modulo = ?',
        $module->{'id_agente_modulo'}
    );
    ...
    if ( $utimestamp < $data_inc->{'utimestamp'} ) {
        logger(
            $pa_config,
            "Received old data for incremental module "
              . $module->{'nombre'}
              . "(module id "
              . $module->{'id_agente_modulo'} . ").",
            3
    ...
    if ( $utimestamp == $data_inc->{'utimestamp'} ) {
        logger(
            $pa_config,
            "Duplicate timestamp for incremental module "
              . $module->{'nombre'}
              . "(module id "
              . $module->{'id_agente_modulo'} . ").",
            3
    ...
    if ( $data < $data_inc->{'datos'} ) {
        db_do(
            $dbh,
'UPDATE tagente_datos_inc SET datos = ?, utimestamp = ? WHERE id_agente_modulo = ?',
            $data,
            $utimestamp,
            $module->{'id_agente_modulo'}
        );
        logger(
            $pa_config,
            "Discarding data and resetting counter for incremental module "
              . $module->{'nombre'}
              . "(module id "
              . $module->{'id_agente_modulo'} . ").",
            10
        ...
        db_do(
            $dbh,
'UPDATE tagente_estado SET utimestamp = ? WHERE id_agente_modulo = ?',
            time(),
            $module->{'id_agente_modulo'}
        );
        return;
    ...
    my $rate = ( $data - $data_inc->{'datos'} ) /
      ( $utimestamp - $data_inc->{'utimestamp'} );
...
sub get_module_status {
    ...
    my ( $critical_min, $critical_max, $warning_min, $warning_max )     = (
        $module->{'min_critical'}, $module->{'max_critical'},
        $module->{'min_warning'},  $module->{'max_warning'}
    );
    my ( $critical_str, $warning_str ) =
      ( $module->{'str_critical'}, $module->{'str_warning'} );
    my $eval_result;
    ...
    $critical_str =
      ( defined($critical_str) && valid_regex($critical_str) == 1 )
      ? safe_output($critical_str)
      : '';
    $warning_str =
      ( defined($warning_str) && valid_regex($warning_str) == 1 )
      ? safe_output($warning_str)
      : '';

    # Adjust percentage max/min values.
    if ( defined( $module->{'percentage_critical'} )
        && $module->{'percentage_critical'} == 1 )
    {
    ...
    if ( defined( $module->{'percentage_warning'} )
        && $module->{'percentage_warning'} == 1 )
    {
    ...
    if (   ( $module_type =~ m/_proc$/ || $module_type =~ /web_analysis/ )
        && ( $critical_min eq $critical_max ) )
    {
        ( $critical_min, $critical_max ) = ( 0, 1 );
    }
    elsif ( $module_type =~ m/keep_alive/
        && ( $critical_min eq $critical_max ) )
    {
        ( $critical_min, $critical_max ) = ( 0, 1 );
    }
    ...
    if ( $module_type !~ m/_string/ ) {
        ...
        if ( $warning_min ne $warning_max ) {
            ...
            # [warning_min, warning_max)
                ...
                return 2
                  if ( $data >= $warning_min && $warning_max < $warning_min );
            }
        }
    }
...
sub pandora_validate_event {
    ...
    if ( !defined($id_agentmodule)
        || $pa_config->{"event_auto_validation"} == 0 )
    {
        return;
    }

    logger( $pa_config,
        "Validating events for id_agentmodule #$id_agentmodule", 10 );
    my $now = time();
    db_do(
        $dbh,
'UPDATE tevento SET estado = 1, ack_utimestamp = ? WHERE estado = 0 AND id_agentmodule = '
          . $id_agentmodule,
        $now
    );
}
...
sub generate_status_event {
    my ( $pa_config, $data, $agent, $module, $status, $last_status,
        $known_status, $dbh )
      = @_;
    ...
    if ( $pa_config->{'warmup_event_on'} == 1 ) {
        ...
        return
          if (
            time() < $pa_config->{'__start_utimestamp__'} +
            $pa_config->{'warmup_event_interval'} );

        $pa_config->{'warmup_event_on'} = 0;
        logger( $pa_config, "Warmup mode for events ended.", 10 );
        pandora_event( $pa_config, "Warmup mode for events ended.",
            0, 0, 0, 0, 0, 'system', 0, $dbh );
    }
        ...
        && $module->{'disabled_types_event'} )
    {
        ...
        eval {
            ...
            $disabled_types_event =
              decode_json( $module->{'disabled_types_event'} );
        };
    ...
    if ( $status == 0 ) {
        ...
        if ( $known_status == 1 ) {
            ...
            $description =
              safe_output( $pa_config->{"text_going_down_warning"} );
        }
        ...
        logger(
            $pa_config,
            "Unknown status $status for module '"
              . $module->{'nombre'}
              . "' agent '"
              . $agent->{'nombre'} . "'.",
            10
    ...
    if ( $status != 0 ) {
        pandora_event(
            ...
            '',
            '',
            $module->{'critical_instructions'},
            $module->{'warning_instructions'},
...
sub save_module_data {
    ...
    if ( $module_type eq "log4x" ) {
        ...
        my $sql =
"INSERT INTO tagente_datos_log4x(id_agente_modulo, utimestamp, severity, message, stacktrace) values (?, ?, ?, ?, ?)";

        db_do(
            $dbh,                          $sql,
            $module->{'id_agente_modulo'}, $utimestamp,
            $data_object->{'severity'},    $data_object->{'message'},
...
sub export_module_data {
    my ( $pa_config, $data, $agent, $module, $module_type, $timestamp, $dbh ) =
      @_;
    ...
    db_do(
        $dbh, 'INSERT INTO tserver_export_data 
		(id_export_server, agent_name , module_name, module_type, data, timestamp) VALUES
		(?, ?, ?, ?, ?, ?)', $module->{'id_export'}, $agent->{'alias'},
        $module->{'nombre'}, $module_type, $data, $timestamp
    );
}
...
sub pandora_inhibit_alerts {
    ...
    return 0
      if ( $agent->{'cascade_protection'} ne '1'
        || $agent->{'id_parent'} eq '0'
        || $depth > 1024 );
...
'SELECT COUNT(*) FROM tagente_modulo, talert_template_modules, talert_templates
            ...
            $agent->{'cascade_protection_module'}
        );
    }
    else {
        $count = get_db_value(
...
sub save_agent_position {
    my ( $pa_config, $current_longitude, $current_latitude, $current_altitude,
        $agent_id, $dbh, $start_timestamp, $description )
      = @_;

    logger(
        $pa_config,
"Updating agent position: longitude=$current_longitude, latitude=$current_latitude, altitude=$current_altitude, start_timestamp=$start_timestamp agent_id=$agent_id",
        10
    );
...
sub update_agent_position {
    my (
        $pa_config,        $current_longitude, $current_latitude,
        $current_altitude, $agent_id,          $dbh,
        $stored_longitude, $stored_latitude,   $stored_altitude,
        $start_timestamp,  $description
    ) = @_;

    if (   defined($stored_longitude)
        && defined($stored_latitude)
        && defined($start_timestamp) )
    {
        # Upadate all the position data of the agent
        logger(
            $pa_config,
"Updating agent position: current_longitude=$current_longitude, current_latitude=$current_latitude,
						 current_altitude=$current_altitude, stored_longitude=$stored_longitude, stored_latitude=$stored_latitude,
						 stored_altitude=$stored_altitude, start_timestamp=$start_timestamp, agent_id=$agent_id",
            10
        );
        db_do(
            $dbh,
'UPDATE tgis_data_status SET current_longitude = ?, current_latitude = ?, current_altitude = ?,
            ...
            $stored_longitude,  $stored_latitude,
        ...
        logger(
            $pa_config,
"Updating agent position: longitude=$current_longitude, latitude=$current_latitude, altitude=$current_altitude, agent_id=$agent_id",
            10
        );
...
sub archive_agent_position {
    my (
        $pa_config, $start_timestamp, $end_timestamp, $longitude,
        $latitude,  $altitude,        $description,   $number_packages,
        $agent_id,  $dbh
    ) = @_;

    logger(
        $pa_config,
"Saving new agent position: start_timestamp=$start_timestamp longitude=$longitude latitude=$latitude altitude=$altitude",
        10
    );

    db_do(
        $dbh,
'INSERT INTO tgis_data_history (longitude, latitude, altitude, tagente_id_agente, start_timestamp,
					end_timestamp, description, number_of_packages) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        $longitude,     $latitude,    $altitude, $agent_id, $start_timestamp,
        $end_timestamp, $description, $number_packages
    );
...
sub pandora_server_statistics {
    ...
    my @servers = get_db_rows(
        $dbh,
        'SELECT * FROM tserver WHERE BINARY name = ?',
        $pa_config->{'servername'}
    );
    ...
    foreach my $server (@servers) {
        ...
        if ( $server->{"server_type"} == INVENTORYSERVER ) {
            ...
            $server->{"modules"} = get_db_value(
                $dbh,
"SELECT COUNT(tagent_module_inventory.id_agent_module_inventory) FROM tagente, tagent_module_inventory WHERE tagente.disabled=0 AND tagent_module_inventory.id_agente = tagente.id_agente AND tagente.server_name = ?",
                $server->{"name"}
            );

            # Get total exported modules
            $server->{"modules_total"} = get_db_value( $dbh,
"SELECT COUNT(tagent_module_inventory.id_agent_module_inventory) FROM tagente, tagent_module_inventory WHERE tagente.disabled=0 AND tagent_module_inventory.id_agente = tagente.id_agente"
            );

            # Calculate lag
            $lag_row = get_db_single_row(
                $dbh,
"SELECT COUNT(tagent_module_inventory.id_agent_module_inventory) AS `module_lag`, AVG(UNIX_TIMESTAMP() - utimestamp - tagent_module_inventory.interval) AS `lag` 
        ...
        # Export server
            ...
            $server->{"modules"} = get_db_value(
                $dbh,
"SELECT COUNT(tagente_modulo.id_agente_modulo) FROM tagente, tagente_modulo, tserver_export WHERE tagente.disabled=0 AND tagente_modulo.id_agente = tagente.id_agente AND tagente_modulo.id_export = tserver_export.id AND tserver_export.id_export_server = ?",
                $server->{"id_server"}
            );

            # Get total exported modules
            $server->{"modules_total"} = get_db_value( $dbh,
"SELECT COUNT(tagente_modulo.id_agente_modulo) FROM tagente, tagente_modulo WHERE tagente.disabled=0 AND tagente_modulo.id_agente = tagente.id_agente AND tagente_modulo.id_export != 0"
            );
            ...
            $server->{"modules"} = get_db_value(
                $dbh,
"SELECT COUNT(id_rt) FROM trecon_task WHERE id_recon_server = ?",
                $server->{"id_server"}
            );

            # Total recon jobs (all servers)
            $server->{"modules_total"} =
              get_db_value( $dbh, "SELECT COUNT(status) FROM trecon_task" );

            # Lag (take average active time of all active tasks)

            $server->{"lag"} = get_db_value(
                $dbh,
"SELECT UNIX_TIMESTAMP() - utimestamp from trecon_task WHERE UNIX_TIMESTAMP() > (utimestamp + interval_sweep) AND interval_sweep > 0 AND id_recon_server = ?",
                $server->{"id_server"}
            );

            $server->{"module_lag"} = get_db_value(
                $dbh,
"SELECT COUNT(id_rt) FROM trecon_task WHERE UNIX_TIMESTAMP() > (utimestamp + interval_sweep) AND interval_sweep > 0 AND id_recon_server = ?",
                $server->{"id_server"}
            );
            ...
            $server->{"modules"} = get_db_value(
                $dbh,
"SELECT count(tagente_estado.id_agente_modulo) FROM tagente_estado, tagente_modulo, tagente WHERE tagente.disabled=0 AND tagente_modulo.id_agente = tagente.id_agente AND tagente_modulo.disabled = 0 AND tagente_modulo.id_agente_modulo = tagente_estado.id_agente_modulo AND tagente_estado.running_by = ?",
                $server->{"id_server"}
            );

            $server->{"modules_total"} = get_db_value(
                $dbh,
"SELECT count(tagente_estado.id_agente_modulo) FROM tserver, tagente_estado, tagente_modulo, tagente WHERE tagente.disabled=0 AND tagente_modulo.id_agente = tagente.id_agente AND tagente_modulo.disabled = 0 AND tagente_modulo.id_agente_modulo = tagente_estado.id_agente_modulo AND tagente_estado.running_by = tserver.id_server AND tserver.server_type = ?",
                $server->{"server_type"}
            );
        ...
        db_do( $dbh,
                "UPDATE tserver SET lag_time = '"
              . $server->{"lag"}
              . "', lag_modules = '"
              . $server->{"module_lag"}
              . "', total_modules_running = '"
              . $server->{"modules_total"}
              . "', my_modules = '"
              . $server->{"modules"}
              . "' , stat_utimestamp = UNIX_TIMESTAMP() WHERE id_server = "
              . $server->{"id_server"} );
    }
}
...
sub pandora_self_monitoring {
    ...
    $xml_output =
        "<agent_data os_name='$OS' os_version='$OS_VERSION' version='"
      . $pa_config->{'version'}
      . "' description='"
      . $pa_config->{'rb_product_name'}
      . " Server version "
      . $pa_config->{'version'}
      . "' agent_name='"
      . $pa_config->{"self_monitoring_agent_name"}
      . "' agent_alias='"
      . $pa_config->{"self_monitoring_agent_name"}
      . "' interval='"
    ...
    my $my_data_server = get_db_value(
        $dbh,
        "SELECT id_server FROM tserver WHERE server_type = ? AND name = '"
          . $pa_config->{"servername"} . "'",
        DATASERVER
    );
    ...
    if ( defined($my_data_server) ) {
        $agents_unknown = get_db_value(
            $dbh, "SELECT COUNT(DISTINCT tagente_estado.id_agente)
    ...
    my $queued_modules = get_db_value( $dbh,
            "SELECT SUM(queued_modules) FROM tserver WHERE BINARY name = '"
          . $pa_config->{"servername"}
          . "'" );
    ...
    my $queued_alerts =
      get_db_value( $dbh, "SELECT count(id) FROM talert_execution_queue" );

    if ( !defined($queued_alerts) ) {
        $queued_alerts = 0;
    }

    my $alert_server_status =
      get_db_value( $dbh, "SELECT status FROM tserver WHERE server_type = ?",
        ALERTSERVER );
    ...
    my $pandoradb        = 0;
    my $pandoradb_tstamp = get_db_value( $dbh,
        "SELECT `value` FROM tconfig WHERE token = 'db_maintance'" );
    if ( !defined($pandoradb_tstamp) || $pandoradb_tstamp == 0 ) {
        pandora_event( $pa_config,
            "Pandora DB maintenance tool has never been run.",
            0, 0, 4, 0, 0, 'system', 0, $dbh );
    }
    elsif ( $pandoradb_tstamp < time() - 86400 ) {
        pandora_event(
            $pa_config,
            "Pandora DB maintenance tool has not been run since "
              . strftime( "%Y-%m-%d %H:%M:%S", localtime($pandoradb_tstamp) )
              . ".",
            0,
            0,
            4,
    ...
    my $num_threads = get_db_value( $dbh,
            'SELECT SUM(threads) FROM tserver WHERE name = "'
          . $pa_config->{"servername"}
          . '"' );
    my $cpu_load = 0;
    $cpu_load = cpu_load();
    ...
    my $data_in_files_badxml =
      count_files_ext( $pa_config->{"incomingdir"}, 'data_BADXML' );
    my $averageTime = 0;

    if (   defined($totalModuleIntervalTime)
        && defined($totalNetworkModules)
        && $totalModuleIntervalTime > 0 )
    {
    ...
    my $filename =
      ...
      . $pa_config->{"self_monitoring_agent_name"} . ".self"
      . $utimestamp . ".data";
    open( my $xml_fh, ">", $filename )
      or die
"[FATAL] Could not open internal monitoring XML file for deploying monitorization at '$filename'";
    print $xml_fh $xml_output;
    close($xml_fh);
}
...
Generate stats for Pandora Open threads.
...
sub pandora_thread_monitoring {
    ...
    $xml_output =
      ...
      . "' agent_name='"
      . $pa_config->{'self_monitoring_agent_name'}
      . "' agent_alias='pandora.internals' interval='"
      . $pa_config->{"self_monitoring_interval"}
      . "' timestamp='"
      . $timestamp . "' >";
    foreach my $server ( @{$servers} ) {
        ...
        while ( my ( $tid, $stats ) = each( %{$producer_stats} ) ) {
            ...
            $xml_output .=
                " <name>"
              . uc( $ServerTypes[ $server->{'_server_type'} ] )
            ...
            $xml_output .=
              ...
              . (
                time() - $stats->{'tstamp'} <
                  2 * $pa_config->{"self_monitoring_interval"} ? 1 : 0 )
              . "</data>";
            $xml_output .=
            ...
            $xml_output .=
                " <name>"
              . uc( $ServerTypes[ $server->{'_server_type'} ] )
              . " Producer Queued Elements</name>";
            $xml_output .= " <type>generic_data</type>";
            $xml_output .= " <module_group>Performance</module_group>";
            $xml_output .=
              " <data>" . ( $#{ $stats->{'task_queue'} } + 1 ) . "</data>";
        ...
        foreach my $tid ( sort( keys( %{$consumer_stats} ) ) ) {
            ...
            $xml_output .=
                " <name>"
              . uc( $ServerTypes[ $server->{'_server_type'} ] )
              . " Consumer #$idx Processing Rate</name>";
            $xml_output .= " <type>generic_data</type>";
            $xml_output .= " <module_group>Performance</module_group>";
            $xml_output .= " <data>" . $stats->{'rate'} . "</data>";
            $xml_output .=
    ...
    open( my $xml_fh, ">", $filename )
      or die
      "[FATAL] Could not write to the thread monitoring XML file '$filename'";
    print $xml_fh $xml_output;
    close($xml_fh);
}
...
sub pandora_installation_monitoring {
    ...
    $module->{'data'} =
      get_db_value( $dbh, 'SELECT COUNT(DISTINCT(id_agente)) FROM tagente' );
    push( @modules, $module );
    undef $module;
    ...
    $module->{'data'}        = get_db_value( $dbh,
        'SELECT COUNT(DISTINCT(id_agente_modulo)) FROM tagente_modulo' );
    push( @modules, $module );
    undef $module;
    ...
    $module->{'data'} =
      get_db_value( $dbh, 'SELECT COUNT(DISTINCT(id_grupo)) FROM tgrupo' );
    push( @modules, $module );
    undef $module;
    ...
    $module->{'data'} =
      get_db_value( $dbh, 'SELECT COUNT(id_agente_modulo) FROM tagente_datos' );
    ...
    $module->{'data'}        = get_db_value( $dbh,
        'SELECT COUNT(id_agente_modulo) FROM tagente_datos_string' );
    $module->{'module_interval'} = '288';
    push( @modules, $module );
    undef $module;
    ...
    $module->{'data'} =
      get_db_value( $dbh, 'SELECT COUNT(id_user) FROM tusuario' );
    push( @modules, $module );
    undef $module;
    ...
    $module->{'data'} =
      get_db_value( $dbh, 'SELECT COUNT(id_session) FROM tsessions_php' );
    push( @modules, $module );
    undef $module;
    ...
    $module->{'data'}        = get_db_value( $dbh,
"SELECT COUNT(DISTINCT(id_agente_modulo)) FROM tagente_estado WHERE estado = 4"
    );
    push( @modules, $module );
    undef $module;
    ...
    my $select = get_db_single_row( $dbh,
        'SHOW /*!50000 GLOBAL */ STATUS WHERE Variable_name= ?', 'Com_select' );
    my $insert = get_db_single_row( $dbh,
        'SHOW /*!50000 GLOBAL */ STATUS WHERE Variable_name= ?', 'Com_insert' );
    my $update = get_db_single_row( $dbh,
        'SHOW /*!50000 GLOBAL */ STATUS WHERE Variable_name= ?', 'Com_update' );
    my $replace =
      get_db_single_row( $dbh,
        'SHOW /*!50000 GLOBAL */ STATUS WHERE Variable_name= ?',
        'Com_replace' );
    my $delete = get_db_single_row( $dbh,
        'SHOW /*!50000 GLOBAL */ STATUS WHERE Variable_name= ?', 'Com_delete' );
    my $data_size = get_db_value( $dbh,
        'SELECT SUM(data_length)/(1024*1024) FROM information_schema.TABLES' );
    my $index_size = get_db_value( $dbh,
    ...
    $module->{'description'} =
      'MySQL: Questions - Reads (#): Number of read questions';
    $module->{'data'} = $reads;
    $module->{'unit'} = 'qu/s';
    ...
    $module->{'description'} =
      'MySQL: Questions - Writes (#): Number of writed questions';
    $module->{'data'} = $writes;
    $module->{'unit'} = 'qu/s';
    $module->{'type'} = 'generic_data_inc';
    push( @modules, $module );
    ...
    $module->{'description'} =
      'MySQL: Size of data (MB): Size of stored data in megabytes';
    ...
    $module->{'description'} =
      'Size of indexes (MB): Size of stored indexes in megabytes';
    ...
    if ( pandora_get_tconfig_token( $dbh, 'console_log_enabled', 0 ) == 1 ) {
        $log_files->{'console_log'} =
          '/var/www/html/pandora_consle/log/console.log';
    }

    if ( pandora_get_tconfig_token( $dbh, 'audit_log_enabled', 0 ) == 1 ) {
        $log_files->{'audit_log'} =
          '/var/www/html/pandora_consle/log/audit.log';
    }
...
sub pandora_set_master {
    ...
    my $current_master = get_db_value_limit(
        $dbh, 'SELECT name FROM tserver 
...
sub pandora_is_master {
    ...
    if ( defined($dbh) && $pa_config->{'multiprocess'} == 1 ) {
        ...
        if ( defined($current_master)
            && $current_master eq $pa_config->{'servername'} )
        {
            return 1;
        }
...
sub pandora_module_unknown {
    ...
    if ( $pa_config->{'warmup_unknown_on'} == 1 ) {
        ...
        pandora_event( $pa_config, "Warmup mode for unknown modules ended.",
            0, 0, 0, 0, 0, 'system', 0, $dbh );
    }
    ...
    my @modules = get_db_rows(
		...
		WHERE tagente.id_agente = tagente_estado.id_agente 
          ...
          . ')
			...
			AND (tagente_estado.current_interval * ?) + tagente_estado.utimestamp < UNIX_TIMESTAMP() LIMIT ?',
        $pa_config->{'unknown_interval'},
        $pa_config->{'unknown_block_size'}
    );
    ...
    foreach my $module (@modules) {
            ...
            || $module->{'id_tipo_modulo'} == 23 )
        {
            ...
            logger( $pa_config,
                "Module " . $module->{'nombre'} . " is going to NORMAL", 10 );
            db_do(
                $dbh,
'UPDATE tagente_estado SET last_status = 0, estado = 0, known_status = 0, last_known_status = 0, last_status_change = ? WHERE id_agente_estado = ?',
                time(),
                $module->{'id_agente_estado'}
            );
            ...
            if ( !defined($agent) ) {
                logger(
                    ...
                    "Agent ID "
                      . $module->{'id_agente'}
                      . " not found while executing unknown alerts for module '"
                      . $module->{'nombre'} . "'.",
                    3
                );
                return;
            ...
            # Update module status count
            pandora_mark_agent_for_module_update( $dbh,
                $module->{'id_agente'} );
                ...
                && pandora_cps_enabled( $agent, $module ) == 0 )
            {
                ...
                pandora_generate_alerts(
                    $pa_config,    0,      3,    $agent,
                    $module,       time(), $dbh, $timestamp,
                    $extra_macros, 0,      'unknown'
                );
            }
            else {
                logger(
                    $pa_config,
                    "Alerts inhibited for agent '" . $agent->{'nombre'} . "'.",
                    10
                );
            }
            ...
            load_module_macros( $module->{'module_macros'}, \%macros );
            $description =
              subst_alert_macros( $description, \%macros, $pa_config, $dbh,
                $agent, $module );
...
# Set the module status to unknown (the module can already be unknown if unknown_updates is enabled).
            if ( $module->{'estado'} != 3 ) {
                logger( $pa_config,
                    "Module " . $module->{'nombre'} . " is going to UNKNOWN",
                    10 );
                my $utimestamp = time();
                db_do(
                    $dbh,
'UPDATE tagente_estado SET last_status = 3, estado = 3, last_unknown_update = ?, last_status_change = ? WHERE id_agente_estado = ?',
                    $utimestamp,
                    $utimestamp,
                    ,
                    $module->{'id_agente_estado'}
            ...
            if (   $pa_config->{'unknown_events'} == 0
                || $module->{'estado'} == 3 )
            {    # Already in unknown status (unknown_updates is enabled).
                $do_event = 0;
            }
            elsif ( !defined( $module->{'disabled_types_event'} )
                || $module->{'disabled_types_event'} eq "" )
            {
                $do_event = 1;
            }
            else {
...
sub pandora_disable_autodisable_agents {
    ...
    $disable_agents =~ s/,$//ig;
    logger( $pa_config,
        "Autodisable agents ($disable_agents) will be disabled", 9 );

    db_do(
        $dbh, 'UPDATE tagente SET disabled=1 
...
#~ my @tags = get_db_rows ($dbh, 'SELECT ' . db_concat('ttag.name', 'ttag.url') . ' name_url FROM ttag, ttag_module
    my @tags = get_db_rows(
        $dbh, 'SELECT ttag.name FROM ttag, ttag_module
	                               WHERE ttag.id_tag = ttag_module.id_tag
	                               AND ttag_module.id_agente_modulo = ?',
        $id_agentmodule
    );
...
sub pandora_get_module_url_tags {
    ...
    my @tags = get_db_rows(
        $dbh, 'SELECT ttag.name,ttag.url name_url FROM ttag, ttag_module
	                               WHERE ttag.id_tag = ttag_module.id_tag
	                               AND ttag_module.id_agente_modulo = ?',
        $id_agentmodule
    );
...
sub pandora_get_module_email_tags {
    ...
    my @email_tags = get_db_rows(
        $dbh, 'SELECT ttag.email FROM ttag, ttag_module
	                               WHERE ttag.id_tag = ttag_module.id_tag
	                               AND ttag_module.id_agente_modulo = ?',
        $id_agentmodule
    );
...
sub pandora_get_module_phone_tags {
    ...
    my @phone_tags = get_db_rows(
        $dbh, 'SELECT ttag.phone FROM ttag, ttag_module
	                               WHERE ttag.id_tag = ttag_module.id_tag
	                               AND ttag_module.id_agente_modulo = ?',
        $id_agentmodule
    );
...
sub pandora_mark_agent_for_module_update {
    ...
    db_do( $dbh, "UPDATE tagente SET update_module_count=1 WHERE id_agente=?",
        $agent_id );
}
...
sub pandora_mark_agent_for_alert_update {
    ...
    # Update the status count
    db_do( $dbh, "UPDATE tagente SET update_alert_count=1 WHERE id_agente=?",
        $agent_id );
}
...
sub pandora_update_agent_module_count {
    ...
    db_do(
		...
		WHERE id_agente = ?', $counts->{'0'}, $counts->{'1'}, $counts->{'2'},
        $counts->{'3'}, $counts->{'4'}, $total, $agent_id
    );
...
sub pandora_update_agent_alert_count {
    ...
    db_do(
        $dbh, 'UPDATE tagente SET update_alert_count=0,
	fired_count=(SELECT COUNT(*) FROM tagente_modulo, talert_template_modules WHERE tagente_modulo.disabled=0 AND tagente_modulo.id_agente_modulo=talert_template_modules.id_agent_module AND talert_template_modules.disabled=0 AND times_fired>0 AND id_agente='
          . $agent_id . ') WHERE id_agente = ' . $agent_id
    );
...
sub pandora_update_secondary_groups_cache {
    ...
    db_do( $dbh,
        'UPDATE tagente SET update_secondary_groups=0 WHERE id_agente = '
          . $agent_id );
...
sub pandora_get_os {
    ...
    my $os_id = get_db_value(
        $dbh,
        'SELECT id_os FROM tconfig_os WHERE name LIKE ?',
        '%' . $os . '%'
    );
    if ( defined($os_id) ) {
        return $os_id;
...
sub pandora_get_os_by_id {
    ...
    # Search for a custom OS
    my $os_name =
      get_db_value( $dbh, 'SELECT name FROM tconfig_os WHERE id_os = ?',
        $os_id );
    if ( defined($os_name) ) {
        return $os_name;
    }
...
sub pandora_create_custom_graph {
    ...
    my (
        $name,   $description, $user,    $idGroup, $width,
        $height, $events,      $stacked, $period,  $dbh
    ) = @_;
    ...
    my $graph_id =
      db_insert( $dbh, 'id_graph', "INSERT INTO tgraph $columns", @{$values} );
...
sub pandora_insert_graph_source {
    ...
    my $source_id =
      db_insert( $dbh, 'id_gs', "INSERT INTO tgraph_source $columns",
        @{$values} );
...
sub pandora_delete_graph_source {
    ...
    if ( defined($id_module) ) {
        $result = db_do(
            $dbh, 'DELETE FROM tgraph_source 
			WHERE id_graph = ?
			AND id_agent_module = ?', $id_graph, $id_module
        );
    }
    else {
        $result = db_do( $dbh, 'DELETE FROM tgraph_source WHERE id_graph = ?',
            $id_graph );
    }
...
sub pandora_delete_custom_graph {
    ...
    my $result =
      db_do( $dbh, 'DELETE FROM tgraph WHERE id_graph = ?', $id_graph );
...
sub pandora_edit_custom_graph {
    ...
    my (
        $id_graph, $name,   $description, $user,   $idGroup, $width,
        $height,   $events, $stacked,     $period, $dbh
    ) = @_;

    my $graph = get_db_single_row(
        $dbh, 'SELECT * FROM tgraph
    ...
    my $res = db_do(
        $dbh,
'UPDATE tgraph SET name = ?, id_user = ?, description = ?, period = ?, width = ?,
		height = ?, private = 0, id_group = ?, events = ?, stacked = ?
		WHERE id_graph = ?', $name, $user, $description, $period, $width, $height,
        $idGroup, $events, $stacked, $id_graph
    );
...
Process a password to be stored in the Pandora Open Database (encrypting it if
necessary).
...
sub pandora_input_password {
    ...
    # Encryption disabled.
    return $password
      if ( !defined( $pa_config->{'encryption_key'} )
        || $pa_config->{'encryption_key'} eq '' );
...
Process a password retrieved from the Pandora Open Database (decrypting it if
necessary).
...
sub safe_mode {
    ...
    if ( $new_status == MODULE_CRITICAL ) {
        logger( $pa_config,
            "Enabling safe mode for agent " . $agent->{'nombre'}, 10 );
        db_do(
            $dbh,
'UPDATE tagente_modulo SET disabled=1, disabled_by_safe_mode=1 WHERE id_agente=? AND id_agente_modulo!=? AND disabled=0',
            $agent->{'id_agente'},
            $module->{'id_agente_modulo'}
        );
    }
    ...
    # Coming back from critical. Enable the rest of the modules.
        ...
        logger( $pa_config,
            "Disabling safe mode for agent " . $agent->{'nombre'}, 10 );
        db_do(
            $dbh,
'UPDATE tagente_modulo SET disabled=0, disabled_by_safe_mode=0 WHERE id_agente=? AND id_agente_modulo!=? AND disabled_by_safe_mode=1',
            $agent->{'id_agente'},
            $module->{'id_agente_modulo'}
        );
        ...
        db_do(
            $dbh,
'UPDATE tagente_estado SET utimestamp = ? WHERE id_agente = ? AND id_agente_modulo!=?',
            time(),
            $agent->{'id_agente'},
            $module->{'id_agente_modulo'}
        );
...
sub pandora_safe_mode_modules_update {
    ...
    my $agent =
      get_db_single_row( $dbh,
        'SELECT alias, safe_mode_module FROM tagente WHERE id_agente = ?',
        $agent_id );
    ...
    my $status =
      get_agentmodule_status( $pa_config, $dbh, $agent->{'safe_mode_module'} );

    # If status is critical, disable the rest of the modules.
    if ( $status == MODULE_CRITICAL ) {
        logger(
            $pa_config,
            "Update modules for safe mode agent with alias:"
              . $agent->{'alias'} . ".",
            10
        );
        db_do(
...
sub process_inventory_data {
    ...
    foreach my $inventory ( @{ $data->{'inventory'} } ) {
        ...
        foreach my $module_data ( @{ $inventory->{'inventory_module'} } ) {
            ...
            process_inventory_module_data(
                $pa_config,   $data_list, $server_id, $agent_name,
                $module_name, $interval,  $timestamp, $dbh
            );
        }
    }
}
...
sub process_inventory_module_data {
    ...
    logger( $pa_config,
        "Processing inventory module '$module_name' for agent '$agent_name'.",
        10 );

    # Get agent data
    my $agent =
      get_db_single_row( $dbh, 'SELECT * FROM tagente WHERE nombre = ?',
        safe_input($agent_name) );
    if ( !defined($agent) ) {
        logger(
            $pa_config,
"Agent '$agent_name' not found for inventory module '$module_name'.",
            3
        );
        return;
    }
        ...
        && $timestamp !~ /(\d+)\-(\d+)\-(\d+) +(\d+):(\d+):(\d+)/ )
    {
        logger(
            $pa_config,
"Invalid timestamp '$timestamp' from module '$module_name' agent '$agent_name'.",
            3
        );
        return;
    }
...
"Invalid timestamp '$timestamp' from module '$module_name' agent '$agent_name'.",
    ...
    my $inventory_module = get_db_single_row(
        $dbh,
...
'SELECT id_module_inventory FROM tmodule_inventory WHERE name = ? AND id_os = ?',
        ...
        $id_agent_module_inventory = db_insert(
            $dbh, 'id_agent_module_inventory',
"INSERT INTO tagent_module_inventory (id_agente, id_module_inventory, 
			...
			VALUES (?, ?, ?, ?, ?, ?, ?)",
            $agent->{'id_agente'}, $module_id,  $interval, safe_input($data),
            $timestamp,            $utimestamp, 0
        );
...
'INSERT INTO tagente_datos_inventory (id_agent_module_inventory, data, timestamp, utimestamp)
			VALUES (?, ?, ?, ?)',
            $id_agent_module_inventory, safe_input($data), $timestamp,
            $utimestamp
        );
    }
    else {
...
sub process_inventory_module_diff {
    my ( $pa_config, $incoming_data, $inventory_module, $timestamp,
        $utimestamp, $dbh, $interval )
      = @_;
    ...
    my $agent_module_inventory_id =
      $inventory_module->{'id_agent_module_inventory'};
    ...
    if ( decode( 'UTF-8', $stored_data ) ne $incoming_data ) {
        ...
        db_do(
            $dbh,
'INSERT INTO tagente_datos_inventory (id_agent_module_inventory, data, timestamp, utimestamp) VALUES (?, ?, ?, ?)',
            $agent_module_inventory_id,
            $incoming_data,
            $timestamp,
            $utimestamp
        );
        ...
        if ( $stored_utimestamp != 0 ) {
            my $inventory_changes_blacklist =
              pandora_get_config_value( $dbh, 'inventory_changes_blacklist' );
            my $inventory_module_blocked = 0;

            if ( $inventory_changes_blacklist ne "" ) {
                foreach my $inventory_id_excluded (
                    split( ',', $inventory_changes_blacklist ) )
                {
# If the inventory_module_id is in the blacklist, the change will not be processed
                    if ( $inventory_module->{'id_module_inventory'} ==
                        $inventory_id_excluded )
                    {
                        logger(
                            $pa_config,
"Inventory change omitted on inventory #$inventory_id_excluded due be on the changes blacklist",
                            10
                        );
...
# If the inventory_module_id is in the blacklist, the change will not be processed
            if ( $inventory_module_blocked == 0 ) {
                my $inventory_module_name = get_db_value(
                    $dbh,
"SELECT name FROM tmodule_inventory WHERE id_module_inventory = ?",
                    $module_inventory_id
                );
...
# Use 'safe_output' to avoid double encode the entities when creating the event with 'pandora_event'
                        $final_d .= "DELETED RECORD: "
                          . safe_output( $values_stored[$i] ) . "\n";
                    }
                }
   ...
   # }
                ...
                pandora_event(
                    ...
                    "Configuration change:\n"
                      ...
                      . $final_a
                      . " for agent '"
                      . safe_output($agent_alias)
                      . "' module '"
                      . safe_output($inventory_module_name) . "'.",
                    $group_id,
                    $agent_id,
                    0,
                    0,
    ...
    if ( defined($interval) ) {
        db_do(
            $dbh, 'UPDATE tagent_module_inventory 
			SET'
              . $RDBMS_QUOTE
              . 'interval'
              . $RDBMS_QUOTE
              . '=?, data=?, timestamp=?, utimestamp=? 
...
sub escalate_warning {
    ...
    if ( $agent_status->{'warning_count'} > $module->{'warning_time'} ) {
        logger(
            $pa_config,
            "Escalating warning status to critical status for agent ID "
              . $agent->{'id_agente'}
              . " module '"
              . $module->{'nombre'} . "'.",
            10
        );
        $agent_status->{'warning_count'} =
          $module->{'warning_time'} + 1;    # Prevent overflows.
...
sub pandora_snmptrapd_still_working {
    ...
    if ( $pa_config->{'snmpserver'} eq '1' ) {
        ...
        my $lastTimestampSaved = get_db_value(
            $dbh, 'SELECT UNIX_TIMESTAMP(timestamp)
			FROM ttrap
        ...
        my ( $protocol, $date, $time ) =
          split( /\[\*\*\]/, $lastTimestampLogFile, 4 );
      ...
      # If time or date not filled in, probably havent caught any snmptraps yet.
        if ( defined $date && defined $time && $time ne '' && $date ne '' ) {
            ...
            my $lastTimestampLogFile =
              timelocal( $sec, $min, $hour, $day, $month - 1, $year );
            if (   $lastTimestampSaved ne $lastTimestampLogFile
                && $lastTimestampLogFile
                gt( $lastTimestampSaved + $timeMaxLapse ) )
            {
                my $lapseMessage = "snmptrapd service probably is stuck.";
                logger( $pa_config, $lapseMessage, 1 );
                pandora_event( $pa_config, $lapseMessage, 0, 0, 4, 0, 0,
                    'system', 0, $dbh );
            }
        }
...
sub exec_cluster_status_module {
    ...
    foreach my $agent_module (@modules) {
        ...
        if ( $agent_module->{'prediction_module'} == 6 ) {
            logger(
                $pa_config,
                "Executing cluster active-active critical module "
                  . $agent_module->{'nombre'},
                10
            );
            exec_cluster_aa_module( $pa_config, $agent_module, $server_id,
                $dbh );
        }
        ...
        # Cluster active-passive module.
            ...
            logger(
                $pa_config,
                "Executing cluster active-passive critical module "
                  . $agent_module->{'nombre'},
                10
            );
            exec_cluster_ap_module( $pa_config, $agent_module, $server_id,
                $dbh );
        }
    }
    ...
    foreach my $cluster_module (@modules) {
        next
          if ( $cluster_module->{'id_agente_modulo'} ==
            $module->{'id_agente_modulo'} );    # Skip the current module.
        ...
        if ( $cluster_module->{'estado'} == MODULE_NORMAL && $data < 0 ) {
            ...
            (
                   $cluster_module->{'estado'} == MODULE_CRITICAL
                || $cluster_module->{'estado'} == MODULE_UNKNOWN
            )
            && $data < 2
          )
        {
    ...
    # Update the module.
    pandora_process_module( $pa_config, { 'data' => $data },
        '', $module, '', $timestamp, $utimestamp, $server_id, $dbh );

    pandora_update_agent( $pa_config, $timestamp, $module->{'id_agente'},
        undef, undef, -1, $dbh );
}
...
sub exec_cluster_aa_module {
    ...
    my $item = get_db_single_row(
        $dbh,
        'SELECT * FROM tcluster_item WHERE id=?',
    ...
    my @agents = get_db_rows(
        $dbh,
        'SELECT id_agent FROM tcluster_agent WHERE id_cluster = ?',
        $module->{'custom_integer_1'}
    );
    foreach my $agent_id (@agents) {
        my $item_status = get_db_value(
            $dbh,
            'SELECT estado
			FROM tagente_estado, tagente_modulo
			WHERE tagente_estado.id_agente_modulo = tagente_modulo.id_agente_modulo
    ...
    # Update module
    pandora_process_module( $pa_config, { 'data' => $not_normal },
        '', $module, '', $timestamp, $utimestamp, $server_id, $dbh );
...
sub exec_cluster_ap_module {
    ...
    foreach my $agent_id (@agents) {
        ...
        if (   defined($status)
            && $status->{'estado'} != MODULE_UNKNOWN
            && $status->{'utimestamp'} > $utimestamp )
        {
...
sub snmp_traps_monitoring {
    ...
    return unless $pa_config->{'snmpconsole'} == 1;
    my $xml_output = '';
    ...
    $module->{'description'} =
      'Size of snmp_logfile (MB): Size of snmp trap log in megabytes';
    $module->{'data'}         = $size_in_mb;
    $module->{'unit'}         = 'MB';
    $module->{'min_critical'} = 1024;
    $module->{'max_critical'} = 0;
...
L<DBI>, L<XML::Simple>, L<HTML::Entities>, L<Time::Local>, L<POSIX>, L<PandoraOpen::DB>, L<PandoraOpen::Config>, L<PandoraOpen::Tools>, L<PandoraOpen::GIS>
