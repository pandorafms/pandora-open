...
sub db_connect {
    ...
    if ( $rdbms eq 'mysql' ) {
        ...
        # Connect to MySQL
        my $dbh = DBI->connect(
            "DBI:mysql:$db_name:$db_host:$db_port;$SSL_OPTS",
            $db_user,
            $db_pass,
            { RaiseError => 1, AutoCommit => 1, AutoInactiveDestroy => 1 }
        );
        ...
        # Connect to Oracle
        my $dbh = DBI->connect(
"DBI:Oracle:dbname=$db_name;host=$db_host;port=$db_port;sid=$db_name",
            $db_user, $db_pass, { RaiseError => 1, AutoCommit => 1 }
        );
        return unless defined($dbh);
...
sub db_history_connect {
    ...
    $conf{'history_db_enabled'} =
      get_db_value( $dbh, "SELECT value FROM tconfig WHERE token = ?",
        "history_db_enabled" );
    $conf{'history_db_host'} =
      get_db_value( $dbh, "SELECT value FROM tconfig WHERE token = ?",
        "history_db_host" );
    $conf{'history_db_port'} =
      get_db_value( $dbh, "SELECT value FROM tconfig WHERE token = ?",
        "history_db_port" );
    $conf{'history_db_name'} =
      get_db_value( $dbh, "SELECT value FROM tconfig WHERE token = ?",
        "history_db_name" );
    $conf{'history_db_user'} =
      get_db_value( $dbh, "SELECT value FROM tconfig WHERE token = ?",
        "history_db_user" );
    $conf{'history_db_pass'} =
      get_db_value( $dbh, "SELECT value FROM tconfig WHERE token = ?",
        "history_db_pass" );

    my $history_dbh =
      ( $conf{'history_db_enabled'} eq '1' )
      ? db_connect(
        $pa_config->{'dbengine'}, $conf{'history_db_name'},
        $conf{'history_db_host'}, $conf{'history_db_port'},
        $conf{'history_db_user'}, $conf{'history_db_pass'}
      )
      : undef;
...
sub get_console_api_url {
    ...
    if ( !defined( $pa_config->{"console_api_url"} ) ) {
        my $console_api_url =
          PandoraOpen::Config::pandora_get_tconfig_token( $dbh, 'public_url',
            '' );
        ...
        if ( $console_api_url eq '' ) {
            $pa_config->{"console_api_url"} =
              'http://127.0.0.1/pandora_console/' . $include_api;
            logger(
                $pa_config,
                "Assuming default path for API url: "
                  . $pa_config->{"console_api_url"},
                3
            );
        }
...
sub get_action_id {
    ...
    my $rc = get_db_value(
        $dbh, "SELECT id FROM talert_actions
...
sub get_action_name {
    ...
    my $rc = get_db_value(
        $dbh, "SELECT name FROM talert_actions
...
sub get_command_id {
    ...
    my $rc = get_db_value(
        $dbh,
        "SELECT id FROM talert_commands WHERE name = ?",
        safe_input($command_name)
    );
    return defined($rc) ? $rc : -1;
}
...
sub get_agent_id {
    my ( $dbh, $agent_name ) = @_;
    my $is_meta = get_db_value( $dbh,
        "SELECT value FROM tconfig WHERE token like 'metaconsole'" );

    my $rc;
    if ( $is_meta == 1 ) {
        $rc =
          get_db_value( $dbh,
            "SELECT id_agente FROM tmetaconsole_agent WHERE nombre = ?",
...
sub get_agent_ids_from_alias {
    ...
    my @rc = get_db_rows(
        $dbh,
        "SELECT id_agente, nombre FROM tagente WHERE alias = ?",
        safe_input($agent_alias)
    );
...
sub get_template_id {
    ...
    my $rc = get_db_value(
        $dbh, "SELECT id FROM talert_templates
...
sub get_template_name {
    ...
    my $rc = get_db_value(
        $dbh, "SELECT name FROM talert_templates
...
sub get_server_id {
    ...
    my $rc = get_db_value(
        $dbh, "SELECT id_server FROM tserver
...
sub get_group_id {
    ...
    my $rc =
      get_db_value( $dbh,
        'SELECT id_grupo FROM tgrupo WHERE ' . db_text('nombre') . ' = ?',
        safe_input($group_name) );
    return defined($rc) ? $rc : -1;
}
...
sub get_os_id {
    ...
    my $rc = get_db_value( $dbh, "SELECT id_os FROM tconfig_os WHERE name = ?",
        $os_name );
    return defined($rc) ? $rc : -1;
}
...
sub get_os_name {
    ...
    my $rc = get_db_value( $dbh, "SELECT name FROM tconfig_os WHERE id_os = ?",
        $os_id );
    return defined($rc) ? $rc : -1;
}
...
sub set_agentmodule_custom_id {
    ...
    my $rc =
      db_update( $dbh,
        ...
        safe_input($custom_id), safe_input($id_agent_module) );
...
sub get_agent_status {
    ...
    foreach my $module (@modules) {
        ...
        my $module_type = get_db_value(
            $dbh, 'SELECT id_tipo_modulo
			FROM tagente_modulo
...
# If all the modules are asynchronous or keep alive, the group cannot be unknown
    if ( $modules_async < $count_modules ) {
        my $last_contact = get_db_value(
            $dbh,
'SELECT (UNIX_TIMESTAMP(ultimo_contacto) + (intervalo * 2)) AS last_contact
			FROM tagente WHERE id_agente = ? AND UNIX_TIMESTAMP(ultimo_contacto) > 0',
            $agent_id
        );
...
sub get_module_agent_id {
    ...
    return get_db_value( $dbh,
        "SELECT id_agente FROM tagente_modulo WHERE id_agente_modulo = ?",
        $agent_module_id );
}
...
sub get_agent_address {
    ...
    return get_db_value( $dbh,
        "SELECT direccion FROM tagente WHERE id_agente = ?", $agent_id );
}
...
sub get_module_name {
    ...
    return get_db_value( $dbh,
        "SELECT nombre FROM tagente_modulo WHERE id_agente_modulo = ?",
        $module_id );
}
...
sub get_agent_module_id {
    ...
    my $rc = get_db_value(
        $dbh,
"SELECT id_agente_modulo FROM tagente_modulo WHERE delete_pending = 0 AND nombre = ? AND id_agente = ?",
...
sub get_template_module_id {
    ...
    my $rc = get_db_value(
        $dbh,
"SELECT id FROM talert_template_modules WHERE id_agent_module = ? AND id_alert_template = ?",
        $module_id,
        $template_id
    );
    return defined($rc) ? $rc : -1;
...
sub is_group_disabled {
    ...
    return get_db_value( $dbh,
        "SELECT disabled FROM tgrupo WHERE id_grupo = ?", $group_id );
}
...
sub get_module_id {
    ...
    my $rc = get_db_value(
        $dbh,
        "SELECT id_tipo FROM ttipo_modulo WHERE nombre = ?",
        safe_input($module_name)
    );
    return defined($rc) ? $rc : -1;
}
...
sub get_user_disabled {
    ...
    my $rc =
      get_db_value( $dbh, "SELECT disabled FROM tusuario WHERE id_user = ?",
        safe_input($user_id) );
    return defined($rc) ? $rc : -1;
}
...
sub get_user_exists {
    ...
    my $rc =
      get_db_value( $dbh, "SELECT id_user FROM tusuario WHERE id_user = ?",
        safe_input($user_id) );
...
sub get_plugin_id {
    ...
    my $rc = get_db_value(
        $dbh,
        "SELECT id FROM tplugin WHERE name = ?",
        safe_input($plugin_name)
    );
    return defined($rc) ? $rc : -1;
}
...
sub get_module_group_id {
    ...
    if ( $case_insensitve == 0 ) {
        $rc = get_db_value(
            $dbh,
            "SELECT id_mg FROM tmodule_group WHERE name = ?",
            safe_input($module_group_name)
        );
    }
    else {
        $rc = get_db_value(
            $dbh,
            "SELECT id_mg FROM tmodule_group WHERE LOWER(name) = ?",
            lc( safe_input($module_group_name) )
        );
    }
    return defined($rc) ? $rc : -1;
}
...
sub get_module_group_name {
    ...
    return get_db_value( $dbh, "SELECT name FROM tmodule_group WHERE id_mg = ?",
        $module_group_id );
}
...
sub get_nc_profile_name {
    ...
    return get_db_value( $dbh,
        "SELECT * FROM tnetwork_profile WHERE id_np = ?", $nc_id );
}
...
sub get_user_profile_id {
    ...
    my $rc = get_db_value(
        $dbh, "SELECT id_up FROM tusuario_perfil
	                              WHERE id_usuario = ?
								  AND id_perfil = ?
								  AND id_grupo = ?",
        safe_input($user_id),
...
sub get_profile_id {
    ...
    my $rc = get_db_value(
        $dbh,
        "SELECT id_perfil FROM tperfil WHERE name = ?",
...
sub get_group_name {
    ...
    return get_db_value( $dbh, "SELECT nombre FROM tgrupo WHERE id_grupo = ?",
        $group_id );
}
...
sub get_db_value_limit {
    ...
    if ( $RDBMS ne 'oracle' ) {
        ...
        $sth = $dbh->prepare_cached(
            'SELECT * FROM (' . $query . ') WHERE ROWNUM <= ' . int($limit) );
    }
...
sub get_db_single_row {
    ...
    while ( my $row = $sth->fetchrow_hashref() ) {
        $sth->finish();
        return { map { lc($_) => $row->{$_} } keys( %{$row} ) }
          if ( $RDBMS eq 'oracle' );
        return $row;
    }
...
##########################################################################
## Get DB information for all known Pandora Open nodes.
##########################################################################
sub get_db_nodes {
    my ( $dbh, $pa_config ) = @_;
    my $dbh_nodes = [];
    ...
    my @nodes = get_db_rows( $dbh,
        'SELECT * FROM tmetaconsole_setup WHERE disabled = 0' );
    foreach my $node (@nodes) {

        # Check and decrypy passwords if necessary.
        if ( defined( $pa_config->{'encryption_passphrase'} ) ) {
            $node->{'dbpass'} =
              PandoraOpen::Core::pandora_output_password( $pa_config,
                $node->{'dbpass'} );
        }
...
sub get_db_rows_node {
    ...
    eval {
        $dbh = db_connect(
            $node->{'dbengine'}, $node->{'dbname'}, $node->{'dbhost'},
            $node->{'dbport'},   $node->{'dbuser'}, $node->{'dbpass'}
        );
    ...
    if ($@) {
        ...
        my $dbh = db_connect(
            $pa_config->{'dbengine'}, $pa_config->{'dbname'},
            $pa_config->{'dbhost'},   $pa_config->{'dbport'},
            $pa_config->{'dbuser'},   $pa_config->{'dbpass'}
        );

        my $msg =
            "Cannot connect to node database: "
          . $node->{'dbhost'}
          . ". Please check node credentials.";
        logger( $pa_config, "[ERROR] " . $msg, 3 );
        PandoraOpen::Core::pandora_event( $pa_config, $msg, 0, 0, 4, 0, 0,
            'error', 0, $dbh );
...
##########################################################################
## Run get_db_rows on all known Pandora Open nodes in parallel.
##########################################################################
sub get_db_rows_parallel {
    ...
    my @threads;
    {
        ...
        local *PandoraOpen::ProducerConsumerServer::DESTROY      = sub { };
        local *PandoraOpen::BlockProducerConsumerServer::DESTROY = sub { };
        local *PandoraOpen::SNMPServer::DESTROY                  = sub { };

        # Query the nodes.
        foreach my $node ( @{$nodes} ) {
            my $thr =
              threads->create( \&get_db_rows_node, $pa_config, $node, $query,
                @values );
...
sub db_update_hash {
    ...
    return unless ( defined($tablename) && $tablename ne "" );
...
sub set_update_agent {
    ...
    return unless ( defined($agent_id) && $agent_id > 0 );
    return unless ( ref($data) eq "HASH" );

    return db_update_hash( $dbh, 'tagente', { 'id_agente' => $agent_id },
        $data );
}
...
sub set_update_agentmodule {
    ...
    return unless ( defined($agentmodule_id) && $agentmodule_id > 0 );
    return unless ( ref($data) eq "HASH" );

    return db_update_hash( $dbh, 'tagente_modulo',
        { 'id_agente_modulo' => $agentmodule_id }, $data );
}
...
sub db_delete_limit {
    ...
    if ( $RDBMS eq 'mysql' ) {
        $sth = $dbh->prepare_cached(
            "DELETE FROM $from WHERE $where LIMIT " . int($limit) );
    }
    ...
    # Oracle
        ...
        $sth = $dbh->prepare_cached(
            "DELETE FROM (SELECT * FROM $from WHERE $where) WHERE ROWNUM <= "
              . int($limit) );
    }
...
sub get_alert_template_module_id {
    ...
    my $rc = get_db_value(
        $dbh,
"SELECT id FROM talert_template_modules WHERE id_agent_module = ? AND id_alert_template = ? AND id_policy_alerts = ?",
        $id_module,
        $id_template,
        $id_policy_alerts
    );
...
sub db_process_insert {
    ...
    my $res =
      db_insert( $dbh, $index,
        "INSERT INTO $table ($columns_string) VALUES " . $wildcards,
        @values_array );
...
sub db_insert_from_array_hash {
    ...
    eval {
        foreach my $row ( @{$data} ) {
            push @inserted_keys,
              db_insert_from_hash( $dbh, $index, $table, $row );
        }
    };
    if ($@) {
        return;
...
sub add_address {
    ...
    return db_insert( $dbh, 'id_a', 'INSERT INTO taddress (ip) VALUES (?)',
        $ip_address );
}
...
sub add_new_address_agent {
    ...
    db_do(
        $dbh, 'INSERT INTO taddress_agent (id_a, id_agent)
...
sub db_text {
    ...
    #return $string;
    return " dbms_lob.substr(" . $string . ", 4000, 1)"
      if ( $RDBMS eq 'oracle' );
...
sub db_concat {
    ...
    return " " . $element1 . " || ' ' || " . $element2 . " "
      if ( $RDBMS eq 'oracle' or $RDBMS eq 'postgresql' );
...
########################################################################
## Try to obtain a persistent lock using Pandora Open's database.
########################################################################
sub db_get_pandora_lock {
    ...
    if ( $lock != 0 ) {
        my $lock_value = get_db_value( $dbh,
"SELECT `value` FROM tconfig WHERE token = 'pandora_lock_$lock_name'"
        );
        if ( !defined($lock_value) ) {
            ...
            my $sth =
              $dbh->prepare('UPDATE tconfig SET `value`=? WHERE `token`=?');
...
sub set_ssl_opts {
    ...
    if ( defined( $pa_config->{'verify_mysql_ssl_cert'} )
        && $pa_config->{'verify_mysql_ssl_cert'} ne "" )
    {
        $SSL_OPTS .= ";mysql_ssl_verify_server_cert="
          . $pa_config->{'verify_mysql_ssl_cert'};
    }
    if ( defined( $pa_config->{'dbsslcapath'} )
        && $pa_config->{'dbsslcapath'} ne "" )
    {
        $SSL_OPTS .= ";mysql_ssl_ca_path=" . $pa_config->{'dbsslcapath'};
    }
    if ( defined( $pa_config->{'dbsslcafile'} )
        && $pa_config->{'dbsslcafile'} ne "" )
    {
...
sub db_synch {
    ...
    foreach my $node (@nodes) {
        eval {
            local $SIG{__DIE__};
            my @values_queue = (
                safe_input($query), $node->{'id'}, time(), $type, $table, '',
                $result
            );

            my $query_queue =
'INSERT INTO tsync_queue (`sql`, `target`, `utimestamp`, `operation`, `table`, `error`, `result`) VALUES (?, ?, ?, ?, ?, ?, ?)';
