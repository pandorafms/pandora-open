#!/usr/bin/perl
################################################################################
# Pandora Open Performance Report Tool
...
my $STRESS_AGENT_XML = {        # Hash representing data coming from an XML data file.
          'os_name' => 'Other',
          'version' => $PandoraOpen::VERSION,
          'interval' => '300',
          'description' => 'Pandora Open XML stress agent',
          'os_version' => '',
...
Pandora Open tool for MySQL testing and optimization.
...
sub stress_agent {
	...
	PandoraOpen::DataServer->new($conf);

	# Create the stress agent.
	my $agent_id = get_agent_id($dbh, $STRESS_AGENT_NAME);
	if ($agent_id == -1) {
		print_log("XML", "creating stress agent");
		$agent_id = pandora_create_agent ($conf, '', safe_input($STRESS_AGENT_NAME), '127.0.0.1', 10,  0, 10, 'Pandora Open XML stress agent', 300, $dbh);
	} else {
	...
	for (my $i = 0; $i < $STRESS_AGENT_OPS; $i++) {
		...
		PandoraOpen::DataServer::process_xml_data ($conf, $0, $STRESS_AGENT_XML, 0, $dbh);
	}
...
# Init Pandora Open libs
pandora_init(\%conf,"Pandora Open Performance Report Tool");
...
Pandora Open Performance Report Tool v$VERSION
...
(**) Single thread worst case scenario. Not representative of the real
     performance of a multi-threaded Pandora Open Data Server.
