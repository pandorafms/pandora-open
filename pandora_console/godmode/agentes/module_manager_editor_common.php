...
$tableBasicThresholds->data['switch_warning_threshold'][0] .= html_print_switch_radio_button(
    [
        ...
        html_print_radio_button_extended('warning_thresholds_checks', 'warning_inverse', __('Inverse interval'), ($warning_inverse) ? 'warning_inverse' : false, false, '', '', true, false, '', 'radius-warning_inverse'),
        html_print_radio_button_extended('warning_thresholds_checks', 'percentage_warning', __('Percentage'), ($percentage_warning) ? 'percentage_warning' : false, false, '', '', true, false, '', 'radius-percentage_warning'),
    ],
    ['class' => 'margin-top-10'],
    true
);
...
$tableBasicThresholds->data['switch_critical_threshold'][0] .= html_print_switch_radio_button(
    [
        ...
        html_print_radio_button_extended('critical_thresholds_checks', 'critical_inverse', __('Inverse interval'), ($critical_inverse) ? 'critical_inverse' : false, false, '', '', true, false, '', 'radius-critical_inverse'),
        html_print_radio_button_extended('critical_thresholds_checks', 'percentage_critical', __('Percentage'), ($percentage_critical) ? 'percentage_critical' : false, false, '', '', true, false, '', 'radius-percentage_critical'),
    ],
    ['class' => 'margin-top-10'],
    true
);
...
$table_advanced->data['custom_id_category'][0] = html_print_input_text(
    ...
    65,
    true,
    (($config['module_custom_id_ro']) ? true : false),
    false,
    '',
    (($config['module_custom_id_ro']) ? 'readonly' : '')
);
...
if (isset($id_agente) === true && (int) $moduletype === MODULE_DATA) {
    ...
    $tableCron->data['cron_from_select'][0] = html_print_extended_select_for_cron($hour_from, $minute_from, $mday_from, $month_from, $wday_from, true, true);

    $tableCron->data['caption_cron_to_select'][0] = __('Cron to');
    $tableCron->data['cron_to_select'][0] = html_print_extended_select_for_cron($hour_to, $minute_to, $mday_to, $month_to, $wday_to, true, true, true);
} else {
    $tableCron->data['caption_cron_from_select'][0] = __('Cron from');
    ...
    $tableCron->data['caption_cron_to_select'][0] = __('Cron to');
    $tableCron->data['cron_to_select'][0] = html_print_extended_select_for_cron($hour_to, $minute_to, $mday_to, $month_to, $wday_to, true, false, true);
}
...
$table_advanced->data['min_max_values'][0] = html_print_input_text('min', $min, '', 5, 15, true, false, false, '', $classdisabledBecauseInPolicy.' w100p');
$table_advanced->data['min_max_values'][1] = html_print_input_text('max', $max, '', 5, 15, true, false, false, '', $classdisabledBecauseInPolicy.' w100p');
...
$table_advanced->data['max_timeout_retries'][0] = html_print_input_text('max_timeout', $max_timeout, '', 5, 10, true, false, false, '', $classdisabledBecauseInPolicy.' w100p');
$table_advanced->data['max_timeout_retries'][1] = html_print_input_text('max_retries', $max_retries, '', 5, 10, true, false, false, '', $classdisabledBecauseInPolicy.' w100p');
...
if (isset($module_macros)) {
    if (is_array($module_macros)) {
        foreach ($module_macros as $macro_name => $macro_value) {
            ...
            $table_macros->data[$macro_count][1] = html_print_input_text('module_macro_names[]', $macro_name, '', 50, 60, true, false, false, '', $classdisabledBecauseInPolicy);
            $table_macros->data[$macro_count][2] = __('Value');
            $table_macros->data[$macro_count][3] = html_print_input_text('module_macro_values[]', $macro_value, '', 50, 60, true, false, false, '', $classdisabledBecauseInPolicy);
            if (!$disabledBecauseInPolicy) {
