            ...
            onload: () => {
                $(document).ready(function () {
                    ...
                    <div class="welcome-wizard-buttons">
                        ...
                        <button type="button" class="secondary buttonButton close-wizard-button"><?php echo __('Close wizard'); ?></button>
                    </div>
                    `);
                    ...
                    // Register button handlers here (in the correct jQuery context,
                    // not inside $.globalEval which may not share the same $ scope).
                    $(document).off('click.welcome');

                    $(document).on('click.welcome', '#button-go_wizard', function() {
                        var selected = $('#task_to_perform').val();
                        if (!selected) {
                            alert("<?php echo __('You must chose an option'); ?>");
                            return;
                        }
                        switch (selected) {
                            case 'load_demo_data':      openCreateDemoDataDialog();    break;
                            case 'wizard_agent':        deployAgent();                 break;
                            case 'check_mail_alert':    openCreateAlertMailDialog();   break;
                            case 'check_connectivity':  openCreateConnectivityDialog(); break;
                            case 'check_web':           openCreateModulesDialog();     break;
                            case 'check_net':           openCreateBasicNetDialog();    break;
                        }
                    });

                    $(document).on('click.welcome', '#button-create_goliat', function() {
                        $.ajax({
                            async: false,
                            type: "POST",
                    ...
                    $(document).on('click.welcome', '#button-create_conectivity', function() {
                    ...
                    $(document).on('click.welcome', '#button-basic_net', function() {
                        var wmi_credentials = [];
                    ...
                    $(document).on('click.welcome', '#button-alert_mail', function() {
                        $.ajax({
                            async: false,
                            type: "POST",
                            url: "include/ajax/task_to_perform.php",
        ...
        if (check_acl($config['id_user'], 0, 'PM')) {
            ...
            if ($flag_um === false || $flag_cm === false || $flag_su === false || $flag_lv === false) {
                ...
                $inputs[] = [
                    ...
                    'block_content' => [
                        [
                            'label'     => __('It seems that your Pandora FMS is working correctly.<br>For more information use the self-diagnosis tool.'),
                            'arguments' => [
            ...
            $output .= html_print_div(
                [
                    ...
                    <p class="mrgn_top_5px">'.__('Are you familiar with').' <a href="https://pandoraopen.io/" target="_blank"><b>'.__('our eLearning system?').'</b></a>'.__(' It\'s completely free for PAO (Operation) and PAT (Administration) courses. Learn how to use Pandora FMS at your own pace.').'</p>',
                ],
                true
            );
        }
    ...
    <script type="text/javascript">
        ...
        function openCreateModulesDialog() {
            $('#dialog_goliat').dialog({
                ...
                resizable: true, draggable: true, modal: true,
                close: false, height: 400, width: 500,
            }).show();
        }
        ...
        function openCreateDemoDataDialog() {
            $('#dialog_demo').dialog({
                ...
                resizable: true, draggable: true, modal: true,
                close: false, height: 300, width: 480,
            }).show();
        }
        ...
        function openCreateConnectivityDialog() {
            $('#dialog_connectivity').dialog({
                ...
                resizable: true, draggable: true, modal: true,
                close: false, height: 350, width: 480,
            }).show();
        }
        ...
        function openCreateBasicNetDialog() {
            $('#dialog_basic_net').dialog({
                ...
                resizable: true, draggable: true, modal: true,
                close: false, height: 590, width: 925,
            }).show();
        }
        ...
        function openCreateAlertMailDialog() {
            $.ajax({
                ...
                type: "POST",
                url: "include/ajax/task_to_perform.php",
                data: { create_unknown_template_alert: 1 },
                success: function(data) {
                    ...
                    $('#text-id_agent').autocomplete({ appendTo: '#dialog_alert_mail' });
                    $("#id_agent_module").select2({ dropdownParent: $("#dialog_alert_mail") });
                }
            });
        }
    </script>
