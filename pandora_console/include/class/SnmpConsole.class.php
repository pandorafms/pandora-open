...
class SnmpConsole extends HTML
{
    ...
    public function draw()
    {
        ...
        // Build timestamp ranges for filtering.
        $now_timestamp = time();
        ...
        try {
            ...
            if ($filters['filter_severity'] != -1) {
                ...
                if ($filters['filter_severity'] !== EVENT_CRIT_OR_NORMAL
                    ...
                    // Search oid in text or oid field in ttrap.
            ...
            if ($filters['filter_hours_ago'] > 0) {
                $whereSubquery .= '
                    AND (`ttrap`.`timestamp` BETWEEN from_unixtime('.$ago_timestamp.') 
                    AND from_unixtime('.$now_timestamp.'))';
            }
