...
function networkmap_create_agent_node(
    ...
    if ((int) $simple === 0) {
        ...
        $node = $agent['id_node'].' [ parent="'.$agent['id_parent'].'", color="'.$status_color.'", fontsize='.$font_size.', shape="doublecircle"'.$url_node_link.', style="filled", fixedsize=true, width=0.20, height=0.20, label="", tooltip="'.$ajax_prefix.'ajax.php?page=operation/agentes/ver_agente&get_agent_status_tooltip=1&id_agent='.$agent['id_agente'].'"];'."\n";
    }
...
function networkmap_links_to_js_links(
    ...
    foreach ($relations as $key => $relation) {
        ...
        if ($relation['parent_type'] == 1) {
            ...
            $item['status_end'] = modules_get_agentmodule_status((int) $id_target_module, false, null);
        ...
        if ($relation['child_type'] == NODE_MODULE) {
            ...
            $item['status_start'] = modules_get_agentmodule_status((int) $id_source_module, false, null);
