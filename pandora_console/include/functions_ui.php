...
function ui_format_alert_row(
    ...
    $actions = alerts_get_alert_agent_module_actions($alert['id'], false, -1);
 ...
 * @param boolean $trash             Deprecated. Do not use.
 *
 * @return string A full URL in Pandora.
 */
function ui_get_full_url($url='', $no_proxy=false, $add_name_php_file=false, $trash=true)
{
    global $config;
...
function ui_get_mobile_login_icon()
{
    ...
    return 'mobile/images/pandora_mobile_console.png';
}
...
function ui_print_styled_tablist($tabs)
{
    $return = '';

    foreach ($tabs as $tab) {
        $return .= '<li><div class="styled-tab-item flex-column"><a href="#'.$tab['id'].'">'.$tab['tag'].'</a><div class="tab-border"></div></div></li>';
    }
