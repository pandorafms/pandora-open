 ...
 */
function api_set_new_agent($thrash1, $thrash2, $other, $trhash3, $return=false, $message=false)
{
    global $config;
 ...
 *
 * @param int $id_agent
 * @param int $thrash Don't use
 * @param $thrash1 Don't use.
 * @param $returnType
 **/
// http://localhost/pandora_console/include/api.php?op=get&op2=agent_alias&id=1&apipass=1234&user=admin&pass=pandora
function api_get_agent_alias($id_agent, $thrash, $trash1, $returnType)
{
    $table_agent_alias = 'tagente';

    if (!util_api_check_agent_and_print_error($id_agent, $returnType, 'AR')) {
        return;
    }
...
/**
 * Get agents alias and id given agent alias
 * matching part of it.
 *
 * @param string $alias
 * @param $trash1
 ...
 * @return void
 */
function api_get_event_mcid($trash1, $console_event_id, $trash2, $returnType)
{
    try {
...
{
    // Get the event
    $event = events_get_event($id, false);
    // If event not exists, end the execution.
    if ($event === false) {
        returnError(
            'event_not_exists',
