 ...
 */
function agents_get_agent($id_agent, $show_disabled=true)
{
 ...
 * @param string  $separator         Separator for the serialized data. By default |.
...
function agents_get_all_groups_agent($id_agent, $group=false)
{
    ...
    if ($group === false) {
        $group = agents_get_agent_group($id_agent);
    }
...
function agents_check_access_agent($id_agent, $access='AR')
{
    ...
    if (users_access_to_agent($id_agent, $access, false)) {
        return true;
    }

    // If agent exist return false
    if (agents_check_agent_exists($id_agent, true)) {
        return false;
    }
...
function agents_get_agent_id_by_alias_regex($alias_regex, $flag='i', $limit=0)
{
    $agents_id = [];
        ...
        $all_agents = agents_get_group_agents(0, true, 'lower', false, false, '|');
...
/**
 * Check if all configuration datafiles of an agent are writable.
 *
 * @param array $config_filenames Array of configuration datafiles.
 *
 * @return boolean True if all configuration datafiles are writable or false in other case.
 */
function config_agents_is_agent_config_writable($config_filenames)
{
    if (! is_array($config_filenames)) {
        return false;
    }

    if (! isset($config_filenames['conf'])) {
        return false;
    }

    if (! isset($config_filenames['md5'])) {
        return false;
    }

    return (file_exists($config_filenames['conf'])
        && is_writable($config_filenames['conf'])
        && file_exists($config_filenames['md5'])
        && is_writable($config_filenames['md5']));
}


/**
 * Check if an agent has remote configuration
 *
 * @param int agent id
 *
 * @return boolean True if has remote configuration, False otherwise
 */
function config_agents_has_remote_configuration($agent_id)
{
    global $config;

    $agent_data = db_get_row_filter('tagente', ['id_agente' => $agent_id]);

    $agent_name = io_safe_output($agent_data['nombre']);
    if (!$agent_name) {
        return false;
    } else {
        $agent_md5 = md5($agent_name, false);
        $filename = [];
        $filename['md5'] = io_safe_output($config['remote_config']).'/md5/'.$agent_md5.'.md5';
        $filename['conf'] = io_safe_output($config['remote_config']).'/conf/'.$agent_md5.'.conf';
    }

    return config_agents_is_agent_config_writable($filename);
}


/**
 * Get all configuration datafiles of an agent.
 *
 * @param integer $id_agent The id of agent.
 *
 * @return array Array of agent's configuration datafiles.
 */
function config_agents_get_agent_config_filenames($id_agent)
{
    global $config;
    ...
    if (empty($id_agent)) {
        ...
        $agent_name = io_safe_output(agents_get_name($id_agent, 'none'));

        $file = md5($agent_name, false);
        $file_md5 = io_safe_output($config['remote_config']).'/md5/'.$file.'.md5';
        $file_conf = io_safe_output($config['remote_config']).'/conf/'.$file.'.conf';
