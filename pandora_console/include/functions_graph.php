...
function progress_bubble($progress, $width, $height, $title='', $mode=1, $value_text=false, $color=false)
{
    ...
    return "<img title='".$title."' alt='".$title."'"." src='".$config['homeurl'].'/include/graphs/fgraph.php?graph_type=progressbubble'.'&width='.$width.'&height='.$height.'&progress='.$progress.'&mode='.$mode.'&out_of_lim_str='.$out_of_lim_str.'&title='.$title.'&value_text='.$value_text.'&colorRGB='.$colorRGB."' />";
}
