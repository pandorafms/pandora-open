...
function load_modal(settings) {
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      // Extract scripts before .html() because jQuery strips them from the DOM.
      var scriptRegex = /<script\b[^>]*>([\s\S]*?)<\/script>/gi;
      var scriptsToEval = [];
      var scriptMatch;
      while ((scriptMatch = scriptRegex.exec(data)) !== null) {
        if (scriptMatch[1].trim()) {
          scriptsToEval.push(scriptMatch[1]);
        }
      }
      settings.target.html(data);
      scriptsToEval.forEach(function(scriptContent) {
        try {
          $.globalEval(scriptContent);
        } catch (e) {
          console.error("load_modal script eval error:", e);
        }
      });
