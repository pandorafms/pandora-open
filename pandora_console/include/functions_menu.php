...
/**
 * Prepare menu data for acl conf.
 *
 * @param  array  $pages
 * @param  string $sec
 * @return string $pages
...
if (is_ajax()) {
    ...
    if ($about) {
        ...
        if ($config['style'] === 'pandora_black') {
            $image_logo = ui_get_full_url('/images/custom_logo/logo-black-pandorafms.png', false, false, false);
        } else {
            $image_logo = ui_get_full_url('/images/custom_logo/logo-default-pandorafms.png', false, false, false);
        }
        
        $image_about = ui_get_full_url('/images/custom_logo/logo-open-hero.png', false, false, false);

        $dialog = '<div id="about-tabs" class="invisible overflow-hidden styled-tabs">';
        $dialog .= '<div class="flex" style="margin: 15px 10px 5px 10px; flex-direction: row;">
                        <div>
                            <img src="'.$image_logo.'" alt="logo">
                        </div>
                        <div class="ui-tabs-close-button w100pi">
                            <img id="about-close" style="cursor: pointer; float: right;" src="'.ui_get_full_url('/include/styles/images/dialog-titlebar-close.png', false, false, false).'" alt="'.__('Close').'" title="'.__('Close').'" width="25px">
                        </div>
                    </div>';
        $dialog .= '<ul>';

        $tabs = [
            ['id' => 'tab-general-view', 'tag' => __('Information')]
        ];

        if ((bool) check_acl($config['id_user'], 0, 'PM') === true || $config['is_admin']['admin'] === true) {
            $tabs[] = ['id' => 'tab-database', 'tag' => __('Database')];
            $tabs[] = ['id' => 'tab-system-info', 'tag' => __('System Info')];
            $tabs[] = ['id' => 'tab-php-system', 'tag' => __('PHP System')];
        }

        $dialog .= ui_print_styled_tablist($tabs);
                        ...
                        <tbody>
                            <tr>
                                <th style="width: 30%; border: 0px;">
                                    <a href="https://pandoraopen.io/" target="_blank" class="flex">
                                        <img src="'.$image_about.'" alt="logo" width="200px">
                                    </a>
                                </th>
                                <th style="width: 70%; text-align: left; border: 0px; padding-left: 20px; vertical-align: initial;">
                                    <p><span class="table-about-title">'.$product_name.'</span></p>
                                    <p><span class="table-about-version">'.__('Version').' 2505'.'</span></p>';
        
        $dialog .= '<p style="margin-top: 16px; margin-bottom: 16px;"><span>'.__('This is the Open Source version of Pandora FMS, licensed under GPL2, without warranty or support. Use it at your own risk.').'</span></p>';
