...
#include "modules/pandora_module_plugin.h"
#include "modules/pandora_module_fim.h"
...
int
Pandora_Windows_Service::killRMMAgent() {
	PROCESS_INFORMATION pi;
	STARTUPINFO         si;		
	string kill_cmd;
	
	kill_cmd = "\"" + Pandora::getPandoraInstallDir () + "util\\pandora_rmm_agent.exe\" -k";
...
Pandora_Windows_Service::getValueFromCmdExec (string cmd_exec, int timeout)
{
	...
	HANDLE              new_stdout, out_read, job;
	string              working_dir;
	string output = "";
...
Pandora_Windows_Service::copyDataFile (string filename, bool secondary_buffer)
{
	...
	if (mode == "tentacle" || mode == "") {
		...
		pandoraLog ("Invalid transfer mode: %s. Allowed modes are tentacle or local.",
			    mode.c_str ());
	}
...
Pandora_Windows_Service::copyToSecondary (string filename, bool secondary_buffer)
{
	...
	if (mode == "tentacle" || mode == "") {
		...
		rc = PANDORA_EXCEPTION;
		pandoraLog ("Invalid secondary transfer mode: %s. Allowed modes are tentacle or local.",
			    mode.c_str ());
	}
...
void Pandora_Windows_Service::fim_plugin() {
    string install_dir = Pandora::getPandoraInstallDir();
    string fim_config_path = install_dir + "fim\\pandora_fim.conf";
    string temporal = conf->getValue("temporal");

    char md5_buffer[33];
    string agent_name = conf->getValue("agent_name") + "_fim";
    
    Pandora_File::md5(agent_name.c_str(), agent_name.length(), md5_buffer);
    string fim_conf_file = string(md5_buffer) + ".conf";
    string fim_md5_file = string(md5_buffer) + ".md5";

    try {
        recvTentacleDataFile(conf->getValue("server_ip"), fim_md5_file, 30);
    } catch (...) {
        pandoraDebug("fim config: No FIM configuration found on server");
        this->removeFIMModule();
        return;
    }

    char* remote_md5_content = NULL;
    char* local_md5_content = NULL;
    bool needUpdate = true;

    try {
        Pandora_File::readBinFile(temporal + "\\" + fim_md5_file, &remote_md5_content);

        DWORD fileAttrs = GetFileAttributes(fim_config_path.c_str());
        if (fileAttrs != INVALID_FILE_ATTRIBUTES && !(fileAttrs & FILE_ATTRIBUTE_DIRECTORY)) {
            Pandora_File::readBinFile(fim_config_path, &local_md5_content);
            needUpdate = (memcmp(local_md5_content, remote_md5_content, 32) != 0);
        }
    } catch (...) {
        needUpdate = true;
    }

    if (local_md5_content != NULL) {
        delete[] local_md5_content;
    }

    if (needUpdate) {
        try {
            CreateDirectory((install_dir + "fim").c_str(), NULL);

            recvTentacleDataFile(conf->getValue("server_ip"), fim_conf_file, 30);

			DeleteFile(fim_config_path.c_str());

            if (!MoveFile(
                (temporal + "\\" + fim_conf_file).c_str(),
                fim_config_path.c_str()
            )) {
                pandoraDebug("fim config: Error moving FIM configuration file");
                delete[] remote_md5_content;
                Pandora_File::removeFile(temporal + "\\" + fim_md5_file);
                this->removeFIMModule();
                return;
            }

            pandoraDebug("fim config: FIM configuration updated");
        } catch (...) {
            pandoraDebug("fim config: Error getting FIM configuration");
            if (remote_md5_content != NULL) {
                delete[] remote_md5_content;
            }
            Pandora_File::removeFile(temporal + "\\" + fim_md5_file);
            this->removeFIMModule();
        ...
        pandoraDebug("fim config: FIM configuration is up to date");
    }
	...
	pandoraDebug("fim config: Getting FIM configuration");
    FILE* fim_config = fopen(fim_config_path.c_str(), "r");
    if (fim_config != NULL) {
        char line[1024];
        bool enabled = false;
		pandoraDebug("fim config: Reading FIM configuration");
        while (fgets(line, sizeof(line), fim_config)) {
        ...
        if (enabled) {
			pandoraDebug("fim enabled: Creating FIM module");
			this->addFIMModuleIfNotExists();
        } else {
            pandoraDebug("fim disabled: Removing FIM module");
            this->removeFIMModule();
        }
    } else {
        pandoraDebug("fim config: Cannot read FIM configuration file");
        this->removeFIMModule();
    }
}


void Pandora_Windows_Service::addFIMModuleIfNotExists() {
	if (this->fimModuleLoaded == true) {
		return;
	}

    bool fim_exists = false;
    if (this->modules != NULL) {
        this->modules->goFirst();
        while (!this->modules->isLast()) {
            Pandora_Module *existing_module = this->modules->getCurrentValue();

            if (existing_module->getModuleKind() == MODULE_PLUGIN) {
                Pandora_Module_Plugin *plugin_module = dynamic_cast<Pandora_Module_Plugin*>(existing_module);
                if (plugin_module != NULL) {
                    string module_exec = plugin_module->getModuleExec();
                    pandoraDebug("Checking module exec: %s", module_exec.c_str());

                    if (module_exec.find("pandora_fim") != string::npos) {
                        fim_exists = true;
                        pandoraDebug("FIM module already exists");
                        break;
                    }
                }
            }
    ...
    if (!fim_exists) {
        Pandora_Module *module = Pandora_Module_FIM::createFIMModule(this->getInterval(), this->getIntensiveInterval());
        if (module != NULL) {
            this->modules->addModule(module);
            pandoraDebug("FIM module added successfully");
			this->fimModuleLoaded = true;
        }
    }
}

void Pandora_Windows_Service::removeFIMModule() {
    if (this->modules == NULL || !this->fimModuleLoaded) {
        return;
    }

    bool fim_found = false;
    
    this->modules->goFirst();
    while (!this->modules->isLast()) {
        Pandora_Module *current_module = this->modules->getCurrentValue();

        if (current_module->getModuleKind() == MODULE_PLUGIN) {
            Pandora_Module_Plugin *plugin_module = dynamic_cast<Pandora_Module_Plugin*>(current_module);
            if (plugin_module != NULL) {
                string module_exec = plugin_module->getModuleExec();
                if (module_exec.find("pandora_fim") != string::npos) {
                    fim_found = true;
                    pandoraDebug("Found FIM module to remove: %s", module_exec.c_str());
                    break;
                }
            }
        }
    ...
    if (fim_found) {
        string conf_file = Pandora::getPandoraInstallDir() + "pandora_agent.conf";

        delete this->modules;
        this->modules = new Pandora_Module_List(conf_file);

        pandoraDebug("Module list recreated without FIM module");
		this->fimModuleLoaded = false;
    } else {
        pandoraDebug("No FIM module found to remove");
    }
}
...
Pandora_Windows_Service::pandora_run (int forced_run) {
	...
	if (getPandoraDebug () == false) {
		...
		/* Start the RMM agent if needed. */
		if (conf->getValue("rmm_enabled") == "1") {
			rmm_path = "start /B \"\" \"" + Pandora::getPandoraInstallDir ();
			rmm_path += "util\\pandora_rmm_agent.exe\" -c \"" + conf_file + "\"";
			system(rmm_path.c_str());
		}
	}
...
Pandora_Windows_Service::generateAgentName () {
	...
	// Generate a 64-bit hash from the string
	std::size_t hashValue = std::hash<std::string>{}(data.str());

	// Convert the numeric hash to a Hex string (looks like a partial SHA)
	std::stringstream hexStream;
	hexStream << std::hex << std::setw(16) << std::setfill('0') << hashValue;
