...
/** 
 * Convert a wide string to UTF-8 encoded standard string.
 * 
 * @param wideString String to convert.
 * 
 * @return A UTF-8 encoded standard string.
 */
string
Pandora_Strutils::strWideToUtf8(const std::wstring& wideString) {

	// Check for empty strings.
	if (wideString.empty()) return std::string();

	// Calculate the size needed for the UTF-8 string (including null terminator).
	...
	std::string utf8String(sizeNeeded - 1, 0); // Exclude null terminator.
