...
public:
	...
	CDhComPtr& operator=(const int null)
	{
...
public:
	...
	CDhStringTemplate(const int null) : m_strptr (NULL)
	{
		if (null != 0) throw(E_POINTER);
	}
	...
	CDhStringTemplate& operator=(const int null)
	{
		if (null != 0) throw(E_POINTER);
		Dispose();
		return *this;
...
/* ===================================================================== */
#ifndef DISPHELPER_NO_EXCEPTIONS
inline bool dhIfFailThrowString(HRESULT hr)
{
	if (FAILED(hr)) dhThrowFunctions::throw_string();
	return true;
}

inline bool dhIfFailThrowWString(HRESULT hr)
{
	if (FAILED(hr)) dhThrowFunctions::throw_wstring();
	return true;
}

inline bool dhIfFailThrowDhException(HRESULT hr)
{
