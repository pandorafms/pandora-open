...
Pandora_Wmi::getOSVersion () {
	...
	try {
		...
		FOR_EACH (quickfix, quickfixes, NULL) {
			dhGetValue (L"%S", &wversion, quickfix, L".CSDVersion");
		
			if (wversion != NULL) {
				std::wstring ws(wversion);
				// Remove the (R) character.
				ws.erase(std::remove(ws.begin(), ws.end(), L'\u00AE'), ws.end());
				ret = Pandora_Strutils::strWideToUtf8(ws);
				dhFreeString (wversion);
			}
		} NEXT_THROW (quickfix);
	} catch (string errstr) {
...
Pandora_Wmi::getOSBuild () {
	...
	try {
		...
		FOR_EACH (quickfix, quickfixes, NULL) {
			dhGetValue (L"%S", &wbuild, quickfix, L".Version");

			if (wbuild != NULL) {
				std::wstring ws(wbuild);
				ret = Pandora_Strutils::strWideToUtf8(ws);
				dhFreeString (wbuild);
			}
		} NEXT_THROW (quickfix);
...
Pandora_Wmi::getSystemName () {
	...
	try {
		...
		FOR_EACH (quickfix, quickfixes, NULL) {
			dhGetValue (L"%S", &wname, quickfix, L".CSName");
    ...
    int          num_objects = 0;
 	try {
		...
		FOR_EACH (nic_info_item, nic_info, NULL) {
            ...
            ret += inventory_field_separator;
			VariantInit(&ip_addresses);
		    if(dhGetValue (L"%v", &ip_addresses, nic_info_item, L".IPAddress") == S_OK)
		    {
				if (ip_addresses.vt != VT_EMPTY && ip_addresses.vt != VT_NULL) {
