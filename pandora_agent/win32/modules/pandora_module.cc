 ...
 */
void
Pandora_Module::setOutput (string output, string metadata) {
	Pandora_Data *data;
 ...
 */
void
Pandora_Module::setOutput (string output, string metadata, SYSTEMTIME *system_time) {
	Pandora_Data *data;
...
Pandora_Module::getXml () {
	ostringstream module_interval, min, max;
 	string        module_xml, data_clean, metadata_clean, interval_str;
	Pandora_Data *data;
	...
	if (this->module_type == TYPE_LOG) {
		...
		module_xml += "]]></source>";
		module_xml += "\n\t<source_type><![CDATA[";
		module_xml += this->source_type == "" ? "syslog" : this->source_type;
		module_xml += "]]></source_type>";
		...
		if (this->data_list && this->data_list->size () > 1) {
			...
			for (iter = this->data_list->begin ();
				...
				metadata_clean = data->getMetadata();
				if (metadata_clean != "") {
					module_xml += "\n\t\t\t<metadata><![CDATA[";
					module_xml += base64Encode(metadata_clean);
					module_xml += "]]></metadata>";
				}
				module_xml += "\n\t\t</data>";
			}
...
Pandora_Module::addGenericCondition (string condition, list<Condition *> **condition_list) {
	...
	/* Regular expression */
		...
		cond->command = "cmd.exe /c \"" + cond->command + "\"";
		try {
			cond->regexp = std::regex(string_value, std::regex_constants::extended);
...
int Pandora_Module::evaluateCondition (string string_value, double double_value, Condition *condition) {
	if ((condition->operation == ">" && double_value > condition->value_1) ||
	    ...
	    (condition->operation == "=" && double_value == condition->value_1) ||
	    (condition->operation == "!=" && double_value != condition->value_1) ||
	    (condition->operation == "=~" && std::regex_search(string_value, condition->regexp)) ||
 ...
 */
void
Pandora_Module::setSourceType (string source_type) {
