/* Pandora logchannel module. This module checks for log events that match a given
   ...
   Copyright (c) 2017-2023 Pandora Open.
...
Pandora_Module_Logchannel::Pandora_Module_Logchannel (string name, string source, string type, string id, string pattern, string application)
	...
	if (!this->pattern.empty()) {
		try {
			// Use POSIX extended syntax to keep compatibility with previous behavior
			this->regexp = std::regex(this->pattern, std::regex_constants::extended);
		} catch (const std::regex_error &e) {
			pandoraLog("Invalid regular expression %s: %s", this->pattern.c_str(), e.what());
			this->pattern.clear();
		}
	}
	this->bookmark_xml = L"";
...
Pandora_Module_Logchannel::run () {
	...
	for (event = event_list.begin (); event != event_list.end(); ++event) {
		// Store the data
		this->setOutput (event->message, event->metadata, &(event->timestamp));
	}
}
...
Pandora_Module_Logchannel::getLogEvents (list<LogChannelList> &event_list) {
	...
	LPCWSTR ppValues[] = {L"Event/System/Provider/@Name", L"Event/System/TimeCreated/@SystemTime"};
 ...
 */
string
Pandora_Module_Logchannel::getEventMetadata(EVT_HANDLE hEvent) {
	DWORD dwReturned = 0;
	DWORD dwBufferSize = 0;
	DWORD dwBufferUsed = 0;
	LPWSTR xml = NULL;
	...
	if (!EvtRenderF(NULL, hEvent, EvtRenderEventXml, dwBufferSize, NULL, &dwBufferUsed, 0)) {
		if (ERROR_INSUFFICIENT_BUFFER == GetLastError()) {
			dwBufferSize = dwBufferUsed;
			xml = (LPWSTR)malloc(dwBufferSize);
			if (xml) {
				EvtRenderF(NULL, hEvent, EvtRenderEventXml, dwBufferSize, xml, &dwBufferUsed, 0);
			} else {
