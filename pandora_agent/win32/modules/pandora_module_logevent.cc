/* Pandora logevent module. This module checks for log events that match a given
   ...
   Copyright (c) 2008-2023 Pandora Open.
...
Pandora_Module_Logevent::Pandora_Module_Logevent (string name, string source, string type, string id, string pattern, string application)
	...
	if (! pattern.empty ()) {
		...
		try {
			this->regexp = std::regex (pattern, std::regex_constants::extended);
			this->regexp_valid = true;
...
Pandora_Module_Logevent::run () {
	...
	// Read events
	this->getLogEvents (event_list, metadata_list);
	...
	for (
		event = event_list.begin (), metadata = metadata_list.begin ();
		event != event_list.end() && metadata != metadata_list.end();
		++event, ++metadata) {
		...
		// Store the data
		this->setOutput (*event, *metadata, &system_time);
	}
}
 ...
 */
int
Pandora_Module_Logevent::getLogEvents (list<string> &event_list, list<string> &metadata_list) {
	...
	while (1) {
		...
		while (read > 0) {		   
			...
			if (filterEvent (pevlr, description) == 0) {
				...
				event_list.push_back (description);

				// Add the event metadata to the list.
				metadata = getEventMetadata (pevlr);
...
Pandora_Module_Logevent::getEventDescription (PEVENTLOGRECORD pevlr, char *message, DWORD flags) {
	...
	for (i = 0; i < pevlr->NumStrings; i++) {
		...
		memcpy(strings[i], (TCHAR *)pevlr + offset, (len + 1) * sizeof(TCHAR));
		offset += len + 1;
	}
...
Pandora_Module_Logevent::getEventDescriptionXPATH (PEVENTLOGRECORD pevlr) {
	...
	// Convert the description from UTF-16 to a UTF-8 encoded string.
	description = strWideToUtf8(pwsMessage);
...
Pandora_Module_Logevent::filterEvent (PEVENTLOGRECORD pevlr, string description) {
	...
	// Pattern filter
	if (! this->pattern.empty () && this->regexp_valid && ! std::regex_search (description, this->regexp)) {
		return -1;
	}
...
/**
 * Gets the metadata associated with the given event as a JSON string.
 *
 * @param event Event log record.
 * @return Returns the metadata string.
 */
string
Pandora_Module_Logevent::getEventMetadata(PEVENTLOGRECORD pevlr) {
	string query;
	wstring pwsQuery;
	wstring pwsPath;
	EVT_HANDLE hResults = NULL;
	...
	if (!EvtRenderF(NULL, hEvents[0], EvtRenderEventXml, dwBufferSize, NULL, &dwBufferUsed, 0)) {
		if (ERROR_INSUFFICIENT_BUFFER == GetLastError()) {
			dwBufferSize = dwBufferUsed;
			xml = (LPWSTR)malloc(dwBufferSize);
			if (xml) {
				EvtRenderF(NULL, hEvents[0], EvtRenderEventXml, dwBufferSize, xml, &dwBufferUsed, 0);
			} else {
				pandoraDebug("malloc error\n");
				EvtCloseF(hContext);
				return "";
			}
		}
		...
		if (ERROR_SUCCESS != GetLastError()) {
			pandoraDebug("EvtRender error %d\n", GetLastError());
	   		EvtCloseF(hContext);
	   		free(xml);
			return "";
		}
	...
	// Convert the metadata to a plain string.
	string xmlStr = strWideToUtf8(xml);
