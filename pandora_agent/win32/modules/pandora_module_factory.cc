...
#define TOKEN_SOURCE_TYPE   ("module_source_type ")
...
Pandora_Module_Factory::getModuleFromDefinition (string definition) {
	...
	string                 macro, module_wait_timeout, module_source_type;
	...
	while (iter != tokens.end()) {
		...
		if (module_source_type == "") {
			module_source_type = parseLine (line, TOKEN_SOURCE_TYPE);
		}
		if (module_eventtype == "") {
	...
	if (macro_list.size () > 0) {
		...
		for (macro_iter = macro_list.begin ();
			...
			if(pos != string::npos) {
				...
				if (module_source_type != "") {
					pos_macro = module_source_type.find(macro_name);
					if (pos_macro != string::npos){
						module_source_type.replace(pos_macro, macro_name.size(), macro_value);
					}
				}
	...
	/* Set the source type */
	if (module_source_type != "") {
		module->setSourceType (module_source_type);
	}
